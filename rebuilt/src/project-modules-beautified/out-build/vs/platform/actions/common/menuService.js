...
B5o = class {
    constructor(e, t, i, r) {
        ...
        this._hiddenStates = new R5o(i, r);
    }
    createMenu(e, t, i) {
    ...
    getMenuActions(e, t, i) {
        ...
        const s = r.getActions(i);
        r.dispose();
        return s;
    }
    getMenuContexts(e) {
        const t = new VDc(e, false);
...
B5o = __decorate([__param(0, fr), __param(1, mo), __param(2, Hi), __param(3, Fn)], B5o);
R5o = class {
    static {
        _Re = this;
    }
    ...
    constructor(e, t) {
        ...
        this._showAllEditorTitleIcons = false;
        try {
        ...
        const s = ["{}", this.defaultVisibleDataStringV0, this.defaultVisibleDataStringV1];
        if (!i || s.includes(r)) {
            i = this.defaultVisibleData;
        }
        this._visibleData = i;
        this._disposables.add(e.onDidChangeValue(0, _Re._visibleCommandsKey, this._disposables)(() => {
            if (!this._ignoreChangeEvent) {
                try {
                    const o = e.get(_Re._visibleCommandsKey, 0, "{}");
                    this._visibleData = JSON.parse(o);
    ...
    setShowAllEditorTitleIcons(e) {
        if (this._showAllEditorTitleIcons !== e) {
            this._showAllEditorTitleIcons = e;
            this._onDidChange.fire();
        }
    }
    isHidden(e, t) {
        if (this.isEditorTitle(e)) {
            if (this._showAllEditorTitleIcons || (this._configurationService.getValue(gSc) ?? []).includes(t)) {
                return false;
            } else {
                return !(this._visibleData[e.id]?.includes(t) ?? false) && !t.startsWith("composer.");
            }
        }
        const i = this._isHiddenByDefault(e, t);
        const r = this._data[e.id]?.includes(t) ?? false;
        if (i) {
            return !r;
        } else {
            return r;
    ...
    updateHidden(e, t, i) {
        ...
        if (this._isHiddenByDefault(e, t)) {
            i = !i;
        }
    ...
    reset(e) {
        if (e === undefined) {
            this._data = Object.create(null);
            this._visibleData = this.defaultVisibleData;
            this._persist();
        } else {
            for (const {
                    id: t
                ...
                if (this._visibleData[t]) {
                    delete this._visibleData[t];
                }
                if (t === st.EditorTitle.id) {
                    this._visibleData[t] = this.defaultVisibleData[t];
                }
            }
            this._persist();
        }
...
R5o = _Re = __decorate([__param(0, Hi), __param(1, Fn)], R5o);
VDc = class oWa {
    constructor(e, t) {
        this._id = e;
        this._collectContextKeysForSubmenus = t;
        this._menuGroups = [];
    ...
    refresh() {
        ...
        for (const i of e) {
            ...
            if (!t || t[0] !== r) {
                t = [r, []];
                this._menuGroups.push(t);
            }
            t[1].push(i);
            this._collectContextKeysAndSubmenuIds(i);
        }
    ...
    _collectContextKeysAndSubmenuIds(e) {
        oWa._fillInKbExprKeys(e.when, this._structureContextKeys);
        if (JBe(e)) {
            if (e.command.precondition) {
                oWa._fillInKbExprKeys(e.command.precondition, this._preconditionContextKeys);
            }
            ...
            or.getMenuItems(e.submenu).forEach(this._collectContextKeysAndSubmenuIds, this);
            this._allMenuIds.add(e.submenu);
        }
    }
...
P5o = R3n = class extends VDc {
        ...
        for (const i of this._menuGroups) {
            ...
            for (const a of s) {
                if (this._contextKeyService.contextMatchesRules(a.when)) {
                    const l = JBe(a);
                    if (l) {
                        this._hiddenStates.setDefaultState(this._id, a.command.id, !!a.isHiddenByDefault);
                    }
                    ...
                    if (l) {
                        ...
                        const d = new R3n(a.submenu, this._hiddenStates, this._collectContextKeysForSubmenus, this._commandService, this._keybindingService, this._contextKeyService).createActionGroups(e);
                        const m = id.join(...d.map(p => p[1]));
                        if (m.length > 0) {
                            (o ??= []).push(new h2(a, u, m));
                        }
                    }
    ...
    static _compareMenuItems(e, t) {
        ...
        if (s < o) {
            ...
            return R3n._compareTitles(JBe(e) ? e.command.title : e.title, JBe(t) ? t.command.title : t.title);
        }
    }
    static _compareTitles(e, t) {
        const i = typeof e == "string" ? e : e.original;
        const r = typeof t == "string" ? t : t.original;
        return i.localeCompare(r);
    }
};
P5o = R3n = __decorate([__param(3, fr), __param(4, mo), __param(5, wi)], P5o);
P3n = class {
    constructor(e, t, i, r, s, o) {
        this._disposables = new Ut();
        this._menuInfo = new P5o(e, t, i.emitEventsForSubmenuChanges, r, s, o);
        const a = new Hu(() => {
        ...
        this._disposables.add(a);
        this._disposables.add(or.onDidChangeMenu(m => {
            for (const p of this._menuInfo.allMenuIds) {
                if (m.has(p)) {
                    a.schedule();
                    break;
                }
        ...
        const l = this._disposables.add(new Ut());
        const u = m => {
            let p = false;
            let g = false;
            let f = false;
            for (const A of m) {
                p = p || A.isStructuralChange;
                g = g || A.isEnablementChange;
                f = f || A.isToggleChange;
                if (p && g && f) {
                    break;
                }
            }
            return {
                menu: this,
                isStructuralChange: p,
                isEnablementChange: g,
                isToggleChange: f
            };
        };
        const d = () => {
            l.add(o.onDidChangeContext(m => {
                const p = m.affectsSome(this._menuInfo.structureContextKeys);
                const g = m.affectsSome(this._menuInfo.preconditionContextKeys);
                const f = m.affectsSome(this._menuInfo.toggledContextKeys);
                if (p || g || f) {
                    this._onDidChange.fire({
                        menu: this,
                        isStructuralChange: p,
