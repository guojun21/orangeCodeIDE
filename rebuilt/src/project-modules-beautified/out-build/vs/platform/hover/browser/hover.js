...
ri();
Kc = xi("hoverService");
Yoe = class extends at {
  get delay() {
    if (this.isInstantlyHovering()) {
      return 0;
    } else if (this.hoverOptions?.dynamicDelay) {
      return e => this.hoverOptions?.dynamicDelay?.(e) ?? this._delay;
    } else {
      return this._delay;
    }
  }
  constructor(e, t, i = {}, r, s) {
    ...
    this.placement = e;
    this.hoverOptions = t;
    this.overrideOptions = i;
    this.configurationService = r;
    this.hoverService = s;
    this.lastHoverHideTime = 0;
    this.timeLimit = 200;
    this.hoverDisposables = this._register(new Ut());
    ...
    this._register(this.configurationService.onDidChangeConfiguration(o => {
      if (o.affectsConfiguration("workbench.hover.delay")) {
        this._delay = this.configurationService.getValue("workbench.hover.delay");
      }
    }));
  }
  showHover(e, t) {
    ...
    for (const o of r) {
      this.hoverDisposables.add(_f(o, "keydown", a => {
        if (a.equals(9)) {
          this.hoverService.hideHover();
        }
      }));
    }
    const s = wf(e.content) ? undefined : typeof e.content == "string" ? e.content.toString() : e.content.value;
...
Yoe = __decorate([__param(3, Fn), __param(4, Kc)], Yoe);
$3t = {
  showHover: function () {
