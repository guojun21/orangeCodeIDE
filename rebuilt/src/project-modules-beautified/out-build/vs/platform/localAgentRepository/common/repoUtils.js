...
function h9A(n) {
  ...
  if (zD(n)) {
    return LXg(n.configPath);
  }
}
function m9A(n) {
  if (oE(n)) {
    return n.uri.path || n.uri.fsPath;
  }
  if (zD(n)) {
    return n.configPath.path || n.configPath.fsPath;
  }
}
function HAi(n) {
  let e;
