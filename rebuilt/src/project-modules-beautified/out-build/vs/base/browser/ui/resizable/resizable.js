...
G9t = class {
  constructor() {
    this._onDidWillResize = new Qe();
    this.onDidWillResize = this._onDidWillResize.event;
    this._onDidResize = new Qe();
    this.onDidResize = this._onDidResize.event;
    this._sashListener = new Ut();
    this._size = new Lu(0, 0);
    this._minSize = new Lu(0, 0);
    this._maxSize = new Lu(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
    this.domNode = document.createElement("div");
    this._eastSash = new Qde(this.domNode, {
    ...
    this._sashListener.add(this._eastSash.onDidChange(i => {
      if (n) {
        t = i.currentX - i.startX;
        this.layout(n.height + e, n.width + t);
        this._onDidResize.fire({
          dimension: this._size,
          done: false,
          east: true
  ...
  enableSashes(n, e, t, i) {
    this._northSash.state = n ? 3 : 0;
    this._eastSash.state = e ? 3 : 0;
    this._southSash.state = t ? 3 : 0;
    this._westSash.state = i ? 3 : 0;
  }
  layout(n = this.size.height, e = this.size.width) {
    ...
    n = Math.max(t, Math.min(r, n));
    e = Math.max(i, Math.min(s, e));
    const o = new Lu(e, n);
    if (!Lu.equals(o, this._size)) {
