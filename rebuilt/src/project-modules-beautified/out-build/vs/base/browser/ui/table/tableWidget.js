...
RGl = class RWb {
  ...
  constructor(e, t, i) {
    this.columns = e;
    this.getColumnSize = i;
    this.templateId = RWb.TemplateId;
  ...
  renderTemplate(e) {
    const t = Rt(e, Ct(".monaco-table-tr"));
    const i = [];
    const r = [];
    for (let o = 0; o < this.columns.length; o++) {
      const a = this.renderers[o];
      const l = Rt(t, Ct(".monaco-table-td", {
        "data-col-index": o
      }));
      l.style.width = `${this.getColumnSize(o)}px`;
      i.push(l);
      r.push(a.renderTemplate(l));
    }
    const s = {
      container: e,
      cellContainers: i,
  ...
  disposeTemplate(e) {
    ...
    th(e.container);
    this.renderedTemplates.delete(e);
  }
  layoutColumn(e, t) {
    for (const {
      cellContainers: i
...
Spg = class extends at {
  ...
  constructor(n, e) {
    ...
    this.column = n;
    this.index = e;
    this._onDidLayout = new Qe();
    this.onDidLayout = this._onDidLayout.event;
    this.element = Ct(".monaco-table-th", {
      "data-col-index": e
    }, n.label);
    if (n.tooltip) {
      this._register(q4().setupManagedHover(Sm("mouse"), this.element, n.tooltip));
    }
  }
  layout(n) {
...
xet = class PWb {
  ...
  constructor(e, t, i, r, s, o) {
    this.virtualDelegate = i;
    this.columns = r;
    this.domId = `table_id_${++PWb.InstanceCount}`;
    this.disposables = new Ut();
    this.cachedWidth = 0;
    this.cachedHeight = 0;
    this.domNode = Rt(t, Ct(`.monaco-table.${this.domId}`));
    const a = r.map((d, m) => this.disposables.add(new Spg(d, m)));
    const l = {
      size: a.reduce((d, m) => d + m.column.weight, 0),
      views: a.map(d => ({
    ...
    this.splitview.el.style.height = `${i.headerRowHeight}px`;
    this.splitview.el.style.lineHeight = `${i.headerRowHeight}px`;
    const u = new RGl(r, s, d => this.splitview.getViewSize(d));
    this.list = this.disposables.add(new JR(e, this.domNode, v0A(i), [u], o));
    In.any(...a.map(d => d.onDidLayout))(([d, m]) => u.layoutColumn(d, m), null, this.disposables);
    this.splitview.onDidSashReset(d => {
      const m = r.reduce((g, f) => g + f.weight, 0);
      const p = r[d].weight / m * this.cachedWidth;
      this.splitview.resizeView(d, p);
    }, null, this.disposables);
    this.styleElement = wC(this.domNode);
    this.style(KIc);
  }
  getColumnLabels() {
  ...
  layout(e, t) {
    e = e ?? QSc(this.domNode);
    t = t ?? KFn(this.domNode);
    this.cachedWidth = t;
    this.cachedHeight = e;
    this.splitview.layout(t);
    const i = e - this.virtualDelegate.headerRowHeight;
    this.list.getHTMLElement().style.height = `${i}px`;
    this.list.layout(i, t);
  }
...
`);
  ...
  focusNext(e = 1, t = false, i) {
    this.list.focusNext(e, t, i);
  }
  focusPrevious(e = 1, t = false, i) {
