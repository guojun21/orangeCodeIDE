...
Op();
Rga();
AE = xi("gitContextService");
Pga = class extends at {
  constructor(e, t, i) {
    super();
    this._workspaceContextService = e;
    this._lifecycleService = t;
    this._gitContextprovider = undefined;
    this._gitIgnoreProvider = undefined;
    this._extensionIsolationEnabled = false;
    this._onDidRunGitStatus = this._register(new Qe());
    this.onDidRunGitStatus = this._onDidRunGitStatus.event;
    this._extensionIsolationEnabled = i.getCursorExtensionsIsolationEnabled();
  }
  async getSubmodules(e) {
    if (!this._gitContextprovider) {
      throw new Error("No git context provider registered");
    }
    return await this._gitContextprovider.getSubmodules(e);
  }
  async waitForGitContextProvider() {
    while (!this._gitContextprovider) {
      await new Promise(e => setTimeout(e, 500));
    }
  }
  async getGitFileBlameWithAbsolutePath(e, t, i) {
    if (!this._gitContextprovider) {
      throw new Error("No full commit provider registered");
    }
  ...
  hasGitContextProvider() {
    return this._gitContextprovider !== undefined;
  }
  hasGitIgnoreProvider() {
    return this._gitIgnoreProvider !== undefined;
  }
  hasCorrectGitIgnoreProvider() {
    if (this._extensionIsolationEnabled) {
      return this.hasGitIgnoreProvider();
    } else {
      return this.hasGitContextProvider();
    }
  }
  ...
  async getCommitMetadataOnly(e, t) {
    if (!this._gitContextprovider) {
      throw new Error("No commit metadata provider registered");
    }
    if (this._gitContextprovider.getCommitMetadataOnly) {
      return this._gitContextprovider.getCommitMetadataOnly(e, t);
    } else {
      return this._gitContextprovider.getFullCommit(e, t);
    }
  }
  async getSingleFileDiff(e, t, i) {
    ...
    if (this._gitContextprovider.getSingleFileDiff) {
      return this._gitContextprovider.getSingleFileDiff(e, t, i);
    }
  }
    ...
    if (t) {
      return new Q9e({
        ...
        diff: t.diff.map(i => ({
          from: i.from,
          to: i.to,
          chunks: i.chunks.map(r => ({
            content: r.content,
            lines: r.changes.map(s => s.content)
          }))
        })),
        author: t.author,
        date: t.date
  ...
  async getLastCommits(e, t = 0, i) {
    ...
    return await this._gitContextprovider.getLastCommits(e, t, undefined, i);
  }
  async getDiff(e) {
    if (!this._gitContextprovider) {
      throw new Error("No full commit provider registered");
  ...
  async areIgnored(e) {
    if (this._extensionIsolationEnabled) {
      if (!this._gitIgnoreProvider) {
        throw new Error("No git ignore provider registered");
      }
  ...
  async isGithubRepo() {
    if (this._gitContextprovider) {
      return await this._gitContextprovider.isGithubRepo();
    } else {
      return false;
    }
  }
};
Pga = __decorate([__param(0, Lr), __param(1, ap), __param(2, Uhn)], Pga);
Vi(AE, Pga, 1);
