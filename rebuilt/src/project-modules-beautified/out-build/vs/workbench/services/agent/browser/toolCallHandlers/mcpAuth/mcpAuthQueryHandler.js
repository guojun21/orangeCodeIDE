...
Tkf = class {
    ...
    async handleMcpAuthRequest(n) {
        const e = this.getMcpService();
        const t = n.args?.serverIdentifier;
        const i = n.args?.toolCallId;
        if (!t || !i) {
            return new nye({
                result: {
                    case: "rejected",
        ...
        if (this.handledToolCalls.has(i)) {
            return new nye({
                result: {
                    case: "rejected",
                    value: new d$e({
        ...
        const u = r.getOrCreateBubbleId({
            toolCallId: i,
            toolIndex: 0,
            modelCallId: i,
            toolCallType: an.MCP_AUTH,
            params: {
                case: "mcpAuthParams",
                value: a
            },
        ...
        if (o.type === "connected") {
            ...
            this.handledToolCalls.delete(i);
            return new nye({
                result: {
                    case: "approved",
                    value: new Nfi()
        ...
        if (d?.userDecision !== undefined) {
            this.handledToolCalls.delete(i);
            if (d.userDecision === "accepted") {
                return this.performAuth(t, u, r);
            } else {
                return new nye({
                    result: {
                        case: "rejected",
    ...
    async performAuth(n, e, t) {
        const i = this.getMcpService();
        const r = this.createAuthFlowClient(e, i, t);
        return zOA({
            serverIdentifier: n,
            client: r,
    ...
    createAuthFlowClient(n, e, t) {
        return {
            getStatus: i => e.statusCache()[i],
            reloadClient: async i => {
