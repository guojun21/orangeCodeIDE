...
Av();
ip();
F1t = dt(3248, "Workspaces");
Fka = class vjb extends rn {
  static {
...
T0i = class yjb extends rn {
  ...
  constructor() {
    super({
      id: yjb.ID,
      title: dt(3251, "Open Folder..."),
      category: Br.File,
      f1: true,
      precondition: Ee.and(XNe.toNegated(), m_.isEqualTo("workspace")),
      keybinding: {
        weight: 200,
        primary: 2093
      }
...
abu = class wjb extends rn {
  ...
  constructor() {
    super({
      id: wjb.ID,
      title: dt(3253, "Open Workspace from File..."),
      category: Br.File,
      f1: true,
      precondition: Pme
    });
...
Oka = class _jb extends rn {
  ...
  constructor() {
    super({
      id: _jb.ID,
      title: dt(3254, "Close Workspace"),
      category: F1t,
      f1: true,
      precondition: Ee.and(m_.notEqualsTo("empty"), npn),
      keybinding: {
        weight: 200,
        primary: Ma(Gm, 36),
        mac: {
...
DBf = class Cjb extends rn {
  ...
  constructor() {
    super({
      id: Cjb.ID,
      title: dt(3255, "Open Workspace Configuration File"),
      category: F1t,
      f1: true,
      precondition: m_.isEqualTo("workspace")
    });
  }
  async run(e) {
    const t = e.get(Lr);
    const i = e.get(yi);
    const r = t.getWorkspace().configuration;
    if (r) {
      await i.openEditor({
        resource: r,
        options: {
...
Uka = class Sjb extends rn {
  ...
  constructor() {
    super({
      ...
      category: F1t,
      f1: true,
      precondition: Ee.or(Pme, m_.isEqualTo("workspace"))
    });
  }
  run(e) {
...
cbu = class kjb extends rn {
  ...
  constructor() {
    super({
      id: kjb.ID,
      title: dt(3256, "Remove Folder from Workspace..."),
      category: F1t,
      f1: true,
      precondition: Ee.and(Tnt.notEqualsTo("0"), Ee.or(Pme, m_.isEqualTo("workspace")))
    });
  }
  async run(e) {
    const t = e.get(fr);
    const i = e.get(uX);
    const r = await t.executeCommand(zqe);
    if (r) {
      await i.removeFolders([r.uri]);
    }
  }
};
lbu = class Ejb extends rn {
  ...
  constructor() {
    super({
      id: Ejb.ID,
      title: dt(3257, "Save Workspace As..."),
      category: F1t,
      f1: true,
      precondition: Pme
    });
  ...
  async run(e) {
    const t = e.get(uX);
    const i = e.get(Lr);
    const r = await t.pickNewWorkspacePath();
    if (r && vOt(r)) {
      switch (i.getWorkbenchState()) {
        case 1:
        case 2:
          {
        ...
        case 3:
          return t.saveAndEnterWorkspace(r);
      }
    }
  }
};
ubu = class xjb extends rn {
  ...
  constructor() {
    super({
      id: xjb.ID,
      title: dt(3258, "Duplicate As Workspace in New Window"),
      category: F1t,
      f1: true,
      precondition: Pme
    });
  ...
  async run(e) {
    ...
    const s = e.get(CM);
    const o = e.get(Cc);
    const a = t.getWorkspace().folders;
    const l = o.remoteAuthority;
    const u = await s.createUntitledWorkspace(a, l);
    await i.copyWorkspaceSettings(u);
    return r.openWindow([{
...
or.appendMenuItem(st.MenubarFileMenu, {
  ...
  order: 2,
  when: Ee.and(XNe.toNegated(), m_.isEqualTo("workspace"))
});
or.appendMenuItem(st.MenubarFileMenu, {
  group: "2_open",
  command: {
...
or.appendMenuItem(st.MenubarFileMenu, {
  ...
  when: Ee.or(Pme, m_.isEqualTo("workspace")),
  order: 1
});
or.appendMenuItem(st.MenubarFileMenu, {
  group: "3_workspace",
...
or.appendMenuItem(st.MenubarFileMenu, {
  ...
  order: 3,
  when: Ee.and(m_.isEqualTo("folder"), npn)
});
or.appendMenuItem(st.MenubarFileMenu, {
  group: "6_close",
  command: {
  ...
  order: 3,
  when: Ee.and(m_.isEqualTo("workspace"), npn)
});
