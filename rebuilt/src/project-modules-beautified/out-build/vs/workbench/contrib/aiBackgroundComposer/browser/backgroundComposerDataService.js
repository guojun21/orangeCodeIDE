...
fN();
Cnu();
Yk = xi("backgroundComposerDataService");
gga = class extends at {
    bcIdForThisWindow() {
    ...
    constructor(e, t, i, r, s, o) {
        ...
        this.workbenchEnvironmentService = r;
        this.fileService = s;
        [this.data, this.setData] = zkA(o);
        const a = d_g(e, OFn);
        const [l, u] = v3(a);
        this.persistentData = l;
        this.setPersistentData = u;
        const d = h_g(e, t, UFn);
        const [m, p] = v3(d);
        this.workspacePersistentData = m;
        this.setWorkspacePersistentData = p;
        this._register(this.storageService.onDidChangeValue(-1, OFn, this._register(new Ut()))(g => {
            const f = d_g(e, OFn);
    ...
    setupEnvironmentJsonWatcher() {
        ...
        if (!e) {
            this.setData("hasEnvironmentJsonOnDisk", false);
            return;
        }
        ...
        this.environmentJsonUri = t;
        this.checkEnvironmentJsonExists();
        this._register(this.fileService.watch(t));
        this._register(this.fileService.onDidFilesChange(i => {
            if (i.contains(t, 1) || i.contains(t, 2)) {
                this.checkEnvironmentJsonExists();
            }
        }));
    }
    async checkEnvironmentJsonExists() {
        ...
        try {
            ...
            console.error(`[Environment JSON] Error checking if environment.json exists: ${e}`);
            this.setData("hasEnvironmentJsonOnDisk", false);
        }
    }
    ...
    getBackgroundComposerFollowUpInputDataHandle() {
        if (!this.data.backgroundComposerFollowUpInputData) {
            throw new Error("No background composer follow up input data");
        }
        return dga(this.data.backgroundComposerFollowUpInputData);
    }
    focusBackgroundComposerFollowUp() {
        if (!this.data.isBranchSelectorOpen) {
            this.data.backgroundComposerFollowUpInputBoxDelegate.focus();
        }
    }
    doesInWindowBCHaveDiffsReactive() {
        const e = this.data.inWindowBackgroundComposer?.detailedDiff;
        return (e && (e.diff && e.diff.diffs && e.diff.diffs.length > 0 || e.submoduleDiffs && e.submoduleDiffs.some(i => i.diff && i.diff.diffs && i.diff.diffs.length > 0))) ?? false;
    }
    getInWindowBcData() {
        if (this.bcIdForThisWindow()) {
            return this.data.inWindowBackgroundComposer;
        }
    }
...
gga = __decorate([__param(0, Hi), __param(1, Lr), __param(2, ku), __param(3, Cc), __param(4, Gr), __param(5, ix)], gga);
Vi(Yk, gga, 2);
