...
dQ();
ns();
T0u = xi("aiApplyToFileActionsService");
aDa = class extends at {
    constructor(e, t, i, r) {
        super();
        this._outlineModelService = e;
        this._textModelService = t;
        this._everythingProviderService = i;
        this._fileService = r;
    }
    async _getTieredSymbols(e) {
        const t = await this._textModelService.createModelReference(e);
        try {
            const i = await this._outlineModelService.getOrCreate(t.object.textEditorModel, Cs.None);
            const r = i.getAllSymbols();
            const s = i.getTopLevelSymbols();
            return [r.filter(l => l.kind === 5), s.filter(l => l.kind === 11), s.filter(l => l.kind === 13), s.filter(l => l.kind === 12), r.filter(l => l.kind === 11 && l.range.endLineNumber - l.range.startLineNumber > 3), r.filter(l => l.kind === 4 && l.range.endLineNumber - l.range.startLineNumber > 3), r.filter(l => l.kind === 13 && l.range.endLineNumber - l.range.startLineNumber > 3)].map(l => l.map(u => ({
                ...u,
                text: t.object.textEditorModel.getValueInRange(u.range)
            })));
        } finally {
            t.dispose();
        }
    ...
    async getImportantSymbolsDefinedInCodeblockThatExistInURICanThrowIfExtHostIsNotReady(e, t) {
        ...
        const i = await hmy(this._textModelService, e);
        const r = [];
        let s;
        try {
...
aDa = __decorate([__param(0, Gne), __param(1, El), __param(2, AU), __param(3, Gr)], aDa);
Vi(T0u, aDa, 1);
