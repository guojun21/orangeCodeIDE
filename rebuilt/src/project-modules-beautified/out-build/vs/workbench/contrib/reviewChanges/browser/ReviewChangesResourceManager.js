...
OEt = class {
  ...
  constructor(n, e, t) {
    ...
    this.composerTextModelService = e;
    this._onDidChangeResources = new Qe();
    this._currentResources = [];
    this._resourcesMap = new Map();
    this._resourcesObservable = tp(this._onDidChangeResources.event, () => this._currentResources);
    this.resources = new qgt(this._resourcesObservable);
    this.contextKeys = {
      appLayoutMultiDiff: true,
      ...(t ? {
  ...
  updateResources(n) {
    ...
    const i = [];
    const r = [];
    const s = this._currentResources.filter(o => {
      const a = this._getResourceKey(o);
      if (e.has(a)) {
        return true;
      } else {
        t = true;
        this._resourcesMap.delete(a);
        i.push(o);
        return false;
      }
    });
    for (const [o, a] of e) {
      if (!this._resourcesMap.has(o)) {
        t = true;
        const u = _N.hydrateSkeleton(a);
        this._resourcesMap.set(o, u);
        s.push(u);
        r.push(u);
      }
    }
  ...
  dispose() {
    ...
    for (const n of this._currentResources) {
      n.dispose();
    }
