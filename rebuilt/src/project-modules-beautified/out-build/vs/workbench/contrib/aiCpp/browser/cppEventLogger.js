...
yn();
Dd();
dce = xi("cppEventLoggerService");
Lfa = class {
  cppClient() {
    return this.cppServiceBackendClient.get();
  }
  constructor(e, t, i, r, s, o, a) {
    this._cppTelemetryService = e;
    this._telemetryService = t;
    this._workspaceContextService = i;
    this._instantiationService = r;
    this._textModelService = s;
    this._reactiveStorageService = o;
    this._aiCodeTrackingService = a;
    this._onDidAcceptAiContent = new Qe();
    this.onDidAcceptAiContent = this._onDidAcceptAiContent.event;
    this._onDidSuggestCpp = new Qe();
    this.onDidSuggestCpp = this._onDidSuggestCpp.event;
    this._onDidRejectCpp = new Qe();
    this.onDidRejectCpp = this._onDidRejectCpp.event;
    this._onDidApplyCppEdits = new Qe();
    this.onDidApplyCppEdits = this._onDidApplyCppEdits.event;
    ...
    this.cppServiceBackendClient = this._instantiationService.createInstance(YS, {
      service: Lvi
    });
  ...
  getExtension(e) {
    ...
    if (i !== undefined && i.length > 0 && i.length < 8) {
      return i.toLowerCase();
    }
  }
  registerExtHostEventLogger(e) {
    this.extHostEventLogger = e;
    return {
      dispose: () => {
        this.extHostEventLogger = undefined;
      }
    };
  }
  recordExtHostEvent(e) {
    if (this._cppTelemetryService.canWeTrackTelem) {
      this.extHostEventLogger?.recordExtHostEvent(e);
    }
  }
  ...
  recordPartialAcceptSuggestionEvent(e, t, i) {
    ...
    const s = {
      ...
      currentlyShownCppSuggestion: {
        modelVersionWhenTheChangeIsFirstIndicatedToTheUserButNotShownInTheModel: t.modelVersionWhenCreated,
        originalText: t.originalText ?? "",
        suggestionId: t.monotonicallyIncreasingSuggestionId,
        suggestionText: t.replaceText,
        rangeOfSuggestionInCurrentModel: r ? {
    ...
    this._cppTelemetryService.recordCppSessionEvent(s);
    this._telemetryService.publicLogCapture("cursor.acceptcppsuggestionpartial");
    ...
    this._aiCodeTrackingService.storeGeneratedContent(a, t.originalText ?? "", t.replaceText, {
      source: rO.TAB,
      fileExtension: this.getExtension(t.uri),
      fileName: a,
      requestId: t.requestId
    }).catch(l => console.error("[AiCodeTracking] Error storing partial accept:", l));
    this._onDidAcceptAiContent.fire({
      file: this._workspaceContextService.asRelativePath(e.uri),
      previousContent: t.originalText ?? "",
      updatedContent: t.replaceText,
      source: rO.TAB,
      range: {
        ...t.range,
        endLineNumber: t.range.endLineNumberInclusive
      },
      requestId: t.requestId,
      cppSource: t.source
    });
  ...
  recordAcceptSuggestionEvent(e, t) {
    ...
    const r = {
      ...
      currentlyShownCppSuggestion: {
        ...
        suggestionId: t.monotonicallyIncreasingSuggestionId,
        suggestionText: t.replaceText,
        rangeOfSuggestionInCurrentModel: i ? {
    ...
    this._cppTelemetryService.recordCppSessionEvent(r);
    this._telemetryService.publicLogCapture("cursor.acceptcppsuggestion");
    ...
    this._aiCodeTrackingService.storeGeneratedContent(o, t.originalText ?? "", t.replaceText, {
      ...
      fileName: o,
      requestId: t.requestId
    }).catch(a => console.error("[AiCodeTracking] Error storing accept:", a));
    this._onDidAcceptAiContent.fire({
      requestId: t.requestId,
      file: this._workspaceContextService.asRelativePath(e.uri),
      previousContent: t.originalText ?? "",
  ...
  recordRejectSuggestionEvent(e, t) {
    ...
    this._cppTelemetryService.recordCppSessionEvent(r);
    this._telemetryService.publicLogCapture("cursor.rejectcppsuggestion");
    const s = this.getExtension(t.uri);
    this.recordCppFate(t.requestId, YKe.REJECT, s, performance.now());
    this._onDidRejectCpp.fire({
      requestId: t.requestId,
      file: this._workspaceContextService.asRelativePath(e.uri),
      previousContent: t.originalText ?? "",
      updatedContent: t.replaceText,
  ...
  recordFinishedCppGeneration(e, t) {
    ...
    this._telemetryService.publicLogCapture("cursor.fullcppsuggestion");
  }
  recordCppSuggestionEvent(e, t, i) {
    ...
    this._cppTelemetryService.recordCppSessionEvent(s);
    this._telemetryService.publicLogCapture("cursor.suggestcpp");
    this._onDidSuggestCpp.fire({
      requestId: t.requestId,
      file: this._workspaceContextService.asRelativePath(e.uri),
      previousContent: t.originalText ?? "",
      updatedContent: t.replaceText,
  ...
  recordModelOpenedEvent(e) {
    ...
    this._telemetryService.publicLogCapture("cursor.modelopened");
  }
  ...
  recordCppFate(e, t, i, r) {
    const s = new T4c({
      requestId: e,
      fate: t,
      performanceNowTime: r,
      extension: i
    });
    const o = new Promise((a, l) => {
      setTimeout(() => l(new Error("Timeout recording CPP fate event")), 2000);
    });
    this.cppClient().then(a => {
      Promise.race([a.recordCppFate(s, {}), o]).then(() => {}).catch(l => {
  ...
  recordAnythingQuickAccessSelectionEvent(e, t, i) {
    ...
    const s = new Set();
    const o = t.map(l => {
      if (l.type === "separator" || l.type === "section" || s.size > 16 && !s.has(l.resource.toString())) {
        return l;
      }
...
Lfa = __decorate([__param(0, Une), __param(1, ea), __param(2, Lr), __param(3, ln), __param(4, El), __param(5, ku), __param(6, Yz)], Lfa);
Vi(dce, Lfa, 1);
