...
Hq();
kk();
ICi = class extends at {
  static {
    this.ID = "chat.relatedFilesWorkingSet";
  }
  constructor(e, t) {
    super();
    this.chatEditingService = e;
    this.chatWidgetService = t;
    ...
    this._register(Oc(i => {
      this.chatEditingService.editingSessionsObs.read(i).forEach(s => {
        ...
        if (o && !this.chatEditingSessionDisposables.has(s.chatSessionId)) {
          this._handleNewEditingSession(s, o);
        }
      });
    }));
  }
  _updateRelatedFileSuggestions(e, t) {
    if (!this._currentRelatedFilesRetrievalOperation && !(e.entries.get().length > 0) && t.attachmentModel.fileAttachments.length !== 0) {
      this._currentRelatedFilesRetrievalOperation = this.chatEditingService.getRelatedFiles(e.chatSessionId, t.getInput(), t.attachmentModel.fileAttachments, Cs.None).then(r => {
        if (!r?.length || !t.viewModel?.sessionId || !t.input.relatedFiles) {
          return;
        }
        const s = this.chatEditingService.getEditingSession(t.viewModel.sessionId);
        if (!s || s.entries.get().length) {
          return;
        }
        ...
        const a = new fu();
        for (const l of r) {
          for (const u of l.files) {
            if (a.size >= 2) {
              break;
            }
            if (!o.has(u.uri)) {
              a.set(u.uri, _(5560, null, u.description));
              o.add(u.uri);
            }
          }
        }
  ...
  _handleNewEditingSession(e, t) {
    ...
    const r = In.debounce(t.inputEditor.onDidChangeModelContent, () => null, 3000);
    i.add(r(() => {
      this._updateRelatedFileSuggestions(e, t);
    }));
    ...
    i.add(t.onDidAcceptInput(() => {
      t.input.relatedFiles?.clear();
      this._updateRelatedFileSuggestions(e, t);
    }));
    this.chatEditingSessionDisposables.set(e.chatSessionId, i);
  }
  dispose() {
    for (const e of this.chatEditingSessionDisposables.values()) {
      e.dispose();
    }
    super.dispose();
  }
};
ICi = __decorate([__param(0, kV), __param(1, M1)], ICi);
c3f = class extends at {
  constructor() {
    super(...arguments);
    this._onDidChange = this._register(new Qe());
    this.onDidChange = this._onDidChange.event;
    this._removedFiles = new lT();
    this._value = [];
  }
  get removedFiles() {
  ...
  remove(n) {
    this._value = this._value.filter(e => !Zc(e.uri, n));
    this._removedFiles.add(n);
    this._onDidChange.fire();
  }
  clearRemovedFiles() {
