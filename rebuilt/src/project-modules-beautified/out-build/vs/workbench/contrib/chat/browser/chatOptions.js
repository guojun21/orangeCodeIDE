...
Jxa = class extends at {
  ...
  constructor(e, t, i, r, s, o, a) {
    super();
    this.foreground = t;
    this.inputEditorBackgroundColor = i;
    this.resultEditorBackgroundColor = r;
    this.configurationService = s;
    this.themeService = o;
    this.viewDescriptorService = a;
    this._onDidChange = this._register(new Qe());
    this.onDidChange = this._onDidChange.event;
    this._register(this.themeService.onDidColorThemeChange(l => this.update()));
    this._register(this.viewDescriptorService.onDidChangeLocation(l => {
      if (l.views.some(u => u.id === e)) {
        this.update();
      }
    }));
    this._register(this.configurationService.onDidChangeConfiguration(l => {
      if (Hxa.relevantSettingIds.some(u => l.affectsConfiguration(u))) {
        this.update();
      }
    }));
    this.update();
  ...
  update() {
    ...
    const t = this.configurationService.getValue("chat")?.editor;
    const i = this.configurationService.getValue("editor.accessibilitySupport");
    this._config = {
      ...
      inputEditor: {
        ...
        accessibilitySupport: i
      },
      resultEditor: {
        backgroundColor: this.themeService.getColorTheme().getColor(this.resultEditorBackgroundColor),
        fontSize: t.fontSize,
        fontFamily: t.fontFamily === "default" ? e.fontFamily : t.fontFamily,
        fontWeight: t.fontWeight,
        lineHeight: t.lineHeight ? t.lineHeight : Hxa.lineHeightEm * t.fontSize,
