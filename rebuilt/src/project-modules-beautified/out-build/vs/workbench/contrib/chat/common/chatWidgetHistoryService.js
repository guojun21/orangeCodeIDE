...
lcu();
SS();
ucu = xi("IChatWidgetHistoryService");
dcu = 40;
oAa = class {
  constructor(e) {
    this._onDidClearHistory = new Qe();
    this.onDidClearHistory = this._onDidClearHistory.event;
    this.memento = new EM("interactive-session", e);
    const t = this.memento.getMemento(1, 1);
    for (const i in t.history) {
      t.history[i] = t.history[i].map(r => typeof r == "string" ? {
        text: r
      } : r);
    }
    this.viewState = t;
  ...
  saveHistory(e, t) {
    ...
    this.viewState.history[i] = t.slice(-dcu);
