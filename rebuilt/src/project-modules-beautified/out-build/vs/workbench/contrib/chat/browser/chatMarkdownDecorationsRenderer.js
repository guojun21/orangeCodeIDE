...
o5f();
Oly();
pxa = "http://_vscodedecoration_";
fSi = "http://_chatagent_";
Ryu = "http://_chatslash_";
bSi = class {
  constructor(e, t, i, r, s, o, a, l, u, d, m) {
    ...
    this.hoverService = s;
    this.chatService = o;
    this.chatWidgetService = a;
    this.commandService = l;
    this.labelService = u;
    this.toolsService = d;
    this.chatMarkdownAnchorService = m;
  }
  convertParsedRequestToMarkdown(e) {
    ...
    for (const i of e.parts) {
      if (i instanceof Aie) {
        t += i.text;
      } else if (i instanceof wQ) {
        t += this.instantiationService.invokeFunction(r => a5f(i.agent, false, r));
      } else {
        t += this.genericDecorationToMarkdown(i);
      }
    }
    return t;
  }
  genericDecorationToMarkdown(e) {
    const t = e instanceof dpn && e.data instanceof je ? e.data : undefined;
    const r = {
    ...
    t.querySelectorAll("a").forEach(r => {
      ...
      if (s) {
        if (s.startsWith(fSi)) {
          ...
          if (o) {
            r.parentElement.replaceChild(this.renderAgentWidget(o, i), r);
          }
          ...
          if (o) {
            r.parentElement.replaceChild(this.renderSlashCommandWidget(r.textContent, o, i), r);
          }
        ...
        } else if (s.startsWith(scu)) {
          this.renderFileWidget(e, s, r, i);
        } else if (s.startsWith("command:")) {
          this.injectKeybindingHint(r, s, this.keybindingService);
        }
      }
    });
    return i;
  ...
  renderAgentWidget(e, t) {
    ...
    if (e.isClickable) {
      ...
      const a = t.add(new pw(r, {
        buttonBackground: zo(rbn),
        buttonForeground: zo(sbn),
        buttonHoverBackground: undefined
      }));
      a.label = i;
      t.add(a.onDidClick(() => {
        const l = this.chatAgentService.getAgent(e.agentId);
        const u = this.chatWidgetService.lastFocusedWidget;
        if (!!u && !!l) {
          this.chatService.sendRequest(u.viewModel.sessionId, l.metadata.sampleRequest ?? "", {
            location: u.location,
            agentId: l.id,
            userSelectedModelId: u.input.currentLanguageModel,
            mode: u.input.currentMode
          });
        }
      }));
    } else {
      r = this.renderResourceWidget(i, undefined, t);
    }
    const s = this.chatAgentService.getAgent(e.agentId);
    const o = new Ob(() => t.add(this.instantiationService.createInstance(ibn)));
    t.add(this.hoverService.setupManagedHover(Sm("element"), r, () => {
      o.value.setAgent(e.agentId);
      return o.value.domNode;
    }, s && Tyu(() => s, this.commandService)));
    return r;
  }
  renderSlashCommandWidget(e, t, i) {
    const r = Ct("span.chat-agent-widget.chat-command-widget");
    const s = this.chatAgentService.getAgent(t.agentId);
    const o = i.add(new pw(r, {
      buttonBackground: zo(rbn),
      buttonForeground: zo(sbn),
      buttonHoverBackground: undefined
    }));
    ...
    i.add(o.onDidClick(() => {
      ...
      this.chatService.sendRequest(a.viewModel.sessionId, l?.sampleRequest ?? "", {
        ...
        userSelectedModelId: a.input.currentLanguageModel,
        mode: a.input.currentMode
      });
    }));
    return r;
  }
  renderFileWidget(e, t, i, r) {
    const s = je.parse(t);
    const o = e.inlineReferences?.[s.path.slice(1)];
    if (!o) {
  ...
  renderResourceWidget(e, t, i) {
    const r = Ct("span.chat-resource-widget");
    const s = Ct("span", undefined, e);
    if (t?.title) {
      i.add(this.hoverService.setupManagedHover(Sm("element"), r, t.title));
    }
    r.appendChild(s);
    return r;
  }
