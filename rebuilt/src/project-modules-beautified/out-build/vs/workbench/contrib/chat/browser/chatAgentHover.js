...
v0();
axa();
ibn = class extends at {
    constructor(e, t, i) {
        super();
        this.chatAgentService = e;
        this.extensionService = t;
        this.chatAgentNameService = i;
        this._onDidChangeContents = this._register(new Qe());
        ...
        this.icon = r.icon;
        this.name = r.name;
        this.extensionName = r.extensionName;
        this.description = r.description;
        r.separator.textContent = "|";
        const s = Ct("span.extension-verified-publisher", undefined, tL(gEt));
        this.publisherName = Ct("span.chat-agent-hover-publisher-name");
        Rt(r.publisher, s, this.publisherName);
        r.warning.appendChild(tL(Be.warning));
        r.warning.appendChild(Ct("span", undefined, _(5181, null)));
    }
    setAgent(e) {
        const t = this.chatAgentService.getAgent(e);
        if (t.metadata.icon instanceof je) {
            const s = Ct("img.icon");
            s.src = og.uriToBrowserUri(t.metadata.icon).toString(true);
            this.icon.replaceChildren(Ct(".avatar", undefined, s));
        } else if (t.metadata.themeIcon) {
            const s = Ct(Qt.asCSSSelector(t.metadata.themeIcon));
            this.icon.replaceChildren(Ct(".avatar.codicon-avatar", undefined, s));
        }
        this.domNode.classList.toggle("noExtensionName", !!t.isDynamic);
        const i = this.chatAgentNameService.getAgentNameRestriction(t);
        this.name.textContent = i ? `@${t.name}` : dyi(t);
        this.extensionName.textContent = t.extensionDisplayName;
        this.publisherName.textContent = t.publisherDisplayName ?? t.extensionPublisherId;
        let r = t.description ?? "";
        if (r) {
            if (!r.match(/[\.\?\!] *$/)) {
                r += ".";
        ...
        this.description.textContent = r;
        this.domNode.classList.toggle("allowedName", i);
        ...
        if (!t.isDynamic) {
            ...
            this.extensionService.getExtensions([{
                ...
                s.dispose();
                if (o[0]?.publisherDomain?.verified) {
