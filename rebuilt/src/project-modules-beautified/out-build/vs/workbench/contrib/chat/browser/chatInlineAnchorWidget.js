...
gSi = class extends at {
  ...
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g) {
    super();
    this.element = e;
    this.inlineReference = t;
    this._isDisposed = false;
    this.data = "uri" in t.inlineReference ? t.inlineReference : "name" in t.inlineReference ? {
      kind: "symbol",
      symbol: t.inlineReference
    } : {
      uri: t.inlineReference
    };
    const f = this._register(i.createScoped(e));
    this._chatResourceContext = ACi.bindTo(f);
    e.classList.add(Byu.className, "show-file-icons");
    let A;
    let w;
    let C;
    let x;
    if (this.data.kind === "symbol") {
      ...
      C = this.data.symbol.location;
      A = this.data.symbol.name;
      w = ["codicon", ...yS(m, u, undefined, undefined, $oe.toIcon(M.kind))];
      this._store.add(a.invokeFunction(O => qAu(O, e, f, {
        value: M.location,
        name: M.name,
        kind: M.kind
      ...
      const $ = () => yS(m, u, C.uri, O, O === xg.FOLDER && !g.getFileIconTheme().hasFolderIcons ? mVe : undefined);
      w = $();
      const H = () => {
    ...
    this._register(new Ep(f, s, u, m)).set(C.uri);
    this._chatResourceContext.set(C.uri.toString());
    const B = Ct("span.icon");
    B.classList.add(...w);
    e.replaceChildren(B, Ct("span.icon-label", {}, A));
    ...
    this._register(o.setupManagedHover(Sm("element"), e, N));
    if (this.data.kind !== "symbol") {
      e.draggable = true;
      this._register(ei(e, "dragstart", M => {
        const O = {
          resource: C.uri,
          selection: C.range
        };
        a.invokeFunction($ => Yme($, [O], M));
...
gSi = Byu = __decorate([__param(2, wi), __param(3, kc), __param(4, Gr), __param(5, Kc), __param(6, ln), __param(7, Ol), __param(8, Jl), __param(9, xd), __param(10, Il), __param(11, ea), __param(12, bo)], gSi);
Dt(class azb extends rn {
  static {
  ...
  constructor() {
    super({
      id: azb.id,
      title: dt(5345, "Add File to Chat"),
      menu: [{
        id: st.ChatInlineResourceAnchorContext,
        group: "chat",
        order: 1,
        when: dB.negate()
      }]
...
Dt(class lzb extends rn {
  ...
  constructor() {
    super({
      id: lzb.id,
      title: dt(5347, "Open to the Side"),
      f1: false,
      precondition: ACi,
      keybinding: {
        weight: 402,
  ...
  getTarget(e, t) {
    ...
    if (r) {
      if (r.data.kind === "symbol") {
        return r.data.symbol.location;
      } else {
        return r.data.uri;
      }
    }
...
Dt(class uzb extends rn {
  ...
  constructor() {
    super({
      id: uzb.id,
      title: {
        ...dt(5348, "Go to Definition"),
        mnemonicTitle: _(5341, null)
      },
      menu: [st.ChatInlineSymbolAnchorContext, st.ChatInputSymbolAttachmentContext].map(e => ({
        id: e,
        group: "4_symbol_nav",
        order: 1.1,
        when: Ci.hasDefinitionProvider
      }))
    });
  }
  async run(e, t) {
...
Dt(class dzb extends rn {
  ...
  constructor() {
    super({
      id: dzb.id,
      title: {
        ...dt(5349, "Go to Type Definitions"),
        mnemonicTitle: _(5342, null)
      },
      menu: [st.ChatInlineSymbolAnchorContext, st.ChatInputSymbolAttachmentContext].map(e => ({
        id: e,
        group: "4_symbol_nav",
        order: 1.1,
        when: Ci.hasTypeDefinitionProvider
      }))
    });
  }
  async run(e, t) {
...
Dt(class hzb extends rn {
  ...
  constructor() {
    super({
      id: hzb.id,
      title: {
        ...dt(5350, "Go to Implementations"),
        mnemonicTitle: _(5343, null)
      },
      menu: [st.ChatInlineSymbolAnchorContext, st.ChatInputSymbolAttachmentContext].map(e => ({
        id: e,
        group: "4_symbol_nav",
        order: 1.2,
        when: Ci.hasImplementationProvider
      }))
    });
  }
  async run(e, t) {
...
Dt(class mzb extends rn {
  ...
  constructor() {
    super({
      id: mzb.id,
      title: {
        ...dt(5351, "Go to References"),
        mnemonicTitle: _(5344, null)
      },
      menu: [st.ChatInlineSymbolAnchorContext, st.ChatInputSymbolAttachmentContext].map(e => ({
        id: e,
        group: "4_symbol_nav",
        order: 1.3,
        when: Ci.hasReferenceProvider
      }))
    });
  }
  async run(e, t) {
