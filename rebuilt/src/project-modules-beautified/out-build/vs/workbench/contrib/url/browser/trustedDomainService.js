...
u9f();
yn();
cbn = xi("ITrustedDomainService");
Uxa = class extends at {
  constructor(e, t) {
    super();
    this._instantiationService = e;
    this._storageService = t;
    this._onDidChangeTrustedDomains = this._register(new Qe());
    this.onDidChangeTrustedDomains = this._onDidChangeTrustedDomains.event;
    const i = () => new T5e(bi, () => {
      const {
        defaultTrustedDomains: r,
        trustedDomains: s
      } = this._instantiationService.invokeFunction(Mxa);
      return [...r, ...s];
    });
    this._staticTrustedDomainsResult = i();
    this._register(this._storageService.onDidChangeValue(-1, s2e, this._store)(() => {
