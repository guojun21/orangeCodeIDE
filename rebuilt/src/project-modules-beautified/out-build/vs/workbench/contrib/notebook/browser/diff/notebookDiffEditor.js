...
q6f = class Tzb {
  ...
  compare(e) {
    if (!(e instanceof Tzb) || this.selections.length !== e.selections.length) {
      return 3;
    }
    for (let t = 0; t < this.selections.length; t++) {
      if (this.selections[t] !== e.selections[t]) {
        return 3;
      }
    }
    return 1;
...
m5 = class extends fD {
  ...
  constructor(e, t, i, r, s, o, a, l, u, d) {
    ...
    this._model = null;
    this._modifiedResourceDisposableStore = this._register(new Ut());
    this._insetModifyQueueByOutputId = new KFt();
    this._onDidDynamicOutputRendered = this._register(new Qe());
    ...
    this._currentChangedIndex = Ua(this, -1);
    this.currentChangedIndex = this._currentChangedIndex;
    this.pendingLayouts = new WeakMap();
    this.diffEditorCalcuator = this.instantiationService.createInstance(Oki, this.fontInfo.lineHeight);
    this._notebookOptions = t.createInstance(Trt, this.window, false, undefined);
  ...
  isOverviewRulerEnabled() {
    return this.configurationService.getValue(yo.diffOverviewRuler) ?? false;
  }
  getSelection() {
  ...
  setScrollPosition(e) {
    if (this._list) {
      this._list.scrollTop = e.scrollTop;
      if (e.scrollLeft !== undefined) {
        this._list.scrollLeft = e.scrollLeft;
      }
    }
  }
  ...
  updateOutputHeight(e, t, i, r) {
    const s = e.diffElement;
    const o = this.getCellByInfo(e);
    ...
    if (s instanceof mwe) {
      ...
      s.updateOutputHeight(s.type === "insert" ? s1.Modified : s1.Original, a, i);
    }
    if (r) {
  ...
  async toggleInlineView() {
    ...
    if (this._lastLayoutProperties) {
      ...
      this._layoutCancellationTokenSource = new Wc();
      this.updateLayout(this._layoutCancellationTokenSource.token);
    }
  }
  createEditor(e) {
    this._rootElement = Rt(e, Ct(".notebook-text-diff-editor"));
    ...
    this._listViewContainer = Rt(this._rootElement, Ct(".notebook-diff-list-view"));
    this._list = this.instantiationService.createInstance(AIa, "NotebookTextDiff", this._listViewContainer, this.instantiationService.createInstance(vIa, this.window), t, this.contextKeyService, {
      ...
      paddingBottom: 0,
      styleController: i => this._list,
      overrideStyles: {
        listBackground: Wm,
        listActiveSelectionBackground: Wm,
        listActiveSelectionForeground: ym,
        listFocusAndSelectionBackground: Wm,
        listFocusAndSelectionForeground: ym,
        listFocusBackground: Wm,
        listFocusForeground: ym,
        listHoverForeground: ym,
        listHoverBackground: Wm,
        listHoverOutline: nN,
        listFocusOutline: nN,
        listInactiveSelectionBackground: Wm,
        listInactiveSelectionForeground: ym,
        listInactiveFocusBackground: Wm,
        listInactiveFocusOutline: Wm
      },
    ...
    this.inlineDiffWidget = this._register(this.instantiationService.createInstance(LIa, this._rootElement, this.group.id, this.window, this.notebookOptions, this._dimension));
    this._register(this._list);
    this._register(this._list.onMouseUp(i => {
      if (i.element) {
        if (typeof i.index == "number") {
          this._list.setFocus([i.index]);
        }
        this._onMouseUp.fire({
          event: i.browserEvent,
          target: i.element
        });
      }
    }));
    ...
    this._webviewTransparentCover = Rt(this._list.rowsContainer, $6f(".webview-cover"));
    this._webviewTransparentCover.style.display = "none";
    this._register($Be(this._overflowContainer, i => {
      if (i.target.classList.contains("slider") && this._webviewTransparentCover) {
  ...
  _updateOutputsOffsetsInWebview(e, t, i, r, s) {
    i.element.style.height = `${t}px`;
    if (i.insetMapping) {
      const o = [];
      const a = [];
      i.insetMapping.forEach((l, u) => {
        const d = r(l.cellInfo.diffElement);
        if (!!d && this._list.indexOf(l.cellInfo.diffElement) !== undefined) {
          if (d.outputsViewModels.indexOf(u) < 0) {
            a.push(u);
          } else {
            const p = this._list.getCellViewScrollTop(l.cellInfo.diffElement);
            const g = d.outputsViewModels.indexOf(u);
            const f = l.cellInfo.diffElement.getOutputOffsetInCell(s, g);
            o.push({
              cell: d,
              output: u,
              cellTop: p,
  ...
  async setInput(e, t, i, r) {
    ...
    if (this._model !== null) {
      if (this._inlineView) {
        ...
        this.inlineDiffWidget?.show(e, s.modified.notebook, s.original.notebook, t);
      } else {
      ...
      this._layoutCancellationTokenSource = new Wc();
      this._modifiedResourceDisposableStore.add(In.any(this._model.original.notebook.onDidChangeContent, this._model.modified.notebook.onDidChangeContent)(o => {
        if (this._model !== null && this.editorService.activeEditor !== e) {
          this._layoutCancellationTokenSource?.dispose();
          this._layoutCancellationTokenSource = new Wc();
          this.updateLayout(this._layoutCancellationTokenSource.token);
        }
      }));
      await this._createOriginalWebview(Wr(), this._model.original.viewType, this._model.original.resource);
      if (this._originalWebview) {
        this._modifiedResourceDisposableStore.add(this._originalWebview);
      }
      await this._createModifiedWebview(Wr(), this._model.modified.viewType, this._model.modified.resource);
      if (this._modifiedWebview) {
        this._modifiedResourceDisposableStore.add(this._modifiedWebview);
      }
      await this.updateLayout(this._layoutCancellationTokenSource.token, t?.cellSelections ? Qne(t.cellSelections) : undefined);
    }
  }
  setVisible(e) {
    super.setVisible(e);
  ...
  _attachModel(e) {
    this._model = e;
    this._eventDispatcher = new D_u();
    const t = () => {
      r_(this.window, () => {
        if (!this._isDisposed) {
          if (this._modifiedWebview) {
            this._updateOutputsOffsetsInWebview(this._list.scrollTop, this._list.scrollHeight, this._modifiedWebview, r => r.modified, s1.Modified);
          }
          if (this._originalWebview) {
            this._updateOutputsOffsetsInWebview(this._list.scrollTop, this._list.scrollHeight, this._originalWebview, r => r.original, s1.Original);
          }
        }
      });
    };
    ...
    this._localStore.add(this._list.onDidChangeFocus(r => {
      if (r.indexes.length && this.notebookDiffViewModel && r.indexes[0] < this.notebookDiffViewModel.items.length) {
        const s = this.notebookDiffViewModel.items[r.indexes[0]];
        const o = this.notebookDiffViewModel.items.filter(a => a.type !== "unchanged" && a.type !== "unchangedMetadata" && a.type !== "placeholder");
        if (s && s?.type !== "placeholder" && s?.type !== "unchanged" && s?.type !== "unchangedMetadata") {
          return this._currentChangedIndex.set(o.indexOf(s), undefined);
        }
      }
    ...
    const i = this.notebookDiffViewModel = this._register(new j_u(this._model, this.notebookEditorWorkerService, this.configurationService, this._eventDispatcher, this.notebookService, this.diffEditorCalcuator, this.fontInfo, undefined));
    this._localStore.add(this.notebookDiffViewModel.onDidChangeItems(r => {
      ...
      if (this._revealFirst && typeof r.firstChangeIndex == "number" && r.firstChangeIndex > -1 && r.firstChangeIndex < this._list.length) {
        this._revealFirst = false;
        this._list.setFocus([r.firstChangeIndex]);
        this._list.reveal(r.firstChangeIndex, 0.3);
      }
      this._list.splice(r.start, r.deleteCount, r.elements);
      if (this.isOverviewRulerEnabled()) {
        this._overviewRuler.updateViewModels(i.items, this._eventDispatcher);
      }
    }));
  }
  async _createModifiedWebview(e, t, i) {
    this._modifiedWebview?.dispose();
    this._modifiedWebview = this.instantiationService.createInstance(wbn, this, e, t, i, {
  ...
  async _createOriginalWebview(e, t, i) {
    this._originalWebview?.dispose();
    this._originalWebview = this.instantiationService.createInstance(wbn, this, e, t, i, {
      ...this._notebookOptions.computeDiffWebviewOptions(),
      fontFamily: this._generateFontFamily()
    }, undefined);
  ...
  setOptions(e) {
    const t = e?.cellSelections ? Qne(e.cellSelections) : undefined;
    if (t) {
      this._list.setFocus(t);
    }
  }
  async updateLayout(e, t) {
    if (!!this._model && !!this.notebookDiffViewModel) {
      await this.notebookDiffViewModel.computeDiff(e);
      if (!e.isCancellationRequested && t) {
        this._list.setFocus(t);
      }
    }
  }
  scheduleOutputHeightAck(e, t, i) {
    ...
    if (r instanceof mwe) {
      ...
      s = r.type === "insert" ? s1.Modified : s1.Original;
    }
  ...
  layoutNotebookCell(e, t) {
    ...
    if (r) {
      this._localStore.delete(r);
    }
    let s;
    const o = r_(this.window, () => {
      this.pendingLayouts.delete(e);
      i(e, t);
      s();
    });
    r = $i(() => {
    ...
    this._localStore.add(r);
    this.pendingLayouts.set(e, r);
  ...
  lastChange() {
    ...
    const e = this.notebookDiffViewModel.items;
    const t = e.slice().reverse().find(r => r.type !== "unchanged" && r.type !== "unchangedMetadata" && r.type !== "placeholder");
  ...
  previousChange() {
    ...
    while (t >= 0) {
      const r = i[t];
      if (r.type !== "unchanged" && r.type !== "unchangedMetadata" && r.type !== "placeholder") {
        break;
      }
      t--;
    }
  ...
  createOutput(e, t, i, r, s) {
    this._insetModifyQueueByOutputId.queue(i.source.model.outputId + (s === s1.Modified ? "-right" : "left"), async () => {
      const o = s === s1.Modified ? this._modifiedWebview : this._originalWebview;
      if (o) {
        if (o.insetMapping.has(i.source)) {
          const a = this._list.getCellViewScrollTop(e);
          const l = t.outputsViewModels.indexOf(i.source);
          const u = e.getOutputOffsetInCell(s, l);
          o.updateScrollTops([{
            cell: t,
            output: i.source,
            cellTop: a,
  ...
  showInset(e, t, i, r) {
    this._insetModifyQueueByOutputId.queue(i.model.outputId + (r === s1.Modified ? "-right" : "left"), async () => {
      ...
      const o = this._list.getCellViewScrollTop(e);
      const a = t.outputsViewModels.indexOf(i);
      const l = e.getOutputOffsetInCell(r, a);
      s.updateScrollTops([{
        cell: t,
        output: i,
        cellTop: o,
  ...
  hideInset(e, t, i) {
    this._modifiedWebview?.hideInset(i);
    this._originalWebview?.hideInset(i);
  }
  getDomNode() {
  ...
  deltaCellOutputContainerClassNames(e, t, i, r) {
    if (e === s1.Original) {
      this._originalWebview?.deltaCellOutputContainerClassNames(t, i, r);
    } else {
      this._modifiedWebview?.deltaCellOutputContainerClassNames(t, i, r);
    }
  }
  getLayoutInfo() {
    if (!this._list) {
  ...
  layout(e, t) {
    this._rootElement.classList.toggle("mid-width", e.width < 1000 && e.width >= 600);
    this._rootElement.classList.toggle("narrow-width", e.width < 600);
    const i = this.isOverviewRulerEnabled();
    this._dimension = e.with(e.width - (i ? Jki.ENTIRE_DIFF_OVERVIEW_WIDTH : 0));
    this._listViewContainer.style.height = `${e.height}px`;
    this._listViewContainer.style.width = `${this._dimension.width}px`;
    if (this._inlineView) {
      this._listViewContainer.style.display = "none";
      this.inlineDiffWidget?.setLayout(e, t);
    } else {
      this.inlineDiffWidget?.hide();
      this._listViewContainer.style.display = "block";
...
m5 = Jki = __decorate([__param(1, ln), __param(2, bo), __param(3, wi), __param(4, IEt), __param(5, Fn), __param(6, ea), __param(7, Hi), __param(8, JA), __param(9, yi)], m5);
Rie(TQ.Base, 10, "notebook-diff-view-viewport-slider");
HI((n, e) => {
  ...
  e.addRule(`
	...
	`);
  e.addRule(`.notebook-text-diff-editor .cell-body { margin: ${hwe}px; }`);
  e.addRule(`.notebook-text-diff-editor .cell-placeholder-body { margin: ${hwe}px 0; }`);
});
