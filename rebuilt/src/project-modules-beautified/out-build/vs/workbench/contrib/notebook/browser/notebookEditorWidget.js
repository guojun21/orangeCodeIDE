...
Nbn = class extends at {
  ...
  set viewModel(e) {
    ...
    this._notebookViewModel = e;
    this._onDidChangeModel.fire(e?.notebookDocument);
  }
  get viewModel() {
  ...
  get activeCellAndCodeEditor() {
    ...
    const [e] = this._list.getFocusedElements();
    const t = this._renderedEditors.get(e);
    if (t) {
      return [e, t];
    }
  }
  ...
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g, f, A, w) {
    ...
    this.notebookRendererMessaging = s;
    this.notebookEditorService = o;
    this.notebookKernelService = a;
    this._notebookService = l;
    this.configurationService = u;
    this.layoutService = m;
    this.contextMenuService = p;
    this.telemetryService = g;
    this.notebookExecutionService = f;
    this.editorProgressService = A;
    this.logService = w;
    this._onDidChangeCellState = this._register(new Qe());
    this.onDidChangeCellState = this._onDidChangeCellState.event;
    this._onDidChangeViewCells = this._register(new Qe());
    ...
    this._onDidAttachViewModel = this._register(new Qe());
    ...
    this._onDidChangeActiveCell = this._register(new Qe());
    this.onDidChangeActiveCell = this._onDidChangeActiveCell.event;
    this._onDidChangeFocus = this._register(new Qe());
    this.onDidChangeFocus = this._onDidChangeFocus.event;
    this._onDidChangeSelection = this._register(new Qe());
    this.onDidChangeSelection = this._onDidChangeSelection.event;
    this._onDidChangeVisibleRanges = this._register(new Qe());
    this.onDidChangeVisibleRanges = this._onDidChangeVisibleRanges.event;
    this._onDidFocusEmitter = this._register(new Qe());
    this.onDidFocusWidget = this._onDidFocusEmitter.event;
    this._onDidBlurEmitter = this._register(new Qe());
    this.onDidBlurWidget = this._onDidBlurEmitter.event;
    this._onDidChangeActiveEditor = this._register(new Qe());
    this.onDidChangeActiveEditor = this._onDidChangeActiveEditor.event;
    this._onDidChangeActiveKernel = this._register(new Qe());
    this.onDidChangeActiveKernel = this._onDidChangeActiveKernel.event;
    this._onMouseUp = this._register(new Qe());
    this.onMouseUp = this._onMouseUp.event;
    this._onMouseDown = this._register(new Qe());
    this.onMouseDown = this._onMouseDown.event;
    this._onDidReceiveMessage = this._register(new Qe());
    this.onDidReceiveMessage = this._onDidReceiveMessage.event;
    this._onDidRenderOutput = this._register(new Qe());
    this.onDidRenderOutput = this._onDidRenderOutput.event;
    this._onDidRemoveOutput = this._register(new Qe());
    this.onDidRemoveOutput = this._onDidRemoveOutput.event;
    this._onDidResizeOutputEmitter = this._register(new Qe());
    ...
    this._localStore = this._register(new Ut());
    this._localCellStateListeners = [];
    this._shadowElementViewInfo = null;
    this._contributions = new Map();
    this._insetModifyQueueByOutputId = new KFt();
    ...
    this._dimension = t;
    this.isReplHistory = e.isReplHistory ?? false;
    this._readOnly = e.isReadOnly ?? false;
    this._overlayContainer = document.createElement("div");
    this.scopedContextKeyService = this._register(d.createScoped(this._overlayContainer));
    this.instantiationService = this._register(i.createChild(new EA([wi, this.scopedContextKeyService])));
    this._notebookOptions = e.options ?? this.instantiationService.createInstance(Trt, this.creationOptions?.codeWindow ?? bi, this._readOnly, undefined);
    this._register(this._notebookOptions);
    const C = this._register(new U8f());
    this._viewContext = new W8f(this._notebookOptions, C, R => this.getBaseCellEditorOptions(R));
    ...
    this._register(this.instantiationService.createInstance(YTa, this));
    this._register(a.onDidChangeSelectedNotebooks(R => {
      if (Zc(R.notebook, this.viewModel?.uri)) {
    ...
    this._register(this.configurationService.onDidChangeConfiguration(R => {
      if (R.affectsConfiguration("editor.scrollBeyondLastLine")) {
        this._scrollBeyondLastLine = this.configurationService.getValue("editor.scrollBeyondLastLine");
    ...
    this._register(this._notebookOptions.onDidChangeOptions(R => {
      if (R.cellStatusBarVisibility || R.cellToolbarLocation || R.cellToolbarInteraction) {
        this._updateForNotebookConfiguration();
      }
      if (R.fontFamily) {
        this._generateFontInfo();
      }
      if (R.compactView || R.focusIndicator || R.insertToolbarPosition || R.cellToolbarLocation || R.dragAndDropEnabled || R.fontSize || R.markupFontSize || R.markdownLineHeight || R.fontFamily || R.insertToolbarAlignment || R.outputFontSize || R.outputLineHeight || R.outputFontFamily || R.outputWordWrap || R.outputScrolling || R.outputLinkifyFilePaths || R.minimalError) {
    ...
    const I = Wr();
    this._overlayContainer.id = `notebook-${I}`;
    ...
    x.appendChild(this._overlayContainer);
    this._createBody(this._overlayContainer);
    this._generateFontInfo();
    this._isVisible = true;
    this._editorFocus = dv.bindTo(this.scopedContextKeyService);
    ...
    new Sn(xki, false).bindTo(this.scopedContextKeyService).set(true);
    this._editorEditable.set(!e.isReadOnly);
    let B;
    if (Array.isArray(this.creationOptions.contributions)) {
      B = this.creationOptions.contributions;
    } else {
      B = Agn.getEditorContributions();
    }
    for (const R of B) {
      let N;
      try {
      ...
      if (N) {
        if (!this._contributions.has(R.id)) {
          this._contributions.set(R.id, N);
        } else {
          N.dispose();
          throw new Error(`DUPLICATE notebook editor contribution: '${R.id}'`);
        }
      }
    }
    this._updateForNotebookConfiguration();
  ...
  getSelectionViewModels() {
    ...
    const e = new Set();
    return this.viewModel.getSelections().map(t => this.viewModel.viewCells.slice(t.start, t.end)).reduce((t, i) => {
      i.forEach(r => {
        if (!e.has(r.handle)) {
          e.add(r.handle);
          t.push(r);
  ...
  getBaseCellEditorOptions(e) {
    ...
    }
    {
      ...
      this._baseCellEditorOptions.set(e, i);
      return i;
    }
  }
  _updateForNotebookConfiguration() {
    ...
    this._overlayContainer.classList.add(`cell-toolbar-${i}`);
  }
  _generateFontInfo() {
    const e = this.configurationService.getValue("editor");
    const t = As(this.getDomNode());
  ...
  _createBody(e) {
    ...
    Rt(e, this._notebookTopToolbarContainer);
    ...
    Rt(e, this._notebookStickyScrollContainer);
    ...
    this._register(this.instantiationService.createInstance(c6f, this, this._list.scrollableElement));
    this._overflowContainer = document.createElement("div");
    this._overflowContainer.classList.add("notebook-overflow-widget-container", "monaco-editor");
    Rt(e, this._overflowContainer);
  }
  _generateFontFamily() {
  ...
  _createLayoutStyles() {
    ...
    const {
      ...
      cellRunGutter: i,
      cellBottomMargin: r,
      codeCellLeftMargin: s,
      markdownCellGutter: o,
      markdownCellLeftMargin: a,
      markdownCellBottomMargin: l,
      markdownCellTopMargin: u,
      collapsedIndicatorHeight: d,
      focusIndicator: m,
      insertToolbarPosition: p,
      outputFontSize: g,
      focusIndicatorLeftMargin: f,
      focusIndicatorGap: A
    } = this._notebookOptions.getLayoutConfiguration();
    const {
      insertToolbarAlignment: w,
      compactView: C,
      fontSize: x
    } = this._notebookOptions.getDisplayOptions();
    const I = this._notebookOptions.getCellEditorContainerLeftMargin();
    const {
      bottomToolbarGap: B,
    ...
    N.push(`
		...
		`);
    if (C) {
      N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row div.cell.code { margin-left: ${I}px; }`);
    } else {
      N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row div.cell.code { margin-left: ${s}px; }`);
    }
    if (m === "border") {
      N.push(`
    ...
    if (p === "betweenCells" || p === "both") {
      N.push(".monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-bottom-toolbar-container { display: flex; }");
      N.push(".monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .cell-list-top-cell-toolbar-container { display: flex; }");
    } else {
      N.push(".monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-bottom-toolbar-container { display: none; }");
      N.push(".monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .cell-list-top-cell-toolbar-container { display: none; }");
    }
    if (w === "left") {
      N.push(`
    ...
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .code-cell-row div.cell.code { margin-left: ${I}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .code-cell-row div.cell.code { margin-left: ${I}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .code-cell-row div.cell { margin-right: ${e}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row div.cell { margin-right: ${e}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row > .cell-inner-container { padding-top: ${t}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row > .cell-inner-container { padding-bottom: ${l}px; padding-top: ${u}px; }`);
    N.push(".notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row > .cell-inner-container.webview-backed-markdown-cell { padding: 0; }");
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .markdown-cell-row > .webview-backed-markdown-cell.markdown-cell-edit-mode .cell.code { padding-bottom: ${l}px; padding-top: ${u}px; }`);
    N.push(`.notebookOverlay .output { margin: 0px ${e}px 0px ${I}px; }`);
    N.push(`.notebookOverlay .output { width: calc(100% - ${I + e}px); }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-comment-container { left: ${I}px; }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-comment-container { width: calc(100% - ${I + e}px); }`);
    N.push(`.monaco-workbench .notebookOverlay .output .output-collapse-container .expandButton { left: -${i}px; }`);
    N.push(`.monaco-workbench .notebookOverlay .output .output-collapse-container .expandButton {
			position: absolute;
			width: ${i}px;
			padding: 6px 0px;
		}`);
    N.push(`.notebookOverlay .output-show-more-container { margin: 0px ${e}px 0px ${I}px; }`);
    N.push(`.notebookOverlay .output-show-more-container { width: calc(100% - ${I + e}px); }`);
    N.push(`.notebookOverlay .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row div.cell.markdown { padding-left: ${i}px; }`);
    N.push(`.monaco-workbench .notebookOverlay > .cell-list-container .notebook-folding-indicator { left: ${(o - 20) / 2 + a}px; }`);
    N.push(`.notebookOverlay > .cell-list-container .notebook-folded-hint { left: ${o + a + 8}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row :not(.webview-backed-markdown-cell) .cell-focus-indicator-top { height: ${t}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row .cell-focus-indicator-side { bottom: ${B}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row.code-cell-row .cell-focus-indicator-left { width: ${I}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row.markdown-cell-row .cell-focus-indicator-left { width: ${s}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row .cell-focus-indicator.cell-focus-indicator-right { width: ${e}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row .cell-focus-indicator-bottom { height: ${r}px; }`);
    N.push(`.notebookOverlay .monaco-list .monaco-list-row .cell-shadow-container-bottom { top: ${r}px; }`);
    N.push(`
    ...
    N.push(`
		...
		`);
    N.push(`.monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-bottom-toolbar-container .monaco-toolbar { height: ${R}px }`);
    N.push(`.monaco-workbench .notebookOverlay > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .view-zones .cell-list-top-cell-toolbar-container .monaco-toolbar { height: ${R}px }`);
    N.push(`.monaco-workbench .notebookOverlay.cell-title-toolbar-right > .cell-list-container > .monaco-list > .monaco-scrollable-element > .monaco-list-rows > .monaco-list-row .cell-title-toolbar {
			right: ${e + 26}px;
		}
...
`);
  ...
  _createCellList() {
    this._body.classList.add("cell-list-container");
    this._dndController = this._register(new $9f(this, this._body));
    ...
    this._listDelegate = this.instantiationService.createInstance($Ta, As(this.getDomNode()));
    this._register(this._listDelegate);
    const i = this.instantiationService.createInstance(cIa, () => this.viewModel, this.isReplHistory);
    this._register(i);
    this._list = this.instantiationService.createInstance(oTa, "NotebookCellList", this._body, this._viewContext.notebookOptions, this._listDelegate, t, this.scopedContextKeyService, {
      ...
      styleController: r => this._list,
      overrideStyles: {
        listBackground: A7e,
        listActiveSelectionBackground: A7e,
        listActiveSelectionForeground: ym,
        listFocusAndSelectionBackground: A7e,
        listFocusAndSelectionForeground: ym,
        listFocusBackground: A7e,
        listFocusForeground: ym,
        listHoverForeground: ym,
        listHoverBackground: A7e,
        listHoverOutline: nN,
        listFocusOutline: nN,
        listInactiveSelectionBackground: A7e,
        listInactiveSelectionForeground: ym,
        listInactiveFocusBackground: A7e,
        listInactiveFocusOutline: A7e
      },
      accessibilityProvider: i
    });
    this._dndController.setList(this._list);
    this._register(this._list);
    this._listViewInfoAccessor = new V9f(this._list);
    this._register(this._listViewInfoAccessor);
    this._register(H_(...t));
    this._listTopCellToolbar = this._register(this.instantiationService.createInstance(XTa, this, this.notebookOptions));
    this._webviewTransparentCover = Rt(this._list.rowsContainer, l6f(".webview-cover"));
    this._webviewTransparentCover.style.display = "none";
    this._register($Be(this._overlayContainer, r => {
      if (r.target.classList.contains("slider") && this._webviewTransparentCover) {
    ...
    this._register(this._list.onDidScroll(r => {
      if (r.scrollTop !== r.oldScrollTop) {
        this._onDidScroll.fire();
        this.clearActiveCellWidgets();
      }
      if (r.scrollTop === r.oldScrollTop && r.scrollHeightChanged) {
        this._onDidChangeLayout.fire();
      }
    }));
    this._focusTracker = this._register(CC(this.getDomNode()));
    ...
    this._register(this.configurationService.onDidChangeConfiguration(r => {
      if (r.affectsConfiguration(i.verbositySettingId)) {
        this._list.ariaLabel = i?.getWidgetAriaLabel();
      }
    }));
  }
  ...
  _registerNotebookActionsToolbar() {
    this._notebookTopToolbar = this._register(this.instantiationService.createInstance(KTa, this, this.scopedContextKeyService, this._notebookOptions, this._notebookTopToolbarContainer));
    ...
    this._notebookStickyScroll = this._register(this.instantiationService.createInstance(kX, this._notebookStickyScrollContainer, this, this._list, e => {
      if (!this.isDisposed) {
        if (this._dimension && this._isVisible) {
          if (e > 0) {
            this.layout(this._dimension);
            this.setScrollTop(this.scrollTop + e);
          } else if (e < 0) {
            this.setScrollTop(this.scrollTop + e);
            this.layout(this._dimension);
          }
  ...
  _updateOutputRenderers() {
    if (!!this.viewModel && !!this._webview) {
      ...
      this.viewModel.viewCells.forEach(e => {
        e.outputsViewModels.forEach(t => {
          if (t.pickedMimeType?.rendererId === uCt) {
            t.resetRenderer();
          }
        });
      });
  ...
  async setModel(e, t, i, r) {
    if (this.viewModel === undefined || !this.viewModel.equal(e)) {
      const s = this._notebookOptions.computeBottomToolbarDimensions(this.viewModel?.viewType);
      this._detachModel();
      await this._attachModel(e, r ?? e.viewType, t, i);
      const o = this._notebookOptions.computeBottomToolbarDimensions(this.viewModel?.viewType);
      if (s.bottomToolbarGap !== o.bottomToolbarGap || s.bottomToolbarHeight !== o.bottomToolbarHeight) {
        this._styleElement?.remove();
        this._createLayoutStyles();
        this._webview?.updateOptions({
          ...this.notebookOptions.computeWebviewOptions(),
      ...
      this.restoreListViewState(t);
    }
    this._restoreSelectedKernel(t);
    this._loadKernelPreloads();
  ...
  _backgroundMarkdownRendering() {
    if (!this._backgroundMarkdownRenderRunning) {
      ...
      Dte(As(this.getDomNode()), e => {
        this._backgroundMarkdownRenderingWithDeadline(e);
      });
    }
  }
  _backgroundMarkdownRenderingWithDeadline(e) {
    const t = Date.now() + e.timeRemaining();
    const i = () => {
      try {
        this._backgroundMarkdownRenderRunning = true;
        if (this._isDisposed || !this.viewModel) {
          return;
        }
  ...
  updateContextKeysOnFocusChange() {
    ...
    if (e) {
      this._cellContextKeyManager ||= this._localStore.add(this.instantiationService.createInstance(YSi, this, e));
      this._cellContextKeyManager.updateForElement(e);
    }
  }
  async setOptions(e) {
    if (e?.isReadOnly !== undefined) {
      this._readOnly = e?.isReadOnly;
    }
    if (!this.viewModel) {
      return;
    }
    ...
    if (t) {
      ...
      if (i) {
        ...
        if (r) {
          i.updateEditState(aw.Editing, "setOptions");
          i.focusMode = Tk.Editor;
          await this.revealRangeInCenterIfOutsideViewportAsync(i, new Zt(r.startLineNumber, r.startColumn, r.endLineNumber || r.startLineNumber, r.endColumn || r.startColumn));
        } else {
          this._list.revealCell(i, e?.cellRevealType ?? 4);
        }
        ...
        if (s) {
          if (t.options?.selection) {
            ...
            const a = new Zt(o.startLineNumber, o.startColumn, o.endLineNumber || o.startLineNumber, o.endColumn || o.startColumn);
            s.setSelection(a);
            s.revealPositionInCenterIfOutsideViewport({
              lineNumber: o.startLineNumber,
              column: o.startColumn
            });
            await this.revealRangeInCenterIfOutsideViewportAsync(i, a);
          }
          if (!t.options?.preserveFocus) {
            s.focus();
          }
        }
      }
    ...
    if (e?.cellSelections) {
      const i = e.cellSelections[0].start;
      const r = this.viewModel.cellAt(i);
      if (r) {
        this.viewModel.updateSelectionsState({
          kind: Wy.Index,
          focus: {
    ...
    if (e?.indexedCellOptions) {
      ...
      if (t) {
        return {
          resource: t.uri,
          options: {
            selection: e.indexedCellOptions.selection,
            preserveFocus: false
          }
        };
      }
  ...
  _detachModel() {
    ...
    Bo(this._localCellStateListeners);
    ...
    if (this.textModel) {
      if (this._webviewResolvePromise) {
        ...
        this._webviewResolvePromise = (async () => {
          ...
          await this._webview.createWebview(this.creationOptions.codeWindow ?? bi);
  ...
  _ensureWebview(e, t, i) {
    ...
    this._webview = this.instantiationService.createInstance(wbn, {
      ...
      getCellByInfo: r.getCellByInfo.bind(r),
      getCellById: r._getCellById.bind(r),
      toggleNotebookCellSelection: r._toggleNotebookCellSelection.bind(r),
      focusNotebookCell: r.focusNotebookCell.bind(r),
      focusNextNotebookCell: r.focusNextNotebookCell.bind(r),
      updateOutputHeight: r._updateOutputHeight.bind(r),
      scheduleOutputHeightAck: r._scheduleOutputHeightAck.bind(r),
      updateMarkupCellHeight: r._updateMarkupCellHeight.bind(r),
      setMarkupCellEditState: r._setMarkupCellEditState.bind(r),
      didStartDragMarkupCell: r._didStartDragMarkupCell.bind(r),
      didDragMarkupCell: r._didDragMarkupCell.bind(r),
      didDropMarkupCell: r._didDropMarkupCell.bind(r),
      didEndDragMarkupCell: r._didEndDragMarkupCell.bind(r),
      didResizeOutput: r._didResizeOutput.bind(r),
      updatePerformanceMetadata: r._updatePerformanceMetadata.bind(r),
      didFocusOutputInputChange: r._didFocusOutputInputChange.bind(r)
    }, e, t, i, {
  ...
  async _attachModel(e, t, i, r) {
    this._ensureWebview(this.getId(), e.viewType, e.uri);
    this.viewModel = this.instantiationService.createInstance(GTa, t, e, this._viewContext, this.getLayoutInfo(), {
      isReadOnly: this._readOnly
    });
    ...
    this._updateForNotebookConfiguration();
    {
      ...
      for (const [a, l] of this._contributions) {
        if (typeof l.restoreViewState == "function") {
          l.restoreViewState(o[a]);
        }
      }
    }
    ...
    this._localStore.add(this._list.onWillScroll(o => {
      if (this._webview?.isResolved()) {
        this._webviewTransparentCover.style.transform = `translateY(${o.scrollTop})`;
      }
    }));
    let s = false;
    this._localStore.add(this._list.onDidChangeContentHeight(() => {
      if (!s) {
        s = true;
        this._localStore.add(r_(As(this.getDomNode()), () => {
          s = false;
    ...
    this._localStore.add(this._list.onDidRemoveCellsFromView(o => {
      ...
      const l = [];
      for (const u of o) {
        if (u.cellKind === zd.Markup) {
          const d = u;
          if (this.viewModel?.viewCells.find(m => m.handle === d.handle)) {
            a.push(d);
          } else {
            l.push(d);
          }
      ...
      this.hideMarkupPreviews(a);
      this.deleteMarkupPreviews(l);
    }));
    await this._warmupWithMarkdownRenderer(this.viewModel, i, r);
    r?.mark("customMarkdownLoaded");
    this._localCellStateListeners = this.viewModel.viewCells.map(o => this._bindCellListener(o));
    this._lastCellWithEditorFocus = this.viewModel.viewCells.find(o => this.getActiveCell() === o && o.focusMode === Tk.Editor) ?? null;
    this._localStore.add(this.viewModel.onDidChangeViewCells(o => {
      if (!this._isDisposed) {
        [...o.splices].reverse().forEach(a => {
          const [l, u, d] = a;
          const m = this._localCellStateListeners.splice(l, u, ...d.map(p => this._bindCellListener(p)));
          Bo(m);
        });
        if (o.splices.some(a => a[2].some(l => l.cellKind === zd.Markup))) {
          this._backgroundMarkdownRendering();
        }
      }
    }));
    ...
    this._dndController?.clearGlobalDragState();
    this.restoreListViewState(i);
  }
  _bindCellListener(e) {
    const t = new Ut();
    t.add(e.onDidChangeLayout(i => {
      if (i.totalHeight || i.outerWidth) {
        this.layoutNotebookCell(e, e.layoutInfo.totalHeight, i.context);
      }
    }));
    if (e.cellKind === zd.Code) {
      t.add(e.onDidRemoveOutputs(i => {
        i.forEach(r => this.removeInset(r));
      }));
    }
    t.add(e.onDidChangeState(i => {
      if (i.inputCollapsedChanged && e.isInputCollapsed && e.cellKind === zd.Markup) {
        this.hideMarkupPreviews([e]);
      }
      if (i.outputCollapsedChanged && e.isOutputCollapsed && e.cellKind === zd.Code) {
        e.outputsViewModels.forEach(r => this.hideInset(r));
      }
      if (i.focusModeChanged) {
        this._validateCellFocusMode(e);
      }
    }));
    t.add(e.onCellDecorationsChanged(i => {
      i.added.forEach(r => {
        if (r.className) {
          this.deltaCellContainerClassNames(e.id, [r.className], [], e.cellKind);
        }
        if (r.outputClassName) {
          this.deltaCellContainerClassNames(e.id, [r.outputClassName], [], e.cellKind);
        }
      });
      i.removed.forEach(r => {
        if (r.className) {
  ...
  _validateCellFocusMode(e) {
    if (e.focusMode === Tk.Editor) {
      if (this._lastCellWithEditorFocus && this._lastCellWithEditorFocus !== e) {
        this._lastCellWithEditorFocus.focusMode = Tk.Container;
      }
      this._lastCellWithEditorFocus = e;
    }
  }
  ...
  async _warmupViewportMarkdownCells(e, t) {
    if (t && t.cellTotalHeights) {
      const i = t.cellTotalHeights;
      const r = t.scrollPosition?.top ?? 0;
      const s = r + Math.max(this._dimension?.height ?? 0, 1080);
      let o = 0;
      const a = [];
      ...
      const i = e.viewCells.filter(a => a.cellKind === zd.Markup).slice(0, 5).map(a => this.createMarkupCellInitialization(a, -10000));
      await this._webview.initializeMarkup(i);
      let r = 0;
      const s = [];
      const o = Math.max(this._dimension?.height ?? 0, 1080);
      for (const a of e.viewCells) {
        if (a.cellKind === zd.Markup) {
          s.push({
            id: a.id,
            top: r
          });
        }
        r += a.getHeight(this.getLayoutInfo().fontInfo.lineHeight);
        if (r > o) {
          break;
        }
      }
  ...
  restoreListViewState(e) {
    ...
    if (e?.scrollPosition !== undefined) {
      this._list.scrollTop = e.scrollPosition.top;
      this._list.scrollLeft = e.scrollPosition.left;
    } else {
  ...
  _restoreSelectedKernel(e) {
    if (e?.selectedKernelId && this.textModel) {
      const t = this.notebookKernelService.getMatchingKernel(this.textModel);
      const i = t.all.find(r => r.id === e.selectedKernelId);
      if (i && !t.selected) {
        this.notebookKernelService.selectKernelForNotebook(i, this.textModel);
      }
    }
  }
  getEditorViewState() {
    ...
    if (this._list) {
      ...
      const i = {};
      for (let r = 0; r < this.viewModel.length; r++) {
        const s = this.viewModel.cellAt(r);
        i[r] = s.layoutInfo.totalHeight;
      }
      e.cellTotalHeights = i;
      if (this.viewModel) {
        const r = this.viewModel.getFocus();
        const s = this.viewModel.cellAt(r.start);
        if (s) {
          const o = this._list.domElementOfElement(s);
          const a = s.getEditState() === aw.Editing && !!o && !!o.ownerDocument.activeElement && !!o.contains(o.ownerDocument.activeElement);
          e.editorFocused = a;
          e.focus = r.start;
        }
      }
    ...
    const t = {};
    for (const [i, r] of this._contributions) {
      if (typeof r.saveViewState == "function") {
        t[i] = r.saveViewState();
      }
    }
    e.contributionsState = t;
    if (this.textModel?.uri.scheme === _n.untitled) {
      e.selectedKernelId = this.activeKernel?.id;
    }
    return e;
  }
  ...
  layout(e, t, i) {
    ...
    if (r) {
      r.then(() => this.layoutNotebook(e, t, i));
    } else {
      this.layoutNotebook(e, t, i);
    }
  }
  layoutNotebook(e, t, i) {
    if (t) {
      this.updateShadowElement(t, e, i);
    }
    ...
    const s = r;
    if (this._list.getRenderHeight() < s) {
      this._list.updateOptions({
        paddingBottom: this._allowScrollBeyondLastLine() ? Math.max(0, s - 50) : 0,
        paddingTop: 0
      });
      this._list.layout(s, e.width);
    } else {
      this._list.layout(s, e.width);
      this._list.updateOptions({
        paddingBottom: this._allowScrollBeyondLastLine() ? Math.max(0, s - 50) : 0,
    ...
    this.layoutContainerOverShadowElement(e, i);
    if (this._webviewTransparentCover) {
      this._webviewTransparentCover.style.height = `${e.height}px`;
      this._webviewTransparentCover.style.width = `${e.width}px`;
    }
  ...
  layoutContainerOverShadowElement(e, t) {
    if (e && t) {
      this._overlayContainer.style.top = `${t.top}px`;
      this._overlayContainer.style.left = `${t.left}px`;
      this._overlayContainer.style.width = `${e.width}px`;
      this._overlayContainer.style.height = `${e.height}px`;
      return;
    }
    ...
    this._overlayContainer.style.top = `${this._shadowElementViewInfo.top - (i?.top || 0)}px`;
    this._overlayContainer.style.left = `${this._shadowElementViewInfo.left - (i?.left || 0)}px`;
    this._overlayContainer.style.width = `${e ? e.width : this._shadowElementViewInfo.width}px`;
    this._overlayContainer.style.height = `${e ? e.height : this._shadowElementViewInfo.height}px`;
  }
  focus() {
    this._isVisible = true;
    this._editorFocus.set(true);
    if (this._webviewFocused) {
      ...
      if (this.viewModel) {
        const e = this.viewModel.getFocus();
        const t = this.viewModel.cellAt(e.start);
        if (!this.hasEditorFocus()) {
          this.focusContainer();
          this.updateEditorFocus();
        }
        if (t && t.focusMode === Tk.Editor) {
          t.updateEditState(aw.Editing, "editorWidget.focus");
          t.focusMode = Tk.Editor;
          this.focusEditor(t);
          return;
        }
      }
  ...
  focusEditor(e) {
    for (const [t, i] of this._renderedEditors.entries()) {
      if (t === e) {
        i.focus();
        return;
      }
  ...
  focusContainer(e = false) {
    if (this._webviewFocused) {
      this._webview?.focusWebview();
    } else {
      this._list.focusContainer(e);
    }
  }
  ...
  clearActiveCellWidgets() {
    this._renderedEditors.forEach((e, t) => {
      if (this.getActiveCell() === t && e) {
        aR.get(e)?.cancelSuggestWidget();
        mme.get(e)?.clearWidgets();
        ZH.get(e)?.clearWidgets();
      }
    });
    this._renderedEditors.forEach((e, t) => {
      if (iP.get(e)?.model.get()?.inlineEditState.get()) {
        e.render(true);
      }
    });
  }
  ...
  updateEditorFocus() {
    ...
    this._editorFocus.set(e);
    this.viewModel?.setEditorFocus(e);
  }
  updateCellFocusMode() {
    const e = this.getActiveCell();
    if (e?.focusMode === Tk.Output && !this._webviewFocused) {
  ...
  hasOutputTextSelection() {
    ...
    if (t.startContainer === t.endContainer && t.endOffset - t.startOffset === 0) {
      return false;
    }
    let i = t.commonAncestorContainer;
    if (!this._body.contains(i)) {
      return false;
    }
    while (i && i !== this._body) {
      if (i.classList && i.classList.contains("output")) {
        return true;
      }
      i = i.parentNode;
    }
  ...
  getViewIndexByModelIndex(e) {
    ...
    if (t) {
      return this._listViewInfoAccessor.getViewIndex(t);
    } else {
      return -1;
    }
  }
  getViewHeight(e) {
    if (this._listViewInfoAccessor) {
      return this._listViewInfoAccessor.getViewHeight(e);
    } else {
      return -1;
    }
  }
  ...
  setHiddenAreas(e) {
    return this._list.setHiddenAreas(e, true);
  }
  ...
  deltaCellContainerClassNames(e, t, i, r) {
    if (r === zd.Markup) {
      this._webview?.deltaMarkupPreviewClassNames(e, t, i);
    } else {
      this._webview?.deltaCellOutputContainerClassNames(e, t, i);
    }
  }
  ...
  async _loadKernelPreloads() {
    ...
    this._webview?.updateKernelPreloads(e);
  }
  get activeKernel() {
  ...
  async cancelNotebookCells(e) {
    if (!!this.viewModel && !!this.hasModel()) {
      e ||= this.viewModel.viewCells;
      return this.notebookExecutionService.cancelNotebookCellHandles(this.textModel, Array.from(e).map(t => t.handle));
    }
  }
  async executeNotebookCells(e) {
    ...
    e ||= this.viewModel.viewCells;
    return this.notebookExecutionService.executeNotebookCells(this.textModel, Array.from(e).map(t => t.model), this.scopedContextKeyService);
  }
  async layoutNotebookCell(e, t, i) {
    this._debug("layout cell", e.handle, t);
    if (this._list.getViewIndex(e) === undefined) {
      return;
    }
    if (this._pendingLayouts?.has(e)) {
      this._pendingLayouts?.get(e).dispose();
    }
    const s = new wy();
    const o = () => {
      if (this._isDisposed || !this.viewModel?.hasCell(e) || this._list.getViewIndex(e) === undefined || this._list.elementHeight(e) === t) {
        return;
      }
      const a = this._pendingLayouts?.get(e);
      this._pendingLayouts?.delete(e);
      if (!this.hasEditorFocus()) {
        const l = this.viewModel?.getCellIndex(e);
        const u = this.visibleRanges;
        if (l !== undefined && u && u.length && u[0].start === l && this._list.scrollTop > this.getAbsoluteTopOfElement(e)) {
          return this._list.updateElementHeight2(e, t, Math.min(l + 1, this.getLength() - 1));
        }
      }
      this._list.updateElementHeight2(e, t);
      s.complete(undefined);
      if (a) {
        a.dispose();
        this._layoutDisposables.delete(a);
      }
    };
    if (this._list.inRenderingTransaction) {
      const a = r_(As(this.getDomNode()), o);
      const l = $i(() => {
        a.dispose();
        s.complete(undefined);
      });
      this._pendingLayouts?.set(e, l);
      this._layoutDisposables.add(l);
    } else {
      o();
    }
    return s.p;
  ...
  _toggleNotebookCellSelection(e, t) {
    const i = this._list.getSelectedElements();
    const r = i.includes(e);
    const s = t ? i[i.length - 1] ?? e : e;
    const o = this._list.getViewIndex(e);
    const a = this._list.getViewIndex(s);
    const l = this.getCellsInViewRange(o, a);
    if (r) {
      this._list.selectElements(i.filter(u => !l.includes(u)));
    } else {
      this.focusElement(e);
      this._list.selectElements([...i.filter(u => !l.includes(u)), ...l]);
    }
  }
  getCellsInViewRange(e, t) {
    const i = [];
    for (let r = 0; r < this._list.length; ++r) {
      const s = this._list.element(r);
      if (s && (r >= e && r <= t || r >= t && r <= e)) {
        i.push(s);
      }
    }
    return i;
  ...
  async focusNotebookCell(e, t, i) {
    if (!this._isDisposed) {
      e.focusedOutputId = undefined;
      if (t === "editor") {
        e.isInputCollapsed = false;
        this.focusElement(e);
        this._list.focusView();
        e.updateEditState(aw.Editing, "focusNotebookCell");
        e.focusMode = Tk.Editor;
        if (!i?.skipReveal) {
          if (typeof i?.focusEditorLine == "number") {
            this._cursorNavMode.set(true);
            await this.revealLineInViewAsync(e, i.focusEditorLine);
            const r = this._renderedEditors.get(e);
            const s = i.focusEditorLine;
            r?.setSelection({
              startLineNumber: s,
              startColumn: 1,
              endLineNumber: s,
        ...
        const r = e.outputsViewModels.find(o => o.model.alternativeOutputId)?.model.alternativeOutputId;
        const s = i?.outputId ?? r ?? e.id;
        this._webview.focusOutput(s, i?.altOutputId, i?.outputWebviewFocused || this._webviewFocused);
        e.updateEditState(aw.Preview, "focusNotebookCell");
        e.focusMode = Tk.Output;
        e.focusedOutputId = i?.outputId;
        this._outputFocus.set(true);
        if (!i?.skipReveal) {
          this.revealInCenterIfOutsideViewport(e);
        }
      } else {
        const r = this._list.domElementOfElement(e);
        if (r && r.ownerDocument.activeElement && r.contains(r.ownerDocument.activeElement)) {
        ...
        if (!i?.skipReveal) {
          if (typeof i?.focusEditorLine == "number") {
            this._cursorNavMode.set(true);
            await this.revealInView(e);
          } else if (i?.revealBehavior === zbn.firstLine) {
            await this.revealFirstLineIfOutsideViewport(e);
          } else if (i?.revealBehavior === zbn.fullCell) {
            await this.revealInView(e);
          } else {
            await this.revealInCenterIfOutsideViewport(e);
          }
        }
        this._list.focusView();
        this.updateEditorFocus();
  ...
  async focusNextNotebookCell(e, t) {
    ...
    if (r) {
      await this.focusNotebookCell(r, t);
    }
  }
  async _warmupCell(e) {
    if (e.isOutputCollapsed) {
      return;
    }
    const t = e.outputsViewModels;
    for (const i of t.slice(0, iTa)) {
      const [r, s] = i.resolveMimeTypes(this.textModel, undefined);
      if (!r.find(d => d.isTrusted) || r.length === 0) {
        continue;
      }
      const o = r[s];
      if (!o) {
      ...
      const u = this._webview?.insetMapping.get(l.source);
      if (!u || !u.initialized) {
        const d = new Promise(m => {
          this._register(In.any(this.onDidRenderOutput, this.onDidRemoveOutput)(p => {
            if (p.model === l.source.model) {
              m();
            }
          }));
        });
        this.createOutput(e, l, 0, false);
        await d;
      } else {
        this.createOutput(e, l, 0, false);
      }
  ...
  async _warmupAll(e) {
    ...
    const t = this.viewModel.viewCells;
    const i = [];
    for (let r = 0; r < t.length; r++) {
      if (t[r].cellKind === zd.Markup && !this._webview.markupPreviewMapping.has(t[r].id)) {
        i.push(this.createMarkupPreview(t[r]));
      }
    }
    if (e && this._list) {
      for (let r = 0; r < this._list.length; r++) {
        const s = this._list.element(r);
        if (s?.cellKind === zd.Code) {
          i.push(this._warmupCell(s));
        }
      }
    }
  ...
  async _warmupSelection(e, t) {
    ...
    const i = this.viewModel.viewCells;
    const r = [];
    for (const s of t) {
      for (let o = s.start; o < s.end; o++) {
        if (i[o].cellKind === zd.Markup && !this._webview.markupPreviewMapping.has(i[o].id)) {
          r.push(this.createMarkupPreview(i[o]));
        }
      }
    }
    if (e && this._list) {
  ...
  async find(e, t, i, r = false, s = false, o) {
    if (!this._notebookViewModel) {
      return [];
    }
    o ||= this.getId();
    const a = this._notebookViewModel.find(e, t).filter(d => d.length > 0);
    if (!t.includeMarkupPreview && !t.includeOutput || t.findScope?.findScopeType === Wne.Text) {
      this._webview?.findStop(o);
      return a;
    }
    const l = {};
    a.forEach(d => {
    ...
    if (this._webview) {
      const d = Date.now();
      if (t.findScope && t.findScope.findScopeType === Wne.Cells && t.findScope.selectedCellRanges) {
        await this._warmupSelection(!!t.includeOutput, t.findScope.selectedCellRanges);
      } else {
        await this._warmupAll(!!t.includeOutput);
      }
      const m = Date.now();
      this.logService.debug("Find", `Warmup time: ${m - d}ms`);
      if (i.isCancellationRequested) {
        return [];
      }
      let p = [];
      if (t.findScope && t.findScope.findScopeType === Wne.Cells && t.findScope.selectedCellRanges) {
        p = Qne(t.findScope.selectedCellRanges).map(A => this._notebookViewModel?.viewCells[A].id ?? "");
      }
      const g = await this._webview.find(e, {
        caseSensitive: t.caseSensitive,
        wholeWord: t.wholeWord,
        includeMarkup: !!t.includeMarkupPreview,
        includeOutput: !!t.includeOutput,
        shouldGetSearchPreviewInfo: s,
        ownerID: o,
        findIds: p
      });
      if (i.isCancellationRequested) {
      ...
      g.forEach(f => {
        ...
        if (f.type === "preview") {
          if (A.getEditState() === aw.Preview && !t.includeMarkupPreview || A.getEditState() === aw.Editing && t.includeMarkupInput) {
            return;
          }
        } else if (!t.includeOutput) {
          return;
        }
        const w = l[f.cellId];
        if (w) {
          w.webviewMatches.push(f);
        } else {
          l[f.cellId] = new Aki(this._notebookViewModel.viewCells.find(C => C.id === f.cellId), this._notebookViewModel.viewCells.findIndex(C => C.id === f.cellId), [], [f]);
        }
      });
    }
    const u = [];
    this._notebookViewModel.viewCells.forEach((d, m) => {
      if (l[d.id]) {
        u.push(new Aki(d, m, l[d.id].contentMatches, l[d.id].webviewMatches));
      }
    });
    return u;
  }
  async findHighlightCurrent(e, t) {
    if (this._webview) {
      return this._webview?.findHighlightCurrent(e, t ?? this.getId());
    } else {
      return 0;
    }
  }
  async findUnHighlightCurrent(e, t) {
    if (this._webview) {
      return this._webview?.findUnHighlightCurrent(e, t ?? this.getId());
    }
  }
  findStop(e) {
  ...
  async createMarkupPreview(e) {
    if (!this._webview || (this._webview.isResolved() || (await this._resolveWebview()), !this._webview || !this._list.webviewElement) || !this.viewModel || !this._list.viewModel || this.viewModel.getCellIndex(e) === -1 || this.cellIsHidden(e)) {
      return;
    }
    const t = parseInt(this._list.webviewElement.domNode.style.top, 10);
    const i = t ? 0 - t : 0;
    const r = this._list.getCellViewScrollTop(e);
  ...
  async unhideMarkupPreviews(e) {
    if (this._webview) {
      ...
      await this._webview?.unhideMarkupPreviews(e.map(t => t.id));
    }
  }
  async hideMarkupPreviews(e) {
    if (!!this._webview && !!e.length) {
      if (!this._webview.isResolved()) {
        await this._resolveWebview();
      }
      await this._webview?.hideMarkupPreviews(e.map(t => t.id));
    }
  }
  async deleteMarkupPreviews(e) {
    if (this._webview) {
      ...
      await this._webview?.deleteMarkupPreviews(e.map(t => t.id));
    }
  }
  ...
  async createOutput(e, t, i, r) {
    this._insetModifyQueueByOutputId.queue(t.source.model.outputId, async () => {
      if (this._isDisposed || !this._webview || (this._webview.isResolved() || (await this._resolveWebview()), !this._webview) || !this._list.webviewElement) {
        return;
      }
      if (t.type === 1) {
        this.notebookRendererMessaging.prepare(t.renderer.id);
      }
      const s = parseInt(this._list.webviewElement.domNode.style.top, 10);
      const o = s ? 0 - s : 0;
      const a = this._list.getCellViewScrollTop(e) + o;
      const l = this._webview.insetMapping.get(t.source);
      if (!l || !l.renderer && t.type === 1) {
        if (r) {
          this._webview.requestCreateOutputWhenWebviewIdle({
            cellId: e.id,
            cellHandle: e.handle,
            cellUri: e.uri,
            executionId: e.internalMetadata.executionId
          }, t, a, i);
        } else {
      ...
      } else if (l.renderer && t.type === 1 && l.renderer.id !== t.renderer.id) {
        this._webview.removeInsets([t.source]);
        this._webview.createOutput({
          cellId: e.id,
          cellHandle: e.handle,
          cellUri: e.uri
        }, t, a, i);
      } else if (l.versionId !== t.source.model.versionId) {
        ...
        const u = e.outputsViewModels.indexOf(t.source);
        const d = e.getOutputOffset(u);
        this._webview.updateScrollTops([{
          cell: e,
          output: t.source,
          cellTop: a,
          outputOffset: d,
          forceDisplay: !e.isOutputCollapsed
        }], []);
      }
    });
  }
  async updateOutput(e, t, i) {
    this._insetModifyQueueByOutputId.queue(t.source.model.outputId, async () => {
      if (this._isDisposed || !this._webview || e.isOutputCollapsed || (this._webview.isResolved() || (await this._resolveWebview()), !this._webview || !this._list.webviewElement)) {
        return;
      }
      if (!this._webview.insetMapping.has(t.source)) {
        return this.createOutput(e, t, i, false);
      }
      if (t.type === 1) {
        this.notebookRendererMessaging.prepare(t.renderer.id);
      }
      const r = parseInt(this._list.webviewElement.domNode.style.top, 10);
      const s = r ? 0 - r : 0;
      const o = this._list.getCellViewScrollTop(e) + s;
      this._webview.updateOutput({
        cellId: e.id,
        cellHandle: e.handle,
        cellUri: e.uri
  ...
  removeInset(e) {
    this._insetModifyQueueByOutputId.queue(e.model.outputId, async () => {
      if (!this._isDisposed && !!this._webview) {
        if (this._webview?.isResolved()) {
          this._webview.removeInsets([e]);
        }
        this._onDidRemoveOutput.fire(e);
      }
    });
  }
  hideInset(e) {
  ...
  postMessage(e) {
    if (this._webview?.isResolved()) {
      this._webview.postKernelMessage(e);
    }
  }
  addClassName(e) {
  ...
  _updateScrollHeight() {
    if (this._isDisposed || !this._webview?.isResolved() || !this._list.webviewElement) {
      return;
    }
    ...
    this._webview?.insetMapping.forEach((a, l) => {
      ...
      if (!u || !(u instanceof jJ) || (this.viewModel?.viewCells.find(f => f.handle === a.cellInfo.cellHandle), this._list.getViewIndex(u) === undefined)) {
        return;
      }
      if (u.outputsViewModels.indexOf(l) < 0) {
        s.push(l);
      }
      const m = this._list.getCellViewScrollTop(u);
      const p = u.outputsViewModels.indexOf(l);
      const g = u.getOutputOffset(p);
      r.push({
        cell: u,
        output: l,
        cellTop: m + i,
    ...
    if (o.length || r.length) {
      this._debug("_list.onDidChangeContentHeight/markdown", o);
      this._webview?.updateScrollTops(r, o);
    }
  }
  _updateOutputHeight(e, t, i, r, s) {
    ...
    if (o && o instanceof jJ) {
      ...
      if (a > -1) {
        this._debug("update cell output", o.handle, i);
        o.updateOutputHeight(a, i, s);
        this.layoutNotebookCell(o, o.layoutInfo.totalHeight);
        if (r) {
          this._onDidRenderOutput.fire(t);
        }
      } else {
  ...
  _updateMarkupCellHeight(e, t, i) {
    ...
    if (r && r instanceof GV) {
      ...
      this._debug("updateMarkdownCellHeight", r.handle, t + s, i);
      r.renderedMarkdownHeight = t;
    }
  }
  _setMarkupCellEditState(e, t) {
    ...
    if (i instanceof GV) {
      this.revealInView(i);
      i.updateEditState(t, "setMarkdownCellEditState");
    }
  }
  ...
  _didDropMarkupCell(e, t) {
    ...
    if (i instanceof GV) {
      const r = this._list.webviewElement ? -parseInt(this._list.webviewElement.domNode.style.top, 10) : 0;
      t.dragOffsetY -= r;
      this._dndController?.explicitDrop(i, t);
    }
  }
  _didEndDragMarkupCell(e) {
    const t = this._getCellById(e);
    if (t instanceof GV) {
      this._dndController?.endExplicitDrag(t);
    }
  }
  _didResizeOutput(e) {
    const t = this._getCellById(e);
    if (t) {
      this._onDidResizeOutputEmitter.fire(t);
    }
  }
  _updatePerformanceMetadata(e, t, i, r) {
    ...
    const s = this._getCellById(e);
    const o = s ? this.getCellIndex(s) : undefined;
    if (s?.internalMetadata.executionId === t && o !== undefined) {
  ...
  dispose() {
    ...
    this._webview?.dispose();
    this._webview = null;
    this._layoutDisposables.forEach(e => e.dispose());
    this.notebookEditorService.removeNotebookEditor(this);
    Bo(this._contributions.values());
    ...
    this._baseCellEditorOptions.forEach(e => e.dispose());
...
Nbn = __decorate([__param(2, ln), __param(3, da), __param(4, eIa), __param(5, pO), __param(6, NM), __param(7, JA), __param(8, Fn), __param(9, wi), __param(10, vS), __param(11, kc), __param(12, ea), __param(13, wki), __param(14, p2), __param(15, awe)], Nbn);
Rie(TQ.Base, 5, "notebook-progress-bar");
Rie(TQ.Base, 10, "notebook-list-insertion-indicator");
Rie(TQ.Base, 20, "notebook-cell-editor-outline");
Rie(TQ.Base, 25, "notebook-scrollbar");
Rie(TQ.Base, 26, "notebook-cell-status");
Rie(TQ.Base, 26, "notebook-folding-indicator");
Rie(TQ.Base, 27, "notebook-output");
Rie(TQ.Base, 28, "notebook-cell-bottom-toolbar-container");
Rie(TQ.Base, 29, "notebook-run-button-container");
Rie(TQ.Base, 29, "notebook-input-collapse-condicon");
Rie(TQ.Base, 30, "notebook-cell-output-toolbar");
Rie(TQ.Sash, 1, "notebook-cell-expand-part-button");
Rie(TQ.Sash, 2, "notebook-cell-toolbar");
Rie(TQ.Sash, 3, "notebook-cell-toolbar-dropdown-active");
uIa = Rn("notebook.cellBorderColor", {
  dark: rl(pW, 1),
  light: rl(pW, 1),
  hcDark: BV,
  hcLight: BV
}, _(9378, null));
zdy = Rn("notebook.focusedEditorBorder", nN, _(9379, null));
u6f = Rn("notebookStatusSuccessIcon.foreground", p_i, _(9380, null));
d6f = Rn("notebookEditorOverviewRuler.runningCellForeground", p_i, _(9381, null));
h6f = Rn("notebookStatusErrorIcon.foreground", P4n, _(9382, null));
Vdy = Rn("notebookStatusRunningIcon.foreground", ym, _(9383, null));
Kdy = Rn("notebook.outputContainerBorderColor", null, _(9384, null));
Ydy = Rn("notebook.outputContainerBackgroundColor", null, _(9385, null));
Zdy = Rn("notebook.cellToolbarSeparator", {
  dark: Xr.fromHex("#808080").transparent(0.35),
  light: Xr.fromHex("#808080").transparent(0.35),
  hcDark: Du,
  hcLight: Du
}, _(9386, null));
T_u = Rn("notebook.focusedCellBackground", null, _(9387, null));
Xdy = Rn("notebook.selectedCellBackground", {
  dark: pW,
  light: pW,
  hcDark: null,
  hcLight: null
}, _(9388, null));
ehy = Rn("notebook.cellHoverBackground", {
  dark: rl(T_u, 0.5),
  light: rl(T_u, 0.7),
  hcDark: null,
  hcLight: null
}, _(9389, null));
thy = Rn("notebook.selectedCellBorder", {
  dark: uIa,
  light: uIa,
  hcDark: Du,
  hcLight: Du
}, _(9390, null));
nhy = Rn("notebook.inactiveSelectedCellBorder", {
  dark: null,
  light: null,
  hcDark: nN,
  hcLight: nN
}, _(9391, null));
ihy = Rn("notebook.focusedCellBorder", nN, _(9392, null));
rhy = Rn("notebook.inactiveFocusedCellBorder", uIa, _(9393, null));
shy = Rn("notebook.cellStatusBarItemHoverBackground", {
  light: new Xr(new Sa(0, 0, 0, 0.08)),
  dark: new Xr(new Sa(255, 255, 255, 0.15)),
  hcDark: new Xr(new Sa(255, 255, 255, 0.15)),
  hcLight: new Xr(new Sa(0, 0, 0, 0.08))
}, _(9394, null));
ohy = Rn("notebook.cellInsertionIndicator", nN, _(9395, null));
ahy = Rn("notebookScrollbarSlider.background", aft, _(9396, null));
chy = Rn("notebookScrollbarSlider.hoverBackground", oVe, _(9397, null));
lhy = Rn("notebookScrollbarSlider.activeBackground", aVe, _(9398, null));
uhy = Rn("notebook.symbolHighlightBackground", {
  dark: Xr.fromHex("#ffffff0b"),
  light: Xr.fromHex("#fdff0033"),
  hcDark: null,
  hcLight: null
}, _(9399, null));
dhy = Rn("notebook.cellEditorBackground", {
  light: mO,
  dark: mO,
  hcDark: null,
  hcLight: null
}, _(9400, null));
A7e = Rn("notebook.editorBackground", {
  light: pgn,
  dark: pgn,
  hcDark: null,
  hcLight: null
