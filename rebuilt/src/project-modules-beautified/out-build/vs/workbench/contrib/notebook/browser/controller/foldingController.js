...
m2e = class extends at {
    ...
    this._register(this._notebookEditor.onDidChangeModel(() => {
      ...
      if (this._notebookEditor.hasModel()) {
        this._localStore.add(this._notebookEditor.onDidChangeCellState(e => {
          if (e.source.editStateChanged && e.cell.cellKind === zd.Markup) {
            this._foldingModel?.recompute();
          }
        }));
        this._foldingModel = new Tfg();
  ...
  restoreViewState(n) {
    this._foldingModel?.applyMemento(n || []);
    this._updateEditorFoldingRanges();
  }
  setFoldingStateDown(n, e, t) {
    const i = e === 2;
    const r = this._foldingModel.getRegionAtLine(n + 1);
    const s = [];
    if (r && (r.isCollapsed !== i && s.push(r), t > 1)) {
      const o = this._foldingModel.getRegionsInside(r, (a, l) => a.isCollapsed !== i && l < t);
      s.push(...o);
    }
    s.forEach(o => this._foldingModel.setCollapsed(o.regionIndex, e === 2));
    this._updateEditorFoldingRanges();
  }
  setFoldingStateUp(n, e, t) {
    ...
    this._foldingModel.getAllRegionsAtLine(n + 1, (r, s) => r.isCollapsed !== (e === 2) && s <= t).forEach(r => this._foldingModel.setCollapsed(r.regionIndex, e === 2));
    this._updateEditorFoldingRanges();
  }
  _updateEditorFoldingRanges() {
    if (!this._foldingModel || !this._notebookEditor.hasModel()) {
      return;
    }
  ...
  onMouseUp(n) {
    if (!n.event.target || !this._notebookEditor.hasModel()) {
      return;
    }
    const e = this._notebookEditor.getViewModel();
    const t = n.event.target;
    if (t.classList.contains("codicon-notebook-collapsed") || t.classList.contains("codicon-notebook-expanded")) {
      if (!t.parentElement.classList.contains("notebook-folding-indicator")) {
        return;
      }
      const r = n.target;
      const s = e.getCellIndex(r);
      const o = e.getFoldingState(s);
      if (o === 0) {
        return;
      }
      this.setFoldingStateUp(s, o === 2 ? 1 : 2, 1);
      this._notebookEditor.focusElement(r);
    }
  }
  recompute() {
    this._foldingModel?.recompute();
...
Dt(class extends rn {
  constructor() {
    super({
      id: "notebook.fold",
      title: dt(9190, "Fold Cell"),
      category: o7,
      keybinding: {
        when: Ee.and(dv, Ee.not(lD)),
        primary: 3164,
        mac: {
          primary: 2652,
          secondary: [15]
  ...
  async run(n, e) {
    ...
    const r = e && e.levels || 1;
    const s = e && e.direction === "up" ? "up" : "down";
    let o;
    if (e) {
      o = e.index;
    } else {
    ...
    if (o !== undefined) {
      if ((o < 0 || o >= i.getLength() ? undefined : i.cellAt(o))?.cellKind === zd.Code && s === "down") {
        return;
      }
      if (s === "up") {
        a.setFoldingStateUp(o, 2, r);
      } else {
        a.setFoldingStateDown(o, 2, r);
      }
...
Dt(class extends rn {
  ...
  async run(n, e) {
    ...
    if (o !== undefined) {
      if (s === "up") {
        a.setFoldingStateUp(o, 1, r);
      } else {
        a.setFoldingStateDown(o, 1, r);
      }
    }
  }
});
