...
vhy();
x9e();
S6f = class extends at {
  constructor(n, e) {
    super();
    e.body.classList.remove("left", "right", "full");
    const t = n.hiddenCells.length === 1 ? _(9246, null, n.hiddenCells.length) : _(9247, null, n.hiddenCells.length);
    e.placeholder.innerText = t;
    this._register(ei(e.placeholder, "dblclick", i => {
      if (i.button === 0) {
        i.preventDefault();
        n.showHiddenCells();
      }
    }));
    this._register(e.marginOverlay.onAction(() => n.showHiddenCells()));
    e.marginOverlay.show();
  }
};
f2e = class extends at {
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g, f) {
    super();
    this.cell = e;
    this.propertyHeaderContainer = t;
    this.notebookEditor = i;
    this.accessor = r;
    ...
    this.menuService = u;
    this.contextKeyService = d;
    this.themeService = m;
    this.telemetryService = p;
    this.accessibilityService = g;
    this.actionBadgeService = f;
  }
  buildHeader() {
    this._foldingIndicator = Rt(this.propertyHeaderContainer, Ct(".property-folding-indicator"));
    this._foldingIndicator.classList.add(this.accessor.prefix);
    const e = Rt(this.propertyHeaderContainer, Ct("div.property-status"));
    this._statusSpan = Rt(e, Ct("span"));
    this._description = Rt(e, Ct("span.property-description"));
    ...
    this._register(i);
    this._propertyChanged = Obn.bindTo(i);
    this._propertyExpanded = P_u.bindTo(i);
    this._menu = this._register(this.menuService.createMenu(this.accessor.menuId, i));
    this._register(this._menu.onDidChange(() => this.updateMenu()));
    this._register(this.notebookEditor.onMouseUp(r => {
      if (!r.event.target || r.target !== this.cell) {
        return;
      }
      ...
      if (s === this.propertyHeaderContainer || s === this._foldingIndicator || this._foldingIndicator.contains(s) || s === e || e.contains(s)) {
        ...
        this.accessor.updateFoldingState(o === kD.Expanded ? kD.Collapsed : kD.Expanded);
  ...
  _updateFoldingIcon() {
    if (this.accessor.getFoldingState() === kD.Collapsed) {
      um(this._foldingIndicator, tL(DWl));
      this._propertyExpanded?.set(false);
    } else {
      um(this._foldingIndicator, tL(BWl));
      this._propertyExpanded?.set(true);
    }
  }
};
f2e = __decorate([__param(4, kc), __param(5, mo), __param(6, fr), __param(7, ms), __param(8, xd), __param(9, wi), __param(10, bo), __param(11, ea), __param(12, Cf), __param(13, cve)], f2e);
gIa = class extends at {
  constructor(e, t, i, r, s, o, a, l, u) {
    ...
    this.textConfigurationService = l;
    this.configurationService = u;
    this._editor = i.sourceEditor;
    ...
    this._register(t.onDidLayoutChange(d => {
      this.layout(d);
      this.updateBorders();
    }));
    this.buildBody();
  ...
  buildBody() {
    ...
    if (this.viewModel instanceof M_u) {
      this._register(this.viewModel.modifiedMetadata.onDidChange(t => {
  ...
  updateSourceEditor() {
    ...
    const e = () => {
      ...
      const o = this.notebookEditor.getLayoutInfo().fontInfo.lineHeight || 17;
      const a = this.viewModel.layoutInfo.editorHeight !== 0 ? this.viewModel.layoutInfo.editorHeight : this.viewModel.computeInputEditorHeight(o);
      this._editorContainer.style.height = `${a}px`;
      this._editorContainer.style.display = "block";
      const l = this._editor.getContentHeight();
      if (l >= 0) {
        this.viewModel.editorHeight = l;
      }
      return a;
    };
    const t = () => {
      ...
      const a = this.viewModel.modifiedMetadata.textBuffer.getLineCount();
      const l = {
        padding: g2e(a)
      };
      const u = this._register(U_u(this.configurationService));
      if (u.options.enabled) {
        l.hideUnchangedRegions = u.options;
      }
      this._editor.updateOptions(l);
      this._register(u.onDidChangeEnablement(() => {
        l.hideUnchangedRegions = u.options;
        this._editor.updateOptions(l);
      }));
      this._editor.layout({
        width: this.notebookEditor.getLayoutInfo().width - hwe * 2,
        height: o
      });
      this._register(this._editor.onDidContentSizeChange(d => {
        if (this.viewModel.cellFoldingState === kD.Expanded && d.contentHeightChanged && this.viewModel.layoutInfo.editorHeight !== d.contentHeight) {
          this.viewModel.editorHeight = d.contentHeight;
        }
      }));
    ...
    this._cellHeader = this._register(this.instantiationService.createInstance(f2e, this.viewModel, this._cellHeaderContainer, this.notebookEditor, {
      updateInfoRendering: () => t(),
      checkIfModified: () => this.viewModel.originalMetadata.getHash() !== this.viewModel.modifiedMetadata.getHash() ? {
        reason: undefined
      } : false,
      getFoldingState: () => this.viewModel.cellFoldingState,
      updateFoldingState: o => this.viewModel.cellFoldingState = o,
      unChangedLabel: "Notebook Metadata",
      changedLabel: "Notebook Metadata changed",
      prefix: "metadata",
      menuId: st.NotebookDiffDocumentMetadata
    }));
    ...
    r.set(this.viewModel.originalMetadata.getHash() !== this.viewModel.modifiedMetadata.getHash());
  ...
  async _initializeSourceDiffEditor() {
    ...
    const i = this._register(this._editor.createViewModel({
      original: e.object.textEditorModel,
      modified: t.object.textEditorModel
    }));
    await i.waitForDiff();
    this._editor.setModel(i);
    const r = () => {
      this._editorViewStateChanged = true;
    };
    const s = l => {
      if (l.scrollTopChanged || l.scrollLeftChanged) {
        this._editorViewStateChanged = true;
      }
    };
    this.updateEditorOptionsForWhitespace();
    this._register(this._editor.getOriginalEditor().onDidChangeCursorSelection(r));
    this._register(this._editor.getOriginalEditor().onDidScrollChange(s));
    this._register(this._editor.getModifiedEditor().onDidChangeCursorSelection(r));
    this._register(this._editor.getModifiedEditor().onDidScrollChange(s));
    const o = this.viewModel.getSourceEditorViewState();
    if (o) {
      this._editor.restoreViewState(o);
    }
...
gIa = __decorate([__param(3, ln), __param(4, El), __param(5, xd), __param(6, wi), __param(7, uy), __param(8, Fn)], gIa);
$_u = class extends at {
  constructor(n, e, t, i, r, s, o, a, l, u, d, m, p, g, f) {
    ...
    this.modelService = o;
    this.textModelService = a;
    this.contextMenuService = l;
    this.keybindingService = u;
    this.notificationService = d;
    this.menuService = m;
    this.contextKeyService = p;
    this.configurationService = g;
    this.textConfigurationService = f;
    this._metadataLocalDisposable = this._register(new Ut());
    this._outputLocalDisposable = this._register(new Ut());
    this._ignoreMetadata = false;
    this._ignoreOutputs = false;
    this._isDisposed = false;
    this._metadataEditorDisposeStore = this._register(new Ut());
    this._outputEditorDisposeStore = this._register(new Ut());
    this._register(e.onDidLayoutChange(A => {
      this.layout(A);
    }));
    this._register(e.onDidLayoutChange(A => this.updateBorders()));
    this.init();
    this.buildBody();
    this._register(e.onDidStateChange(() => {
  ...
  buildBody() {
    ...
    this._diffEditorContainer = this.templateData.diffEditorContainer;
    n.classList.remove("left", "right", "full");
    switch (this.style) {
      case "left":
    ...
    this._register(this.configurationService.onDidChangeConfiguration(e => {
      ...
      if (e.affectsConfiguration("notebook.diff.ignoreMetadata")) {
        ...
        if (r !== undefined && this._ignoreMetadata !== r) {
          this._ignoreMetadata = r;
      ...
      if (e.affectsConfiguration("notebook.diff.ignoreOutputs")) {
        ...
        if (r !== undefined && this._ignoreOutputs !== (r || this.notebookEditor.textModel?.transientOptions.transientOutputs)) {
          this._ignoreOutputs = r || !!this.notebookEditor.textModel?.transientOptions.transientOutputs;
  ...
  updateMetadataRendering() {
    if (this.cell.metadataFoldingState === kD.Expanded) {
      this._metadataInfoContainer.style.display = "block";
      if (!this._metadataEditorContainer || !this._metadataEditor) {
        this._metadataEditorContainer = Rt(this._metadataInfoContainer, Ct(".metadata-editor-container"));
  ...
  updateOutputRendering(n) {
    if (this.cell.outputFoldingState === kD.Expanded) {
  ...
  _buildOutputRawContainer() {
    if (!this._outputEditorContainer) {
      this._outputEditorContainer = Rt(this._outputInfoContainer, Ct(".output-editor-container"));
  ...
  async _buildMetadataEditor() {
    ...
    if (this.cell instanceof mwe) {
      ...
      this._metadataEditorDisposeStore.add(e);
      this._metadataEditorDisposeStore.add(t);
      const i = this._metadataEditor.createViewModel({
        original: e.object.textEditorModel,
        modified: t.object.textEditorModel
      });
      this._metadataEditor.setModel(i);
      await i.waitForDiff();
      if (this._isDisposed) {
        return;
      }
      this.cell.metadataHeight = this._metadataEditor.getContentHeight();
      this._metadataEditorDisposeStore.add(this._metadataEditor.onDidContentSizeChange(s => {
        if (s.contentHeightChanged && this.cell.metadataFoldingState === kD.Expanded) {
          this.cell.metadataHeight = s.contentHeight;
        }
      }));
      let r = false;
      this._metadataEditorDisposeStore.add(t.object.textEditorModel.onDidChangeContent(() => {
        ...
        this._applySanitizedMetadataChanges(this.cell.modified.metadata, s);
      ...
      this._metadataEditorDisposeStore.add(this.cell.modified.textModel.onDidChangeMetadata(() => {
        ...
        const s = CEt(this.notebookEditor.textModel?.transientOptions.transientCellMetadata, this.cell.modified?.metadata || {}, this.cell.modified?.language, true);
      ...
      this._metadataEditorDisposeStore.add(this._metadataEditor);
      const n = this.languageService.createById("jsonc");
      const e = CEt(this.notebookEditor.textModel?.transientOptions.transientCellMetadata, this.cell.type === "insert" ? this.cell.modified.metadata || {} : this.cell.original.metadata || {}, undefined, true);
      const t = this.cell.type === "insert" ? this.cell.modified.uri : this.cell.original.uri;
      const i = this.cell.type === "insert" ? this.cell.modified.handle : this.cell.original.handle;
      const r = Dg.generateCellPropertyUri(t, i, _n.vscodeNotebookCellMetadata);
      const s = this.modelService.createModel(e, n, r, false);
      this._metadataEditor.setModel(s);
      this._metadataEditorDisposeStore.add(s);
      this.cell.metadataHeight = this._metadataEditor.getContentHeight();
      this._metadataEditorDisposeStore.add(this._metadataEditor.onDidContentSizeChange(o => {
        if (o.contentHeightChanged && this.cell.metadataFoldingState === kD.Expanded) {
          this.cell.metadataHeight = o.contentHeight;
        }
      }));
    }
  }
  _buildOutputEditor() {
    ...
    if ((this.cell.type === "modified" || this.cell.type === "unchanged") && !this.notebookEditor.textModel.transientOptions.transientOutputs) {
      const i = $bn(this.cell.original?.outputs || []);
      const r = $bn(this.cell.modified?.outputs || []);
      if (i !== r) {
        const s = this.languageService.createById("json");
        const o = this.modelService.createModel(i, s, undefined, true);
        const a = this.modelService.createModel(r, s, undefined, true);
        this._outputEditorDisposeStore.add(o);
        this._outputEditorDisposeStore.add(a);
        const l = this.notebookEditor.getLayoutInfo().fontInfo.lineHeight || 17;
        const u = Math.max(o.getLineCount(), a.getLineCount());
        this._outputEditor = this.instantiationService.createInstance(JB, this._outputEditorContainer, {
          ...
          automaticLayout: false,
          dimension: {
            height: Math.min(Rki, this.cell.layoutInfo.rawOutputHeight || l * u),
            width: this.cell.getComputedCellContainerWidth(this.notebookEditor.getLayoutInfo(), false, true)
          },
        ...
        this.cell.rawOutputHeight = this._outputEditor.getContentHeight();
        this._outputEditorDisposeStore.add(this._outputEditor.onDidContentSizeChange(d => {
          if (d.contentHeightChanged && this.cell.outputFoldingState === kD.Expanded) {
            this.cell.rawOutputHeight = d.contentHeight;
          }
        }));
    ...
    this._outputEditor = this.instantiationService.createInstance(WS, this._outputEditorContainer, {
      ...Tki,
      dimension: {
        width: Math.min(Rki, this.cell.getComputedCellContainerWidth(this.notebookEditor.getLayoutInfo(), false, this.cell.type === "unchanged" || this.cell.type === "modified") - 32),
        height: this.cell.layoutInfo.rawOutputHeight
      },
      overflowWidgetsDomNode: this.notebookEditor.getOverflowContainerDomNode()
    }, {});
    this._outputEditorDisposeStore.add(this._outputEditor);
    const n = this.languageService.createById("json");
    const e = $bn(this.notebookEditor.textModel.transientOptions.transientOutputs ? [] : this.cell.type === "insert" ? this.cell.modified?.outputs || [] : this.cell.original?.outputs || []);
    const t = this.modelService.createModel(e, n, undefined, true);
    this._outputEditorDisposeStore.add(t);
    this._outputEditor.setModel(t);
    this._outputEditor.restoreViewState(this.cell.getOutputEditorViewState());
    this.cell.rawOutputHeight = this._outputEditor.getContentHeight();
    this._outputEditorDisposeStore.add(this._outputEditor.onDidContentSizeChange(i => {
      if (i.contentHeightChanged && this.cell.outputFoldingState === kD.Expanded) {
        this.cell.rawOutputHeight = i.contentHeight;
      }
    }));
  }
  layoutNotebookCell() {
...
q_u = class extends $_u {
  constructor(n, e, t, i, r, s, o, a, l, u, d, m, p, g, f) {
    super(n, e, t, i, r, s, o, a, l, u, d, m, p, g, f);
    this.cell = e;
    this.templateData = t;
    this.updateBorders();
  }
  ...
  updateSourceEditor() {
    ...
    const n = () => {
      ...
      const e = this.notebookEditor.getLayoutInfo().fontInfo.lineHeight || 17;
      const t = this.cell.computeInputEditorHeight(e);
      this._editorContainer.style.height = `${t}px`;
      this._editorContainer.style.display = "block";
      if (this._editor) {
        const i = this._editor.getContentHeight();
        if (i >= 0) {
          this.cell.editorHeight = i;
        }
        return;
      }
      this._editor = this.templateData.sourceEditor;
      this._editor.layout({
        width: (this.notebookEditor.getLayoutInfo().width - hwe * 2) / 2 - 18,
        height: t
      });
      this._editor.updateOptions({
        readOnly: this.readonly
      });
      this.cell.editorHeight = t;
      this._register(this._editor.onDidContentSizeChange(i => {
        if (this.cell.cellFoldingState === kD.Expanded && i.contentHeightChanged && this.cell.layoutInfo.editorHeight !== i.contentHeight) {
          this.cell.editorHeight = i.contentHeight;
        }
      }));
    ...
    this._cellHeader = this._register(this.instantiationService.createInstance(f2e, this.cell, this._cellHeaderContainer, this.notebookEditor, {
      updateInfoRendering: () => n(),
      checkIfModified: () => ({
        reason: undefined
      }),
      getFoldingState: () => this.cell.cellFoldingState,
      updateFoldingState: e => this.cell.cellFoldingState = e,
      unChangedLabel: "Input",
      changedLabel: "Input",
      prefix: "input",
      menuId: st.NotebookDiffCellInputTitle
    }));
    this._cellHeader.buildHeader();
    n();
    this._initializeSourceDiffEditor(this.nestedCellViewModel);
  ...
  _buildMetadata() {
    ...
    this._metadataHeader = this.instantiationService.createInstance(f2e, this.cell, this._metadataHeaderContainer, this.notebookEditor, {
      updateInfoRendering: this.updateMetadataRendering.bind(this),
      checkIfModified: () => this.cell.checkMetadataIfModified(),
      getFoldingState: () => this.cell.metadataFoldingState,
      updateFoldingState: n => {
        this.cell.metadataFoldingState = n;
      },
      unChangedLabel: "Metadata",
      changedLabel: "Metadata changed",
      prefix: "metadata",
      menuId: st.NotebookDiffCellMetadataTitle
    });
  ...
  _buildOutput() {
    ...
    this._outputHeader = this.instantiationService.createInstance(f2e, this.cell, this._outputHeaderContainer, this.notebookEditor, {
      updateInfoRendering: this.updateOutputRendering.bind(this),
      checkIfModified: () => this.cell.checkIfOutputsModified(),
      getFoldingState: () => this.cell.outputFoldingState,
      updateFoldingState: n => {
      ...
      prefix: "output",
      menuId: st.NotebookDiffCellOutputsTitle
    });
...
fIa = class extends q_u {
  ...
  styleContainer(e) {
    e.classList.remove("inserted");
    e.classList.add("removed");
  }
  layout(e) {
    r_(As(this._diffEditorContainer), () => {
      if ((e.editorHeight || e.outerWidth) && this._editor) {
      ...
      if (e.outerWidth && this._editor) {
        this._editorContainer.style.height = `${this.cell.layoutInfo.editorHeight}px`;
        this._editor.layout();
      }
      if (e.metadataHeight || e.outerWidth) {
        this._metadataEditor?.layout({
          width: this.cell.getComputedCellContainerWidth(this.notebookEditor.getLayoutInfo(), false, false),
          height: this.cell.layoutInfo.metadataHeight
        });
      }
      if (e.outputTotalHeight || e.outerWidth) {
  ...
  _buildOutputRendererContainer() {
    if (!this._outputViewContainer) {
      this._outputViewContainer = Rt(this._outputInfoContainer, Ct(".output-view-container"));
      this._outputEmptyElement = Rt(this._outputViewContainer, Ct(".output-empty-view"));
      const e = Rt(this._outputEmptyElement, Ct("span"));
      e.innerText = "No outputs to render";
      ...
      this.cell.layoutChange();
      this._outputLeftView = this.instantiationService.createInstance(TEt, this.notebookEditor, this.notebookEditor.textModel, this.cell, this.cell.original, s1.Original, this._outputViewContainer);
      this._register(this._outputLeftView);
      this._outputLeftView.render();
      const t = this.notebookEditor.onDidDynamicOutputRendered(i => {
        if (i.cell.uri.toString() === this.cell.original.uri.toString()) {
...
fIa = __decorate([__param(3, Jl), __param(4, Il), __param(5, El), __param(6, ln), __param(7, kc), __param(8, mo), __param(9, ms), __param(10, xd), __param(11, wi), __param(12, Fn), __param(13, uy)], fIa);
bIa = class extends q_u {
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g, f) {
  ...
  _buildOutputRendererContainer() {
    if (!this._outputViewContainer) {
      ...
      this.cell.layoutChange();
      this._outputRightView = this.instantiationService.createInstance(TEt, this.notebookEditor, this.notebookEditor.textModel, this.cell, this.cell.modified, s1.Modified, this._outputViewContainer);
      this._register(this._outputRightView);
      this._outputRightView.render();
      const e = this.notebookEditor.onDidDynamicOutputRendered(t => {
        if (t.cell.uri.toString() === this.cell.modified.uri.toString()) {
...
bIa = __decorate([__param(3, ln), __param(4, Jl), __param(5, Il), __param(6, El), __param(7, kc), __param(8, mo), __param(9, ms), __param(10, xd), __param(11, wi), __param(12, Fn), __param(13, uy)], bIa);
Pki = class extends $_u {
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g, f) {
    super(e, t, i, "full", r, s, o, a, l, u, d, m, p, g, f);
    this.cell = t;
    this.templateData = i;
    this._editorViewStateChanged = false;
    this.updateBorders();
  ...
  _buildOutputRendererContainer() {
    if (!this._outputViewContainer) {
      ...
      this._outputLeftContainer = Rt(this._outputViewContainer, Ct(".output-view-container-left"));
      this._outputRightContainer = Rt(this._outputViewContainer, Ct(".output-view-container-right"));
      this._outputMetadataContainer = Rt(this._outputViewContainer, Ct(".output-view-container-metadata"));
      const e = this.cell.checkIfOutputsModified();
      const t = e && e.kind === 1 && this.cell.original.outputs.length === 1 && this.cell.modified.outputs.length === 1 && mhy(this.cell.original.outputs[0], this.cell.modified.outputs[0]) === 1;
      if (e && !t) {
        const i = this.notebookEditor.onDidDynamicOutputRendered(s => {
          if (s.cell.uri.toString() === this.cell.original.uri.toString() && this.cell.checkIfOutputsModified()) {
            this.notebookEditor.deltaCellOutputContainerClassNames(s1.Original, this.cell.original.id, ["nb-cellDeleted"], []);
            i.dispose();
          }
        });
        const r = this.notebookEditor.onDidDynamicOutputRendered(s => {
          if (s.cell.uri.toString() === this.cell.modified.uri.toString() && this.cell.checkIfOutputsModified()) {
            this.notebookEditor.deltaCellOutputContainerClassNames(s1.Modified, this.cell.modified.id, ["nb-cellAdded"], []);
            r.dispose();
          }
        });
      ...
      this._outputLeftView = this.instantiationService.createInstance(TEt, this.notebookEditor, this.notebookEditor.textModel, this.cell, this.cell.original, s1.Original, this._outputLeftContainer);
      this._outputLeftView.render();
      this._register(this._outputLeftView);
      this._outputRightView = this.instantiationService.createInstance(TEt, this.notebookEditor, this.notebookEditor.textModel, this.cell, this.cell.modified, s1.Modified, this._outputRightContainer);
      this._outputRightView.render();
      this._register(this._outputRightView);
      if (e && !t) {
        this._decorate();
      ...
      if (t) {
        ...
        this._outputMetadataEditor = this.instantiationService.createInstance(JB, this._outputMetadataContainer, {
          ...Iki,
          overflowWidgetsDomNode: this.notebookEditor.getOverflowContainerDomNode(),
          readOnly: true,
          ignoreTrimWhitespace: false,
        ...
        const i = JSON.stringify(this.cell.original.outputs[0].metadata ?? {}, undefined, "\t");
        const r = JSON.stringify(this.cell.modified.outputs[0].metadata ?? {}, undefined, "\t");
        const s = this.languageService.createById("json");
        const o = this.modelService.createModel(i, s, undefined, true);
        const a = this.modelService.createModel(r, s, undefined, true);
        ...
        this._register(this._outputMetadataEditor.onDidContentSizeChange(l => {
  ...
  updateSourceEditor() {
    ...
    const t = () => {
      ...
      const l = e.textModel.textBuffer.getLineCount();
      const u = this.notebookEditor.getLayoutInfo().fontInfo.lineHeight || 17;
      const d = this.cell.layoutInfo.editorHeight !== 0 ? this.cell.layoutInfo.editorHeight : this.cell.computeInputEditorHeight(u);
      this._editorContainer.style.height = `${d}px`;
      this._editorContainer.style.display = "block";
      if (this._editor) {
        const g = this._editor.getContentHeight();
        if (g >= 0) {
          this.cell.editorHeight = g;
        }
        return;
      }
      this._editor = this.templateData.sourceEditor;
      ...
      const p = this._register(U_u(this.configurationService));
      if (p.options.enabled) {
        m.hideUnchangedRegions = p.options;
      }
      this._editor.updateOptions(m);
      this._register(p.onDidChangeEnablement(() => {
        m.hideUnchangedRegions = p.options;
        this._editor?.updateOptions(m);
      }));
      this._editor.layout({
      ...
      this._register(this._editor.onDidContentSizeChange(g => {
        if (this.cell.cellFoldingState === kD.Expanded && g.contentHeightChanged && this.cell.layoutInfo.editorHeight !== g.contentHeight) {
          this.cell.editorHeight = g.contentHeight;
        }
      }));
      this._initializeSourceDiffEditor();
    };
    this._cellHeader = this._register(this.instantiationService.createInstance(f2e, this.cell, this._cellHeaderContainer, this.notebookEditor, {
      updateInfoRendering: () => t(),
      checkIfModified: () => this.cell.modified?.textModel.getTextBufferHash() !== this.cell.original?.textModel.getTextBufferHash() ? {
        reason: undefined
      } : false,
      getFoldingState: () => this.cell.cellFoldingState,
      updateFoldingState: l => this.cell.cellFoldingState = l,
      unChangedLabel: "Input",
    ...
    const s = g6f.bindTo(i);
    const o = this.textConfigurationService.getValue(this.cell.modified.uri, "diffEditor.ignoreTrimWhitespace");
    s.set(o);
    this._toolbar = this.templateData.toolbar;
    this._toolbar.context = this.cell;
    const a = () => {
    ...
    this._register(this.cell.modified.textModel.onDidChangeContent(() => a()));
    this._register(this.textConfigurationService.onDidChangeConfiguration(l => {
      if (l.affectsConfiguration(this.cell.modified.uri, "diffEditor") && l.affectedKeys.has("diffEditor.ignoreTrimWhitespace")) {
        a();
      }
    }));
    a();
  ...
  layout(e) {
    r_(As(this._diffEditorContainer), () => {
      if (e.editorHeight && this._editor) {
        this._editorContainer.style.height = `${this.cell.layoutInfo.editorHeight}px`;
        this._editor.layout({
          width: this._editor.getViewWidth(),
          height: this.cell.layoutInfo.editorHeight
      ...
      if ((e.metadataHeight || e.outerWidth) && this._metadataEditorContainer) {
...
Pki = __decorate([__param(3, ln), __param(4, Jl), __param(5, Il), __param(6, El), __param(7, kc), __param(8, mo), __param(9, ms), __param(10, xd), __param(11, wi), __param(12, Fn), __param(13, uy)], Pki);
k6f = class extends at {
  constructor(n) {
    super();
    this.container = n;
    this._nodes = kl("div.diff-hidden-cells", [kl("div.center@content", {
      style: {
        display: "flex"
      }
    }, [Ct("a", {
