...
G8f = class {
  ...
  intervalSearch(n, e, t, i, r, s = false) {
...
GTa = class extends at {
  ...
  get selectionHandles() {
    const e = new Set();
    const t = [];
    Qne(this._selectionCollection.selections).map(i => i < this.length ? this.cellAt(i) : undefined).forEach(i => {
      if (i && !e.has(i.handle)) {
        t.push(i.handle);
      }
    });
  ...
  set selectionHandles(e) {
    ...
    this._selectionCollection.setSelections(Vpi(t), true, "model");
  }
  get focused() {
  ...
  constructor(e, t, i, r, s, o, a, l, u, d) {
    ...
    this._undoService = l;
    this._textModelService = u;
    this.notebookExecutionStateService = d;
    ...
    this._selectionCollection = this._register(new q8f());
    this._decorationsTree = new G8f();
    this._decorations = Object.create(null);
    this._lastDecorationId = 0;
    this._foldingRanges = null;
    this._onDidFoldingStateChanged = new Qe();
    ...
    JTa++;
    this.id = "$notebookViewModel" + JTa;
    this._instanceId = V0c(JTa);
    const m = (g, f) => {
      ...
      A.reverse().forEach(I => {
        ...
        this._decorationsTree.acceptReplace(I[0], I[1], I[2].length, true);
        B.forEach(R => {
          this._handleToViewCellMapping.delete(R.handle);
          R.dispose();
        });
        I[2].forEach(R => {
          this._handleToViewCellMapping.set(R.handle, R);
      ...
      if (w.length) {
        const I = w[0];
        const B = this._viewCells.indexOf(this.getCellByHandle(I));
        C = [I];
        let R = 0;
      ...
      this._selectionCollection.setState(Vpi([x[0]])[0], Vpi(x), true, "model");
    };
    this._register(this._notebook.onDidChangeContent(g => {
      for (let f = 0; f < g.rawEvents.length; f++) {
        const A = g.rawEvents[f];
        let w = [];
        const C = g.synchronous ?? true;
        if (A.kind === sb.ModelChange || A.kind === sb.Initialize) {
          w = A.changes;
          m(w, C);
          continue;
        } else if (A.kind === sb.Move) {
          m([[A.index, A.length, []]], C);
          m([[A.newIdx, 0, A.cells]], C);
        } else {
          continue;
        }
      }
    ...
    this._register(this._notebook.onDidChangeContent(g => {
      g.rawEvents.forEach(f => {
        if (f.kind === sb.ChangeDocumentMetadata) {
          this._viewContext.eventDispatcher.emit([new qEf(this._notebook.metadata)]);
        }
      });
      if (g.endSelectionState) {
        this.updateSelectionsState(g.endSelectionState);
      }
    }));
    this._register(this._viewContext.eventDispatcher.onDidChangeLayout(g => {
      this._layoutInfo = g.value;
      this._viewCells.forEach(f => {
        if (f.cellKind === zd.Markup) {
          if (g.source.width || g.source.fontInfo) {
            f.layoutChange({
              outerWidth: g.value.width,
              font: g.value.fontInfo
            });
          }
        } else if (g.source.width !== undefined) {
          f.layoutChange({
            outerWidth: g.value.width,
            font: g.value.fontInfo
          });
    ...
    this._register(this._viewContext.notebookOptions.onDidChangeOptions(g => {
      for (let f = 0; f < this.length; f++) {
        this._viewCells[f].updateOptions(g);
      }
    }));
    ...
    for (let g = 0; g < p; g++) {
      this._viewCells.push(H8f(this._instantiationService, this, this._notebook.cells[g], this._viewContext));
    }
  ...
  getCellsBefore(e) {
    ...
    if (t !== -1) {
      return this._viewCells.slice(0, t);
    }
  }
  getCellsAfter(e) {
    const t = this.getCellIndex(e);
    if (t !== -1) {
      return this._viewCells.slice(t + 1);
    }
  }
  validateRange(e) {
    if (!e) {
    ...
    const t = zA(e.start, 0, this.length);
    const i = zA(e.end, 0, this.length);
    if (t <= i) {
      return {
        start: t,
        end: i
  ...
  updateSelectionsState(e, t = "model") {
    if (this._focused || t === "model") {
      if (e.kind === Wy.Handle) {
        const i = e.primary !== null ? this.getCellIndexByHandle(e.primary) : null;
        const r = i !== null ? this.validateRange({
          start: i,
          end: i + 1
        }) : null;
        const s = Vpi(e.selections.map(o => this.getCellIndexByHandle(o))).map(o => this.validateRange(o)).filter(o => o !== null);
        this._selectionCollection.setState(r, QUe(s), true, t);
      } else {
        const i = this.validateRange(e.focus);
        const r = e.selections.map(s => this.validateRange(s)).filter(s => s !== null);
        this._selectionCollection.setState(i, QUe(r), true, t);
      }
    }
  }
  ...
  getFoldingState(e) {
    ...
    const t = this._foldingRanges.findRange(e + 1);
    if (this._foldingRanges.getStartLineNumber(t) - 1 !== e) {
      return 0;
    } else if (this._foldingRanges.isCollapsed(t)) {
      return 2;
    } else {
      return 1;
    }
  ...
  getFoldedLength(e) {
    ...
    const t = this._foldingRanges.findRange(e + 1);
    const i = this._foldingRanges.getStartLineNumber(t) - 1;
  ...
  updateFoldingRanges(e) {
    ...
    for (; r < e.length; r++) {
      ...
      const l = e.getStartLineNumber(r) + 1;
      const u = e.getEndLineNumber(r);
      if (!(o <= l) || !(u <= a)) {
        if (!t && s < this._hiddenRanges.length && this._hiddenRanges[s].start + 1 === l && this._hiddenRanges[s].end + 1 === u) {
          i.push(this._hiddenRanges[s]);
          s++;
        } else {
          t = true;
          i.push({
    ...
    if (t || s < this._hiddenRanges.length) {
      this._hiddenRanges = i;
      this._onDidFoldingStateChanged.fire();
    }
    this._viewCells.forEach(l => {
      if (l.cellKind === zd.Markup) {
        l.triggerFoldingStateChange();
      }
    });
  }
  getHiddenRanges() {
  ...
  getNearestVisibleCellIndexUpwards(e) {
    for (let t = this._hiddenRanges.length - 1; t >= 0; t--) {
      const i = this._hiddenRanges[t];
      const r = i.start - 1;
      const s = i.end;
      if (!(r > e)) {
        if (r <= e && s >= e) {
  ...
  getNextVisibleCellIndex(e) {
    for (let t = 0; t < this._hiddenRanges.length; t++) {
      const i = this._hiddenRanges[t];
      const r = i.start - 1;
      const s = i.end;
      if (!(s < e)) {
  ...
  _getDecorationRange(e) {
    ...
    if (t.cachedVersionId !== i) {
      this._decorationsTree.resolveNode(t, i);
    }
    if (t.range === null) {
      return {
        start: t.cachedAbsoluteStart - 1,
        end: t.cachedAbsoluteEnd - 1
      };
    } else {
      return {
        start: t.range.startLineNumber - 1,
        end: t.range.endLineNumber - 1
      };
    }
  }
  setTrackedRange(e, t, i) {
    ...
    if (r) {
      if (t) {
        ...
        r.reset(this.getVersionId(), t.start, t.end + 1, new Zt(t.start + 1, 1, t.end + 1, 1));
        r.setOptions(p_u[i]);
      ...
      return this._deltaCellDecorationsImpl(0, [], [{
        range: new Zt(t.start + 1, 1, t.end + 1, 1),
        options: p_u[i]
      }])[0];
    } else {
      return null;
  ...
  _deltaCellDecorationsImpl(e, t, i) {
    ...
    while (o < s || l < a) {
      ...
      if (o < s) {
        do {
          d = this._decorations[t[o++]];
        } while (!d && o < s);
        if (d) {
          this._decorationsTree.delete(d);
        }
      }
      if (l < a) {
        if (!d) {
          const f = ++this._lastDecorationId;
          const A = `${this._instanceId};${f}`;
          d = new pOo(A, 0, 0);
          this._decorations[A] = d;
        }
        const m = i[l];
        const p = m.range;
        const g = Bdy(m.options);
        d.ownerId = e;
        d.reset(r, p.startLineNumber, p.endLineNumber, Zt.lift(p));
        d.setOptions(g);
        this._decorationsTree.insert(d);
        u[l] = d.id;
        l++;
      } else if (d) {
        delete this._decorations[d.id];
      }
    }
    return u;
  }
  deltaCellDecorations(e, t) {
    e.forEach(r => {
      ...
      if (s !== undefined) {
        this.getCellByHandle(s)?.deltaCellDecorations([r], []);
        this._decorationIdToCellMap.delete(r);
      }
      if (this._overviewRulerDecorations.has(r)) {
        this._overviewRulerDecorations.delete(r);
      }
    });
    const i = [];
    t.forEach(r => {
      if (Mhy(r)) {
        ...
        const s = ++this._lastOverviewRulerDecorationId;
        const o = `_overview_${this.id};${s}`;
        this._overviewRulerDecorations.set(o, r);
        i.push(o);
      }
    });
    return i;
  ...
  deltaCellStatusBarItems(e, t) {
    const i = vze(e, s => this._statusBarItemIdToCellMap.get(s) ?? -1);
    const r = [];
    t.forEach(s => {
      const o = this.getCellByHandle(s.handle);
      const a = i[s.handle] ?? [];
      delete i[s.handle];
      a.forEach(u => this._statusBarItemIdToCellMap.delete(u));
    ...
    for (const s in i) {
      const o = parseInt(s);
      const a = i[o];
      this.getCellByHandle(o)?.deltaCellStatusBarItems(a, []);
      a.forEach(u => this._statusBarItemIdToCellMap.delete(u));
    }
    return r;
  }
  ...
  getEditorViewState() {
    ...
    this._viewCells.forEach((o, a) => {
      if (o.getEditState() === aw.Editing) {
        e[a] = true;
      }
      if (o.isInputCollapsed) {
        t[a] = true;
      }
      if (o instanceof jJ && o.isOutputCollapsed) {
        i[a] = true;
      }
      if (o.lineNumbers !== "inherit") {
        r[a] = o.lineNumbers;
      }
    });
    const s = {};
    this._viewCells.map(o => ({
      handle: o.model.handle,
      state: o.saveEditorViewState()
    })).forEach((o, a) => {
      if (o.state) {
        s[a] = o.state;
      }
    ...
    return {
      editingCells: e,
      editorViewStates: s,
      cellLineNumberStates: r,
      collapsedInputCells: t,
      collapsedOutputCells: i
    };
  }
  restoreEditorViewState(e) {
    if (e) {
      this._viewCells.forEach((t, i) => {
        const r = e.editingCells && e.editingCells[i];
        const s = e.editorViewStates && e.editorViewStates[i];
        t.updateEditState(r ? aw.Editing : aw.Preview, "viewState");
        const o = e.cellTotalHeights ? e.cellTotalHeights[i] : undefined;
        t.restoreEditorViewState(s, o);
        if (e.collapsedInputCells && e.collapsedInputCells[i]) {
          t.isInputCollapsed = true;
        }
        if (e.collapsedOutputCells && e.collapsedOutputCells[i] && t instanceof jJ) {
          t.isOutputCollapsed = true;
        }
        if (e.cellLineNumberStates && e.cellLineNumberStates[i]) {
          t.lineNumbers = e.cellLineNumberStates[i];
        }
      });
    }
  }
  ...
  _deltaModelDecorationsImpl(e, t) {
    ...
    e.forEach(s => {
      ...
      const a = i.get(o);
      if (a) {
        a.oldDecorations = s.decorations;
      }
    });
    t.forEach(s => {
      const o = s.ownerId;
      ...
      const a = i.get(o);
      if (a) {
        a.newDecorations = s.decorations;
      }
    });
    const r = [];
    i.forEach((s, o) => {
  ...
  find(e, t) {
    ...
    r.forEach((s, o) => {
      ...
      if (a) {
        i.push(new Aki(a.cell, o, a.contentMatches, []));
      }
    });
    return i.filter(s => s.cell.cellKind === zd.Code ? t.includeCodeInput : (s.cell.getEditState() === aw.Editing || !t.includeMarkupPreview) && t.includeMarkupInput);
  }
  replaceOne(e, t, i) {
    const r = e;
    this._lastNotebookEditResource.push(r.uri);
    return r.resolveTextModel().then(() => {
  ...
  async replaceAll(e, t) {
    ...
    const i = [];
    this._lastNotebookEditResource.push(e[0].cell.uri);
    e.forEach(r => {
    ...
    return Promise.all(e.map(r => r.cell.resolveTextModel())).then(async () => {
  ...
  async _withElement(e, t) {
    const i = this._viewCells.filter(s => e.matchesResource(s.uri));
    const r = await Promise.all(i.map(s => this._textModelService.createModelReference(s.uri)));
    await t();
    r.forEach(s => s.dispose());
  }
  async undo() {
    const e = this._undoService.getElements(this.uri);
    const t = e.past.length ? e.past[e.past.length - 1] : undefined;
    if (t && t instanceof oRe || t instanceof _ft) {
      await this._withElement(t, async () => {
