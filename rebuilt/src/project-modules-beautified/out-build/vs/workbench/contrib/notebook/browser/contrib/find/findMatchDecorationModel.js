...
m_u = class extends at {
  ...
  async highlightCurrentFindMatchDecorationInCell(n, e) {
    this.clearCurrentFindMatchDecoration();
    this._notebookEditor.changeModelDecorations(t => {
      const i = edn._CURRENT_FIND_MATCH_DECORATION;
      const r = [{
        range: e,
        options: i
      }];
      ...
      this._currentMatchDecorations = {
        kind: "input",
        decorations: t.deltaDecorations(this._currentMatchDecorations?.kind === "input" ? this._currentMatchDecorations.decorations : [], [s])
      };
    });
  ...
  clearCurrentFindMatchDecoration() {
    if (this._currentMatchDecorations?.kind === "input") {
      this._notebookEditor.changeModelDecorations(n => {
        n.deltaDecorations(this._currentMatchDecorations?.kind === "input" ? this._currentMatchDecorations.decorations : [], []);
        this._currentMatchDecorations = null;
      });
  ...
  setAllFindMatchesDecorations(n) {
    this._notebookEditor.changeModelDecorations(e => {
      const t = edn._FIND_MATCH_DECORATION;
      const i = n.map(r => {
        const s = new Array(r.contentMatches.length);
        for (let o = 0; o < r.contentMatches.length; o++) {
          s[o] = {
            range: r.contentMatches[o].range,
            options: t
          };
        }
        return {
    ...
    this._allMatchesCellDecorations = this._notebookEditor.deltaCellDecorations(this._allMatchesCellDecorations, n.map(e => ({
      ownerId: e.cell.handle,
      handle: e.cell.handle,
      options: {
        overviewRuler: {
          color: W5e,
          modelRanges: e.contentMatches.map(t => t.range),
          includeOutput: e.webviewMatches.length > 0,
          position: HU.Center
        }
      }
    })));
