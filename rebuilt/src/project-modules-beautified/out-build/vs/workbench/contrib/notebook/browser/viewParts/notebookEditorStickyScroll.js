...
E_u = class Ezb extends at {
  constructor(e, t, i, r, s) {
    ...
    this.header = i;
    this.entry = r;
    this.notebookEditor = s;
    this._register(ei(this.header, ir.CLICK || MA.Tap, () => {
      this.focusCell();
    }));
  ...
  toggleFoldRange(e) {
    const t = this.notebookEditor.getContribution(m2e.id);
    const i = this.entry.index;
    const r = this.entry.level;
    const s = e === 2 ? 1 : 2;
    t.setFoldingStateDown(i, s, r);
    this.focusCell();
  }
  focusCell() {
    this.notebookEditor.focusNotebookCell(this.entry.cell, "container");
    const e = this.notebookEditor.getAbsoluteTopOfElement(this.entry.cell);
    const t = Ezb.getParentCount(this.entry);
...
kX = k_u = class extends at {
  ...
  getCurrentStickyHeight() {
    let e = 0;
    this.currentStickyLines.forEach(t => {
      if (t.rendered) {
        e += 22;
      }
    });
  ...
  onContextMenu(e) {
    const t = new yy(As(this.domNode), e);
    const i = t.target.parentElement;
    const r = Array.from(this.currentStickyLines.values()).find(o => o.line.element.contains(i))?.line.entry;
    if (!r) {
      return;
    }
    const s = {
    ...
    this._contextMenuService.showContextMenu({
      menuId: st.NotebookStickyScrollContext,
      getAnchor: () => t,
      menuActionOptions: {
  ...
  updateConfig(e) {
    if (e.stickyScrollEnabled) {
      ...
      this.updateContent(Eki(this.notebookEditor, this.notebookCellList, this.notebookCellOutlineReference?.object?.entries, this.getCurrentStickyHeight()));
    }
  }
  async init() {
    const {
    ...
    this._disposables.add(e.onDidChange(() => {
      const i = Eki(this.notebookEditor, this.notebookCellList, e.entries, this.getCurrentStickyHeight());
      if (this.compareStickyLineMaps(i, this.currentStickyLines)) {
        this.disposeStickyLineMap(i);
      } else {
    ...
    this._disposables.add(this.notebookEditor.onDidScroll(() => {
      ...
      i.trigger(() => {
        ...
        if (this.compareStickyLineMaps(r, this.currentStickyLines)) {
          this.disposeStickyLineMap(r);
        } else {
  ...
  static getVisibleOutlineEntry(e, t) {
    ...
    if (r >= 0) {
      ...
      const o = [];
      s.asFlatList(o);
      return o.find(a => a.index === e);
    }
  }
  updateContent(e) {
    th(this.domNode);
    this.disposeCurrentStickyLines();
    this.renderStickyLines(e, this.domNode);
    ...
    if (i !== 0) {
      ...
      const r = this._layoutDisposableStore.add(r_(As(this.getDomNode()), () => {
        this.layoutFn(i);
        this.updateDisplay();
        this._layoutDisposableStore.delete(r);
      }));
    } else {
      this.updateDisplay();
    }
  ...
  updateDisplay() {
    if (this.getCurrentStickyHeight() > 0) {
  ...
  static computeStickyHeight(e) {
    let t = 0;
    for (e.cell.cellKind === zd.Markup && e.level < 7 && (t += 22); e.parent;) {
      t += 22;
      e = e.parent;
    }
    return t;
  ...
  renderStickyLines(e, t) {
    ...
    for (const [, r] of i) {
      if (r.rendered) {
        t.append(r.line.element);
      }
    }
  }
  static createStickyElement(e, t) {
    const i = document.createElement("div");
    i.classList.add("notebook-sticky-scroll-element");
    if (t.notebookOptions.getLayoutConfiguration().stickyScrollMode === "indented") {
      i.style.paddingLeft = E_u.getParentCount(e) * 10 + "px";
    }
    let s = false;
    if (e.cell.cellKind === zd.Markup) {
      s = e.cell.foldingState === 2;
    }
    const o = new s6f(s, 16);
    o.domNode.classList.add("notebook-sticky-scroll-folding-icon");
    o.setVisible(true);
    const a = document.createElement("div");
    a.classList.add("notebook-sticky-scroll-header");
    a.innerText = e.label;
    i.append(o.domNode, a);
    return new E_u(i, o, a, e, t);
  }
