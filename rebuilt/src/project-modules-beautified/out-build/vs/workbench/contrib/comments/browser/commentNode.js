...
bTa = class extends at {
  ...
  constructor(e, t, i, r, s, o, a, l, u, d, m, p, g, f, A, w, C, x) {
    super();
    this.parentEditor = e;
    this.commentThread = t;
    this.comment = i;
    this.pendingEdit = r;
    this.owner = s;
    this.resource = o;
    this.parentThread = a;
    this.markdownRenderer = l;
    this.instantiationService = u;
    this.commentService = d;
    this.notificationService = m;
    this.contextMenuService = p;
    this.configurationService = f;
    this.hoverService = A;
    this.accessibilityService = w;
    this.keybindingService = C;
    this.textModelService = x;
    this._md = this._register(new uo());
    this._editAction = null;
    this._commentEditContainer = null;
    this._reactionsActionBar = this._register(new uo());
    this._reactionActions = this._register(new Ut());
    this._commentEditor = null;
    this._commentEditorDisposables = [];
    this._commentEditorModel = null;
    this._editorHeight = kbn;
    this._actionRunner = this._register(new g8f());
    this.toolbar = this._register(new uo());
    this._commentFormActions = null;
    this._commentEditorActions = null;
    this._onDidClick = new Qe();
    this.isEditing = false;
    this._domNode = Ct("div.review-comment");
    this._contextKeyService = this._register(g.createScoped(this._domNode));
    this._commentContextValue = SD.commentContext.bindTo(this._contextKeyService);
    ...
    this._avatar = Rt(this._domNode, Ct("div.avatar-container"));
    this.updateCommentUserIcon(this.comment.userIconPath);
    this._commentDetailsContainer = Rt(this._domNode, Ct(".review-comment-contents"));
    this.createHeader(this._commentDetailsContainer);
    this._body = document.createElement("div");
    this._body.classList.add("comment-body", USe);
    if (f.getValue(_bn)?.maxHeight !== false) {
    ...
    this._domNode.setAttribute("aria-label", `${i.userName}, ${this.commentBodyValue}`);
    this._domNode.setAttribute("role", "treeitem");
    this._clearTimeout = null;
    this._register(ei(this._domNode, ir.CLICK, () => this.isEditing || this._onDidClick.fire(this)));
    this._register(ei(this._domNode, ir.CONTEXT_MENU, I => this.onContextMenu(I)));
    if (r) {
  ...
  createScroll(e, t) {
    this._scrollable = this._register(new Fde({
      ...
      scheduleAtNextAnimationFrame: r => r_(As(e), r)
    }));
    this._scrollableElement = this._register(new Yft(t, {
      horizontal: 3,
      vertical: 3
    }, this._scrollable));
    this._register(this._scrollableElement.onScroll(r => {
      if (r.scrollLeftChanged) {
        t.scrollLeft = r.scrollLeft;
      }
      if (r.scrollTopChanged) {
        t.scrollTop = r.scrollTop;
      }
    }));
    const i = this._register(new Hg(t, "scroll")).event;
    this._register(i(r => {
      const s = this._scrollableElement.getScrollPosition();
      const o = Math.abs(t.scrollLeft - s.scrollLeft) <= 1 ? undefined : t.scrollLeft;
      const a = Math.abs(t.scrollTop - s.scrollTop) <= 1 ? undefined : t.scrollTop;
  ...
  updateCommentBody(e) {
    ...
    if (typeof e == "string") {
      this._plainText = Rt(this._body, Ct(".comment-body-plainstring"));
      this._plainText.innerText = e;
    } else {
      this._md.value = this.markdownRenderer.render(e);
  ...
  updateCommentUserIcon(e) {
    ...
    if (e) {
      ...
      t.src = og.uriToBrowserUri(je.revive(e)).toString(true);
      t.onerror = i => t.remove();
    }
  }
  get onDidClick() {
  ...
  createTimestamp(e) {
    this._timestamp = Rt(e, Ct("span.timestamp-container"));
  ...
  updateTimestamp(e) {
    ...
    const t = e !== undefined ? new Date(e) : undefined;
    if (t) {
      if (this._timestampWidget) {
        this._timestampWidget.setTimestamp(t);
      } else {
        this._timestampWidget = new fTa(this.configurationService, this.hoverService, this._timestamp, t);
  ...
  createHeader(e) {
    const t = Rt(e, Ct(`div.comment-title.${USe}`));
    const i = Rt(t, Ct("comment-header-info"));
    const r = Rt(i, Ct("strong.author"));
    r.innerText = this.comment.userName;
    this.createTimestamp(i);
    this._isPendingLabel = Rt(i, Ct("span.isPending"));
    if (this.comment.label) {
      this._isPendingLabel.innerText = this.comment.label;
    } else {
      this._isPendingLabel.innerText = "";
    }
    this._actionsToolbarContainer = Rt(t, Ct(".comment-actions"));
    this.toggleToolbarHidden(true);
    this.createActionsToolbar();
  }
  toggleToolbarHidden(e) {
    if (e && !this.accessibilityService.isScreenReaderOptimized()) {
  ...
  createToolbar() {
    this.toolbar.value = new ave(this._actionsToolbarContainer, this.contextMenuService, {
      actionViewItemProvider: (e, t) => e.id === Ebn.ID ? new VH(e, e.menuActions, this.contextMenuService, {
        ...
        classNames: ["toolbar-toggle-pickReactions", ...Qt.asClassNameArray(Be.reactions)],
        anchorAlignmentProvider: () => 1
      }) : this.actionViewItemProvider(e, t),
      orientation: 0
    });
  ...
  createActionsToolbar() {
    const e = [];
    const i = this.commentService.hasReactionHandler(this.owner) ? this.createReactionPicker(this.comment.commentReactions || []) : undefined;
    if (i) {
      e.push(i);
    }
    const r = this._commentMenus.getCommentTitleActions(this.comment, this._contextKeyService);
    this._register(r);
    this._register(r.onDidChange(a => {
      const {
        primary: l,
        secondary: u
      } = this.getToolbarActions(r);
      if (!this.toolbar && (l.length || u.length)) {
        this.createToolbar();
      }
      if (i) {
        l.unshift(i);
      }
      this.toolbar.value.setActions(l, u);
    }));
    const {
      primary: s,
      secondary: o
    ...
    if (e.length || o.length) {
      this.createToolbar();
      this.toolbar.value.setActions(e, o);
    }
  }
  actionViewItemProvider(e, t) {
    ...
    if (e.id === Hwu.ID) {
      ...
      return this.instantiationService.createInstance(f2, e, {
        hoverDelegate: t.hoverDelegate
      });
    } else if (e instanceof h2) {
      return this.instantiationService.createInstance(CRe, e, t);
    } else {
      return new aI({}, e, t);
    }
  }
  ...
  createReactionPicker(e) {
    ...
    let i = [];
    if (e && e.length) {
      i = e.map(s => this._reactionActions.add(new Hs(`reaction.command.${s.label}`, `${s.label}`, "", true, async () => {
        try {
    ...
    const r = this._reactionActions.add(new VH(t, t.menuActions, this.contextMenuService, {
      actionViewItemProvider: (s, o) => s.id === Ebn.ID ? r : this.actionViewItemProvider(s, o),
  ...
  createReactionsContainer(e) {
    ...
    this._reactionActionsContainer = Rt(e, Ct("div.comment-reactions"));
    this._reactionsActionBar.value = new Gf(this._reactionActionsContainer, {
      actionViewItemProvider: (i, r) => i.id === Ebn.ID ? new VH(i, i.menuActions, this.contextMenuService, {
        actionViewItemProvider: (s, o) => this.actionViewItemProvider(s, o),
        classNames: ["toolbar-toggle-pickReactions", ...Qt.asClassNameArray(Be.reactions)],
        anchorAlignmentProvider: () => 1
      }) : this.actionViewItemProvider(i, r)
    });
    const t = this.commentService.hasReactionHandler(this.owner);
    this.comment.commentReactions?.filter(i => !!i.count).map(i => {
      const r = this._reactionActions.add(new Hwu(`reaction.${i.label}`, `${i.label}`, i.hasReacted && (i.canEdit || t) ? "active" : "", i.canEdit || t, async () => {
        try {
          ...
          let o;
          if (i.hasReacted) {
            o = s.message ? _(5899, null, s.message) : _(5900, null);
          } else {
            o = s.message ? _(5901, null, s.message) : _(5902, null);
          }
  ...
  get commentBodyValue() {
    if (typeof this.comment.body == "string") {
      return this.comment.body;
    } else {
      return this.comment.body.value;
    }
  }
  async createCommentEditor(e) {
    ...
    const r = await this.textModelService.createModelReference(i);
    this._commentEditorModel = r;
    ...
    if (this.pendingEdit) {
      ...
      const o = this._commentEditorModel.object.textEditorModel.getLineCount();
      const a = this._commentEditorModel.object.textEditorModel.getLineLength(o) + 1;
      this._commentEditor.setPosition(new ar(o, a));
    }
    this.pendingEdit = undefined;
    this._commentEditor.layout({
      width: t.clientWidth - 14,
    ...
    this.commentService.setActiveEditingCommentThread(s);
    this.commentService.setActiveCommentAndThread(this.owner, {
      thread: s,
      comment: this.comment
    });
  ...
  getPendingEdit() {
    ...
    if (this._commentEditor && e && e.getValueLength() > 0) {
      return {
        body: e.getValue(),
        cursor: this._commentEditor.getPosition()
      };
    }
  }
  removeCommentEditor() {
    ...
    Bo(this._commentEditorDisposables);
    this._commentEditorDisposables = [];
  ...
  layout(e) {
    const t = e !== undefined ? e - 72 : this._commentEditor?.getLayoutInfo().width ?? 0;
    this._commentEditor?.layout({
      width: t,
      height: this._editorHeight
    });
    const i = this._body.scrollWidth;
    const r = KFn(this._body);
    const s = this._body.scrollHeight;
    const o = QSc(this._body) + 4;
  ...
  async switchToEditMode() {
    ...
    this._commentEditContainer = Rt(this._commentDetailsContainer, Ct(".edit-container"));
    await this.createCommentEditor(this._commentEditContainer);
    const e = Rt(this._commentEditContainer, Ct(".form-actions"));
    const t = Rt(e, Ct(".other-actions"));
  ...
  setFocus(e, t = false) {
    if (e) {
      ...
      if (this.comment.mode === NOt.Editing) {
        this._commentEditor?.focus();
      }
    } else {
  ...
  registerActionBarListeners(e) {
    ...
    this._register(ei(this._domNode, "mouseleave", () => {
      if (e.classList.contains("mouseover") && !e.classList.contains("tabfocused")) {
        this.toggleToolbarHidden(true);
      }
  ...
  async update(e) {
    if (e.body !== this.comment.body) {
      this.updateCommentBody(e.body);
    }
    if (this.comment.userIconPath && e.userIconPath && je.from(this.comment.userIconPath).toString() !== je.from(e.userIconPath).toString()) {
      this.updateCommentUserIcon(e.userIconPath);
    }
    const t = e.mode !== undefined && e.mode !== this.comment.mode;
    this.comment = e;
    if (t) {
      if (e.mode === NOt.Editing) {
    ...
    if (e.label) {
      this._isPendingLabel.innerText = e.label;
    } else {
      this._isPendingLabel.innerText = "";
    }
    this.createReactionsContainer(this._commentDetailsContainer);
  ...
  onContextMenu(e) {
    ...
    this.contextMenuService.showContextMenu({
      getAnchor: () => t,
      menuId: st.CommentThreadCommentContext,
      menuActionOptions: {
        shouldForwardArgs: true
      },
      contextKeyService: this._contextKeyService,
      actionRunner: this._actionRunner,
      getActionsContext: () => this.commentNodeContext
    });
  }
  focus() {
    this.domNode.focus();
