...
Er();
ps();
$0u = xi("recentFilesTrackerService");
TDa = class extends at {
  constructor(e, t) {
    super();
    this._codeEditorService = e;
    this._workspaceContextService = t;
    this.modelOpenedEvents = new Map();
    this.editorListeners = new Map();
    this.recentlyViewedFiles = [];
    for (let i of this._codeEditorService.listCodeEditors()) {
      this.registerEditorListenersToEditor(i);
    }
    this._register(this._codeEditorService.onCodeEditorAdd(i => {
      const r = i.getModel();
      if (r) {
        this.modelOpenedEvents.set(r.uri.toString(), {
          editor: i,
          model: r
        });
      }
    ...
    this._register(this._codeEditorService.onCodeEditorRemove(i => {
      const r = i.getModel();
      if (r) {
        this.modelOpenedEvents.delete(r.uri.toString());
      }
    }));
  }
  registerEditorListenersToEditor(e) {
    const t = e.getId();
    const i = new Ut();
    i.add(e.onDidChangeModel(r => {
      const s = r.newModelUrl;
      if (s && s.scheme === "file") {
        this.registerFileView(s);
      }
    }));
    ...
    this.editorListeners.set(t, [i]);
    return i;
  }
  getRecentlyViewedFiles(e = 10, t = []) {
    if (e > 100) {
      throw new Error("topK must be less than 100");
    }
  ...
  registerFileView(e) {
    ...
    if (r.find(s => s.relativePath === t)) {
      r = r.filter(s => s.relativePath !== t);
    }
    r.push({
      uri: e,
      relativePath: t,
    ...
    r = r.slice(0, 100);
    this.recentlyViewedFiles = r;
  }
};
TDa = __decorate([__param(0, fl), __param(1, Lr)], TDa);
Vi($0u, TDa, 1);
