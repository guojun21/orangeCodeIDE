...
Vae = class $at {
  ...
  constructor(e) {
    ...
    for (const i of e) {
      if (!(i.replaceRange.start >= t)) {
        throw new _m(`Edits must be disjoint and sorted. Found ${i} after ${t}`);
      }
  ...
  normalize() {
    ...
    for (const i of this.edits) {
      if (i.newText.length !== 0 || i.replaceRange.length !== 0) {
        if (t && t.replaceRange.endExclusive === i.replaceRange.start) {
          t = new E2(t.replaceRange.join(i.replaceRange), t.newText + i.newText);
        } else {
          if (t) {
            e.push(t);
          }
  ...
  apply(e) {
    ...
    for (const r of this.edits) {
      t.push(e.substring(i, r.replaceRange.start));
      t.push(r.newText);
      i = r.replaceRange.endExclusive;
    }
  ...
  inverse(e) {
    ...
    let i = 0;
    for (const r of this.edits) {
      t.push(new E2(dm.ofStartAndLength(r.replaceRange.start + i, r.newText.length), e.substring(r.replaceRange.start, r.replaceRange.endExclusive)));
  ...
  getNewTextRanges() {
    ...
    let t = 0;
    for (const i of this.edits) {
      e.push(dm.ofStartAndLength(i.replaceRange.start + t, i.newText.length));
  ...
  tryRebase(e, t) {
    ...
    let s = 0;
    let o = 0;
    while (s < this.edits.length || r < e.edits.length) {
      const a = e.edits[r];
      const l = this.edits[s];
      if (l) {
        if (!a) {
          i.push(new E2(l.replaceRange.delta(o), l.newText));
          s++;
        ...
        } else if (l.replaceRange.start < a.replaceRange.start) {
          i.push(new E2(l.replaceRange.delta(o), l.newText));
          s++;
        } else {
          r++;
  ...
  applyToOffset(e) {
    let t = 0;
    for (const i of this.edits) {
      if (i.replaceRange.start <= e) {
        if (e < i.replaceRange.endExclusive) {
          return i.replaceRange.start + t;
        }
        t += i.newText.length - i.replaceRange.length;
      } else {
        break;
  ...
  equals(e) {
    if (this.edits.length !== e.edits.length) {
      return false;
    }
    for (let t = 0; t < this.edits.length; t++) {
      if (!this.edits[t].equals(e.edits[t])) {
        return false;
      }
    }
    return true;
