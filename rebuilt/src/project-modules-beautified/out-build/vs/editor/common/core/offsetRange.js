...
dm = class C_e {
  ...
  static addRange(e, t) {
    let i = 0;
    while (i < t.length && t[i].endExclusive < e.start) {
      i++;
    }
    let r = i;
    while (r < t.length && t[r].start <= e.endExclusive) {
      r++;
    }
    if (i === r) {
      t.splice(i, 0, e);
    } else {
      const s = Math.min(e.start, t[i].start);
      const o = Math.max(e.endExclusive, t[r - 1].endExclusive);
  ...
  constructor(e, t) {
    ...
    this.endExclusive = t;
    if (e > t) {
      throw new _m(`Invalid range: ${this.toString()}`);
    }
  }
  get isEmpty() {
  ...
  intersect(e) {
    ...
    const i = Math.min(this.endExclusive, e.endExclusive);
    if (t <= i) {
      return new C_e(t, i);
    }
  }
  ...
  clip(e) {
    if (this.isEmpty) {
      throw new _m(`Invalid clipping range: ${this.toString()}`);
    }
...
Clh = class RJb {
  ...
  addRange(e) {
    let t = 0;
    while (t < this._sortedRanges.length && this._sortedRanges[t].endExclusive < e.start) {
      t++;
    }
    let i = t;
    while (i < this._sortedRanges.length && this._sortedRanges[i].start <= e.endExclusive) {
      i++;
    }
    if (t === i) {
      this._sortedRanges.splice(t, 0, e);
    } else {
      const r = Math.min(e.start, this._sortedRanges[t].start);
      const s = Math.max(e.endExclusive, this._sortedRanges[i - 1].endExclusive);
