...
Fch = class {
  ...
  getElectricCharacters() {
    ...
    if (this._richEditBrackets) {
      for (const e of this._richEditBrackets.brackets) {
        for (const t of e.close) {
  ...
  onElectricCharacter(n, e, t) {
    ...
    if (GBe(e.getStandardTokenType(i))) {
      return null;
    }
    const r = this._richEditBrackets.reversedRegex;
    const s = e.getLineContent().substring(0, t - 1) + n;
    const o = Ede.findPrevBracketInRange(r, 1, s, 0, s.length);
    if (!o) {
      return null;
    }
    const a = s.substring(o.startColumn - 1, o.endColumn - 1).toLowerCase();
    if (this._richEditBrackets.textIsOpenBracket[a]) {
      return null;
    }
