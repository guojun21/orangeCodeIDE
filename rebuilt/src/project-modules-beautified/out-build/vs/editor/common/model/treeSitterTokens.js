...
wOn = class extends mxc {
  constructor(e, t, i, r) {
    ...
    this._tokensChangedListener = this._register(new uo());
    this._onDidChangeBackgroundTokenization = this._register(new uo());
    this._initialize();
  }
  _initialize() {
    const e = this.getLanguageId();
    if (!this._tokenizationSupport || this._lastLanguageId !== e) {
      this._lastLanguageId = e;
      this._tokenizationSupport = RSe.get(e);
      this._tokensChangedListener.value = this._tokenizationSupport?.onDidChangeTokens(t => {
        if (t.textModel === this._textModel) {
          this._onDidChangeTokens.fire(t.changes);
        }
      });
  ...
  getLineTokens(e) {
    ...
    if (this._tokenizationSupport && t.length > 0) {
      ...
      if (i && i.length > 0) {
        return new OB(i, t, this._languageIdCodec);
      }
    }
  ...
  forceTokenization(e) {
    if (this._tokenizationSupport && !this.hasAccurateTokensForLine(e)) {
      this._tokenizationSupport.tokenizeEncoded(e, this._textModel);
    }
  }
  ...
  tokenizeLinesAt(e, t) {
    if (this._tokenizationSupport) {
      const i = this._tokenizationSupport.guessTokensForLinesContent(e, this._textModel, t);
      const r = [];
      if (i) {
        for (let s = 0; s < i.length; s++) {
          r.push(new OB(i[s], t[s], this._languageIdCodec));
        }
        return r;
      }
    }
