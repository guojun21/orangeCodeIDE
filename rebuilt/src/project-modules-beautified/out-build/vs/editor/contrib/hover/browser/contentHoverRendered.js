...
hla = nQl = class extends at {
  constructor(e, t, i, r, s, o) {
    ...
    const {
      showAtPosition: d,
      showAtSecondaryPosition: m
    } = nQl.computeHoverPositions(e, u.range, a);
    this.shouldAppearBeforeContent = a.some(p => p.isBeforeContent);
    this.showAtPosition = d;
    this.showAtSecondaryPosition = m;
    this.initialMousePosX = u.initialMousePosX;
    this.initialMousePosY = u.initialMousePosY;
  ...
  static computeHoverPositions(e, t, i) {
    let r = 1;
    if (e.hasModel()) {
      const d = e._getViewModel();
      const m = d.coordinatesConverter;
      const p = m.convertModelRangeToViewRange(t);
      const g = d.getLineMinColumn(p.startLineNumber);
      const f = new ar(p.startLineNumber, g);
    ...
    for (const d of i) {
      const m = d.range;
      const p = m.startLineNumber === s;
      const g = m.endLineNumber === s;
      if (p && g) {
        const A = m.startColumn;
        const w = Math.min(o, A);
        o = Math.max(w, r);
      }
      if (d.forceShowAtRange) {
        a = m;
      }
    }
    let l;
    ...
    if (a) {
      ...
      l = t.getStartPosition();
      u = new ar(s, o);
    }
    return {
      showAtPosition: l,
      showAtSecondaryPosition: u
    };
  }
};
hla = nQl = __decorate([__param(4, mo), __param(5, Kc)], hla);
zbg = class {
  constructor(n, e) {
    this._statusBar = e;
    n.appendChild(this._statusBar.hoverElement);
  }
  get hoverElement() {
...
mla = class extends at {
  ...
  constructor(e, t, i, r, s, o) {
    ...
    this._register(this._renderParts(t, i, r, s, o));
    this._register(this._registerListenersOnRenderedParts());
    this._register(this._createEditorDecorations(e, i));
    this._updateMarkdownAndColorParticipantInfo(t);
  }
  _createEditorDecorations(e, t) {
    ...
    r.set([{
      range: i,
      options: iQl._DECORATION_OPTIONS
    }]);
    return $i(() => {
      r.clear();
    });
  ...
  _renderParts(e, t, i, r, s) {
    ...
    for (const d of e) {
      ...
      l.add(m);
      for (const p of m.renderedHoverParts) {
  ...
  _registerListenersOnRenderedParts() {
    const e = new Ut();
    this._renderedParts.forEach((t, i) => {
      const r = t.hoverElement;
      r.tabIndex = 0;
      e.add(ei(r, ir.FOCUS_IN, s => {
        s.stopPropagation();
        this._focusedHoverPartIndex = i;
      }));
  ...
  _updateMarkdownAndColorParticipantInfo(e) {
    ...
    if (t) {
      this._markdownHoverParticipant = t;
    }
    this._colorHoverParticipant = e.find(i => i instanceof bpi);
  }
  focusHoverPartWithIndex(e) {
    if (!(e < 0) && !(e >= this._renderedParts.length)) {
      this._renderedParts[e].hoverElement.focus();
    }
  }
  getAccessibleContent() {
    const e = [];
    for (let t = 0; t < this._renderedParts.length; t++) {
      e.push(this.getAccessibleHoverContentAtIndex(t));
    }
    return e.join(`
  ...
  getAccessibleHoverContentAtIndex(e) {
    ...
    if (t.type === "statusBar") {
      ...
      for (const r of t.actions) {
        ...
        if (s) {
          i.push(_(1245, null, r.actionLabel, s));
        } else {
          i.push(_(1246, null, r.actionLabel));
        }
      }
      return i.join(`
`);
  ...
  async updateHoverVerbosityLevel(e, t, i) {
    ...
    for (let s = r.start; s < r.endExclusive; s++) {
      ...
      if (a) {
        this._renderedParts[s] = {
          type: "hoverPart",
          participant: this._markdownHoverParticipant,
          hoverPart: a.hoverPart,
          hoverElement: a.hoverElement
        };
      }
    }
    if (i) {
      if (t >= 0) {
        this.focusHoverPartWithIndex(t);
      } else {
        this._context.focus();
      }
    }
  ...
  doesHoverAtIndexSupportVerbosityAction(e, t) {
    ...
    if (i === undefined) {
      ...
      return this._markdownHoverParticipant.doesMarkdownHoverAtIndexSupportVerbosityAction(i, t);
    }
  }
  isColorPickerVisible() {
  ...
  _findRangeOfMarkdownHoverParts(e) {
    const t = this._renderedParts.slice();
    const i = t.findIndex(o => o.type === "hoverPart" && o.participant === e);
    const r = t.reverse().findIndex(o => o.type === "hoverPart" && o.participant === e);
    const s = r >= 0 ? t.length - r : r;
    return {
      start: i,
      endExclusive: s + 1
    };
