...
ex = class extends at {
  ...
  constructor(e, t, i) {
    ...
    this._onHoverContentsChanged = this._register(new Qe());
    ...
    this._reactToEditorMouseMoveRunner = this._register(new Hu(() => {
  ...
  _onEditorMouseDown(e) {
    ...
    if (!this._shouldKeepHoverWidgetVisible(e)) {
      this.hideContentHover();
    }
  }
  ...
  _isMouseOnContentHoverWidget(e) {
    if (this._contentWidget) {
      return f$o(this._contentWidget.getDomNode(), e.event.posx, e.event.posy);
    } else {
      return false;
    }
  }
  ...
  _onEditorMouseLeave(e) {
    if (!this.shouldKeepOpenOnEditorMouseMoveOrLeave && !(this._cancelScheduler(), this._shouldKeepHoverWidgetVisible(e)) && !fla) {
      this.hideContentHover();
    }
  }
  _shouldKeepCurrentHover(e) {
    ...
    const i = this._hoverSettings.sticky;
    const r = (d, m) => {
      const p = this._isMouseOnContentHoverWidget(d);
      return m && p;
    };
    const s = d => {
      const m = t.isColorPickerVisible;
      const p = this._isMouseOnContentHoverWidget(d);
      const g = m && p;
      const f = m && this._isMouseDown;
      return g || f;
    };
    const o = (d, m) => {
      const p = d.event.browserEvent.view;
      if (p) {
        return m && t.containsNode(p.document.activeElement) && !p.getSelection()?.isCollapsed;
      } else {
        return false;
      }
    };
    const a = t.isFocused;
    const l = t.isResizing;
    const u = this._hoverSettings.sticky && t.isVisibleFromKeyboard;
    return this.shouldKeepOpenOnEditorMouseMoveOrLeave || a || l || u || r(e, i) || s(e) || o(e, i);
  }
  _onEditorMouseMove(e) {
    this._mouseMoveEvent = e;
    if (this._shouldKeepCurrentHover(e)) {
      this._reactToEditorMouseMoveRunner.cancel();
      return;
    }
  ...
  _shouldRescheduleHoverComputation() {
    ...
    return (this._contentWidget?.isVisible ?? false) && this._hoverSettings.sticky && e > 0;
  }
  _reactToEditorMouseMove(e) {
    if ((!this._hoverSettings.enabled || !this._getOrCreateContentWidget().showsOrWillShow(e)) && !fla) {
      this.hideContentHover();
    }
  }
  _onKeyDown(e) {
    ...
    const t = this._isPotentialKeyboardShortcut(e);
    const i = this._isModifierKeyPressed(e);
    if (!t && !i && (!this._contentWidget.isFocused || e.keyCode !== 2)) {
      this.hideContentHover();
    }
  }
  _isPotentialKeyboardShortcut(e) {
    ...
    const t = this._keybindingService.softDispatch(e, this._editor.getDomNode());
    const i = t.kind === 1;
    const r = t.kind === 2 && (t.commandId === E5c || t.commandId === Lvt || t.commandId === Nvt) && this._contentWidget.isVisible;
    return i || r;
  }
  ...
  hideContentHover() {
    if (!fla && !Mvt.dropDownVisible) {
      this._contentWidget?.hide();
    }
  }
  _getOrCreateContentWidget() {
    if (!this._contentWidget) {
      this._contentWidget = this._instantiationService.createInstance(pla, this._editor);
