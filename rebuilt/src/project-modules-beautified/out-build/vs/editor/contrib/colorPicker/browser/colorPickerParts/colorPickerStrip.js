...
AGl = class extends at {
  constructor(n, e, t) {
    ...
    if (t === "standalone") {
      this.domNode = Rt(n, _un(".standalone-strip"));
      this.overlay = Rt(this.domNode, _un(".standalone-overlay"));
    } else {
      this.domNode = Rt(n, _un(".strip"));
      this.overlay = Rt(this.domNode, _un(".overlay"));
    }
    this.slider = Rt(this.domNode, _un(".slider"));
    this.slider.style.top = "0px";
    this._register(ei(this.domNode, ir.POINTER_DOWN, i => this.onPointerDown(i)));
  ...
  onPointerDown(n) {
    ...
    const e = this._register(new Jft());
    const t = qS(this.domNode);
    this.domNode.classList.add("grabbing");
    if (n.target !== this.slider) {
      this.onDidChangeTop(n.offsetY);
    }
    e.startMonitoring(n.target, n.pointerId, n.buttons, r => this.onDidChangeTop(r.pageY - t.top), () => null);
    const i = ei(n.target.ownerDocument, ir.POINTER_UP, () => {
      this._onColorFlushed.fire();
      i.dispose();
      e.stopMonitoring(true);
...
Jmg = class extends AGl {
  ...
  onDidChangeColor(n) {
    ...
    const r = new Xr(new Sa(e, t, i, 1));
    const s = new Xr(new Sa(e, t, i, 0));
