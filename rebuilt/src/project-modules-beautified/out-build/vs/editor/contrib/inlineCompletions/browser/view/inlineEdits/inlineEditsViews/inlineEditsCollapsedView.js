...
vwg = class extends at {
  constructor(n, e) {
    ...
    const i = this._edit.map(a => a?.edit.edits[0] ?? null).map(a => a ? XUe(a, this._editor.getModel()).range.getStartPosition() : null);
    const r = this._editorObs.observePosition(i, this._store);
    const s = Ro(a => {
      const l = r.read(a);
      if (!l) {
        return null;
      }
      const u = this._editorObs.layoutInfoContentLeft.read(a);
      const d = this._editorObs.scrollLeft.read(a);
  ...
  getCollapsedIndicator(n) {
    const e = this._editorObs.layoutInfoContentLeft;
    const t = n.map((r, s) => r ? r.deltaX(-e.read(s)) : null);
    const i = this.createIconPath(t);
    return Mv.svg({
      class: "collapsedView",
      style: {
        position: "absolute",
  ...
  createIconPath(n) {
    return n.map(r => {
      if (!r) {
        return new rua().build();
      }
      const s = r.deltaX(-3).deltaY(-1);
      const o = s.deltaX(6);
      const a = s.deltaY(1);
      const l = o.deltaY(1);
      const u = a.deltaX(3).deltaY(3);
