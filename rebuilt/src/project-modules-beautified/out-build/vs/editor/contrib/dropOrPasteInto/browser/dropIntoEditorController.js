...
EBc();
ZSh();
bla = "editor.dropIntoEditor.preferences";
oQl = "editor.changeDropType";
vla = new Sn("dropWidgetVisible", false, _(1072, null));
mme = class extends at {
  static {
    KUe = this;
  }
  static {
  ...
  constructor(e, t, i, r, s) {
    super();
    this._configService = i;
    this._languageFeaturesService = r;
    this._treeViewsDragAndDropService = s;
    this.treeItemsTransfer = GB.getInstance();
    this._dropProgressManager = this._register(t.createInstance(K3n, "dropIntoEditor", e));
    this._postDropWidgetManager = this._register(t.createInstance(Z3n, "dropIntoEditor", e, vla, {
      id: oQl,
      label: _(1073, null)
    }, () => KUe._configureDefaultAction ? [KUe._configureDefaultAction] : []));
    this._register(e.onDropIntoEditor(o => this.onDropIntoEditor(e, o.position, o.event)));
  }
  clearWidgets() {
  ...
  async onDropIntoEditor(e, t, i) {
    ...
    KUe._currentDropOperation?.cancel();
    e.focus();
    e.setPosition(t);
    const r = dw(async s => {
      const o = new Ut();
      const a = o.add(new ERe(e, 1, undefined, s));
      try {
        ...
        const d = this._languageFeaturesService.documentDropEditProvider.ordered(u).filter(p => p.dropMimeTypes ? p.dropMimeTypes.some(g => l.matches(g)) : true);
        const m = o.add(await this.getDropEdits(d, u, t, l, a.token));
        if (a.token.isCancellationRequested) {
          return;
        }
        if (m.edits.length) {
          const p = this.getInitialActiveEditIndex(u, m.edits);
          const g = e.getOption(36).showDropSelector === "afterDrop";
        ...
        o.dispose();
        if (KUe._currentDropOperation === r) {
          KUe._currentDropOperation = undefined;
        }
      }
    });
  ...
  async getDropEdits(e, t, i, r, s) {
    const o = new Ut();
    const a = await WP(Promise.all(e.map(async u => {
      try {
        const d = await u.provideDocumentDropEdits(t, i, r, s);
        if (d) {
          o.add(d);
        }
        return d?.edits.map(m => ({
          ...m,
          providerId: u.id
        }));
      } catch (d) {
  ...
  getInitialActiveEditIndex(e, t) {
    ...
    for (const r of Array.isArray(i) ? i : []) {
      const s = new p0(r);
      const o = t.findIndex(a => a.kind && s.contains(a.kind));
      if (o >= 0) {
        return o;
      }
    }
