...
u3t = class AK extends mIc {
  ...
  constructor(e, t, i, r) {
    ...
    this._cellValueBuffers = [new ArrayBuffer(s), new ArrayBuffer(s)];
    const o = 2;
    this._scrollOffsetBindBuffer = this._register(GY.createBuffer(this._device, {
      label: "Monaco scroll offset buffer",
      size: o * Float32Array.BYTES_PER_ELEMENT,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST
    })).object;
    this._scrollOffsetValueBuffer = new Float32Array(o);
  }
  ...
  onTokensChanged(e) {
    for (const t of e.ranges) {
      this._invalidateLineRange(t.fromLineNumber, t.toLineNumber);
    }
    return true;
  }
  onLinesDeleted(e) {
  ...
  update(e, t) {
    ...
    const W = AK.maxSupportedColumns * 6;
    const z = this._upToDateLines[this._activeDoubleBufferIndex];
    let Y = 3000;
    let j = 0;
        ...
        case 17:
          {
            H.fill(0);
            Y = 1;
            j = Math.max(j, this._finalRenderedLine);
            this._finalRenderedLine = 0;
            break;
          }
        case 10:
          {
            const ne = (re.fromLineNumber - 1) * AK.maxSupportedColumns * 6;
            const pe = re.toLineNumber * AK.maxSupportedColumns * 6;
            const le = (this._finalRenderedLine - (re.toLineNumber - re.fromLineNumber + 1)) * AK.maxSupportedColumns * 6;
            H.set(H.subarray(pe), ne);
            H.fill(0, le);
            Y = Math.min(Y, re.fromLineNumber);
            j = Math.max(j, this._finalRenderedLine);
            this._finalRenderedLine -= re.toLineNumber - re.fromLineNumber + 1;
            break;
          }
      }
    }
    for (o = e.startLineNumber; o <= e.endLineNumber; o++) {
      if (!this._viewGpuContext.canRender(t, e, o)) {
        R = (o - 1) * AK.maxSupportedColumns * 6;
        N = o * AK.maxSupportedColumns * 6;
        H.fill(0, R, N);
        Y = Math.min(Y, o);
        j = Math.max(j, o);
        continue;
      ...
      if (!z.has(o)) {
        ...
        g = I.minColumn - 1;
        f = 0;
        for (let re = 0, ne = M.getCount(); re < ne; re++) {
          f = M.getEndOffset(re);
          if (!(f <= g)) {
            A = M.getMetadata(re);
            a = g;
            for (; a < f && !(a > AK.maxSupportedColumns); a++) {
                      ...
                      case "color":
                        {
                          ...
                          if (!ke) {
                            throw new _m("Invalid color format " + fe);
                          }
                          C = ke.toNumber32Bit();
                          break;
                        }
              ...
              if (i === " " || i === "\t") {
                p = ((o - 1) * AK.maxSupportedColumns + a) * 6;
                H.fill(0, p, p + 6);
                if (i === "\t") {
                  const le = a + d;
              ...
              m = this._viewGpuContext.atlas.getGlyph(this.glyphRasterizer, i, A, pe, l);
        ...
        R = ((o - 1) * AK.maxSupportedColumns + f) * 6;
        N = o * AK.maxSupportedColumns * 6;
        H.fill(0, R, N);
        z.add(o);
      }
    ...
    Y = Math.min(Y, AK.maxSupportedLines);
    j = Math.min(j, AK.maxSupportedLines);
    if (Y <= j) {
      this._device.queue.writeBuffer(this._cellBindBuffer, (Y - 1) * W * Float32Array.BYTES_PER_ELEMENT, H.buffer, (Y - 1) * W * Float32Array.BYTES_PER_ELEMENT, (j - Y + 1) * W * Float32Array.BYTES_PER_ELEMENT);
    }
    this._finalRenderedLine = Math.max(this._finalRenderedLine, j);
    this._activeDoubleBufferIndex = this._activeDoubleBufferIndex ? 0 : 1;
    this._visibleObjectCount = ee;
    return ee;
  }
  draw(e, t) {
    ...
    e.draw(QVe.length / 2, this._visibleObjectCount, undefined, (t.startLineNumber - 1) * AK.maxSupportedColumns);
  }
