...
mRe = class extends yy {
  constructor(n, e, t) {
    ...
    this.isFromPointerCapture = e;
    this.pos = new GOn(this.posx, this.posy);
    this.editorPos = hTc(t);
    this.relativePos = mTc(t, this.editorPos, this.pos);
  }
};
gvh = class {
  constructor(n) {
  ...
  _create(n) {
    return new mRe(n, false, this._editorViewDomNode);
  }
  onContextMenu(n, e) {
...
bvh = class extends at {
  constructor(n) {
    super();
    this._editorViewDomNode = n;
    this._globalPointerMoveMonitor = this._register(new Jft());
    this._keydownListener = null;
  }
  startMonitoring(n, e, t, i, r) {
    this._keydownListener = _f(n.ownerDocument, "keydown", s => {
      if (!s.toKeyCodeChord().isModifierKey()) {
        this._globalPointerMoveMonitor.stopMonitoring(true, s.browserEvent);
      }
    }, true);
    this._globalPointerMoveMonitor.startMonitoring(n, e, t, s => {
      i(new mRe(s, true, this._editorViewDomNode));
    }, s => {
...
gTc = class mGb {
  ...
  constructor(e) {
    this._editor = e;
    this._instanceId = ++mGb._idPool;
    this._counter = 0;
    this._rules = new Map();
    this._garbageCollectionScheduler = new Hu(() => this.garbageCollect(), 1000);
  }
  ...
  getOrCreateRule(e) {
    ...
    if (!i) {
      ...
      i = new vvh(t, `dyn-rule-${this._instanceId}-${r}`, YFn(this._editor.getContainerDomNode()) ? this._editor.getContainerDomNode() : undefined, e);
      this._rules.set(t, i);
    }
    return i;
  }
...
vvh = class {
  constructor(n, e, t, i) {
    ...
    this._styleElementDisposables = new Ut();
    this._styleElement = wC(t, undefined, this._styleElementDisposables);
