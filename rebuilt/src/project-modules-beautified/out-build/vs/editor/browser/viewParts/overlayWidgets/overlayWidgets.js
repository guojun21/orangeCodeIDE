...
j$();
ri();
ZAh = class extends yW {
  constructor(n, e) {
    super(n);
    this._viewDomNode = e;
    const i = this._context.configuration.options.get(151);
    this._widgets = {};
    this._verticalScrollbarWidth = i.verticalScrollbarWidth;
    this._minimapWidth = i.minimap.minimapWidth;
    this._horizontalScrollbarHeight = i.horizontalScrollbarHeight;
    this._editorHeight = i.height;
    this._editorWidth = i.width;
    ...
    tve.write(this._domNode, 4);
    this._domNode.setClassName("overlayWidgets");
    this.overflowingOverlayWidgetsDomNode = mw(document.createElement("div"));
    tve.write(this.overflowingOverlayWidgetsDomNode, 5);
  ...
  onConfigurationChanged(n) {
    ...
    this._verticalScrollbarWidth = t.verticalScrollbarWidth;
    this._minimapWidth = t.minimap.minimapWidth;
    this._horizontalScrollbarHeight = t.horizontalScrollbarHeight;
    this._editorHeight = t.height;
    this._editorWidth = t.width;
    return true;
  }
  addWidget(n) {
    ...
    if (n.allowEditorOverflow) {
      this.overflowingOverlayWidgetsDomNode.appendChild(e);
    } else {
  ...
  setWidgetPosition(n, e) {
    ...
    const i = e ? e.preference : null;
    const r = e?.stackOridinal;
    if (t.preference === i && t.stack === r) {
      this._updateMaxMinWidth();
      return false;
    } else {
      t.preference = i;
  ...
  _updateMaxMinWidth() {
    ...
    for (let t = 0, i = e.length; t < i; t++) {
      const r = e[t];
      const o = this._widgets[r].widget.getMinContentWidthInPx?.();
  ...
  _renderWidget(n, e) {
    ...
    const i = this._verticalScrollbarWidth * 2 + this._minimapWidth;
    if (n.preference === 0 || n.preference === 1) {
      if (n.preference === 1) {
        const r = t.domNode.clientHeight;
        t.setTop(this._editorHeight - r - this._horizontalScrollbarHeight * 2);
      } else {
        t.setTop(0);
      }
      if (n.stack !== undefined) {
        t.setTop(e[n.preference]);
        e[n.preference] += t.domNode.clientWidth;
      } else {
        t.setRight(i);
      }
    } else if (n.preference === 2) {
      t.domNode.style.right = "50%";
      if (n.stack !== undefined) {
        t.setTop(e[2]);
        e[2] += t.domNode.clientHeight;
      } else {
        t.setTop(0);
      }
    } else if (n.preference === 3) {
      t.setTop("");
      t.setBottom(this._horizontalScrollbarHeight);
      t.setWidth("100%");
    } else {
      const {
        top: r,
