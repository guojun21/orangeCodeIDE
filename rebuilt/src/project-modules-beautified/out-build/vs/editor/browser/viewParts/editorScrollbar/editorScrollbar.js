...
CAh = class extends yW {
  constructor(n, e, t, i) {
    ...
    const l = r.get(111);
    const u = {
      listenOnDomNode: t.domNode,
      className: "editor-scrollable " + Q4n(n.theme.type),
      useShadows: false,
      lazyRender: true,
      vertical: s.vertical,
      ...
      ignoreVerticalScrolling: s.ignoreVerticalScrolling,
    ...
    this.scrollbar = this._register(new Yft(e.domNode, u, this._context.viewLayout.getScrollable()));
    tve.write(this.scrollbar.getDomNode(), 6);
    this.scrollbarDomNode = mw(this.scrollbar.getDomNode());
    ...
    const d = (m, p, g) => {
      ...
      if (p) {
        const A = m.scrollTop;
        if (A) {
          f.scrollTop = this._context.viewLayout.getCurrentScrollTop() + A;
          m.scrollTop = 0;
        }
      }
      if (g) {
        ...
        if (A) {
          f.scrollLeft = this._context.viewLayout.getCurrentScrollLeft() + A;
          m.scrollLeft = 0;
        }
      }
      this._context.viewModel.viewLayout.setScrollPosition(f, 1);
    };
    this._register(ei(t.domNode, "scroll", m => d(t.domNode, true, true)));
    this._register(ei(e.domNode, "scroll", m => d(e.domNode, true, false)));
    this._register(ei(i.domNode, "scroll", m => d(i.domNode, true, false)));
    this._register(ei(this.scrollbarDomNode.domNode, "scroll", m => d(this.scrollbarDomNode.domNode, true, false)));
  }
  dispose() {
    super.dispose();
  }
  _setLayout() {
    ...
    const e = n.get(151);
    this.scrollbarDomNode.setLeft(e.contentLeft);
    if (n.get(74).side === "right") {
      this.scrollbarDomNode.setWidth(e.contentWidth + e.minimap.minimapWidth);
    } else {
      this.scrollbarDomNode.setWidth(e.contentWidth);
    }
    this.scrollbarDomNode.setHeight(e.height);
  }
  ...
  onThemeChanged(n) {
    this.scrollbar.updateClassName("editor-scrollable " + Q4n(this._context.theme.type));
    return true;
  }
  prepareRender(n) {}
  render(n) {
