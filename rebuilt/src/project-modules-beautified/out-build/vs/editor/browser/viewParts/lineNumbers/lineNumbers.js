...
HTc = class Iad extends p9e {
  ...
  constructor(e) {
    ...
    this._context = e;
    this._readConfig();
    this._lastCursorModelPosition = new ar(1, 1);
  ...
  _readConfig() {
    ...
    const t = e.get(69);
    this._renderLineNumbers = t.renderType;
    this._renderCustomLineNumbers = t.renderFn;
    this._renderFinalNewline = e.get(100);
    const i = e.get(151);
    this._lineNumbersLeft = i.lineNumbersLeft;
    this._lineNumbersWidth = i.lineNumbersWidth;
  }
  dispose() {
  ...
  onCursorStateChanged(e) {
    ...
    let i = false;
    if (this._activeLineNumber !== t.lineNumber) {
      this._activeLineNumber = t.lineNumber;
      i = true;
    }
  ...
  _getLineRenderLineNumber(e) {
    ...
    if (this._renderLineNumbers === 2) {
      ...
      if (r === 0) {
        return "<span class=\"relative-current-line-number\">" + i + "</span>";
      } else {
        return String(r);
      }
    }
  ...
  prepareRender(e) {
    ...
    const t = xv ? this._lineHeight % 2 === 0 ? " lh-even" : " lh-odd" : "";
    const i = e.visibleRange.startLineNumber;
    const r = e.visibleRange.endLineNumber;
    const s = this._context.viewModel.getDecorationsInViewport(e.visibleRange).filter(u => !!u.options.lineNumberClassName);
    s.sort((u, d) => Zt.compareRangesUsingEnds(u.range, d.range));
    let o = 0;
    const a = this._context.viewModel.getLineCount();
    const l = [];
    for (let u = i; u <= r; u++) {
      const d = u - i;
      let m = this._getLineRenderLineNumber(u);
      let p = "";
      while (o < s.length && s[o].range.endLineNumber < u) {
        o++;
      }
      for (let g = o; g < s.length; g++) {
        ...
        if (f.startLineNumber <= u) {
          p += " " + A.lineNumberClassName;
        }
      }
      if (!m && !p) {
        l[d] = "";
      ...
      if (u === a && this._context.viewModel.getLineLength(u) === 0) {
        if (this._renderFinalNewline === "off") {
          m = "";
        }
        if (this._renderFinalNewline === "dimmed") {
          p += " dimmed-line-number";
        }
      }
      if (u === this._activeLineNumber) {
        p += " active-line-number";
      }
      l[d] = `<div class="${Iad.CLASS_NAME}${t}${p}" style="left:${this._lineNumbersLeft}px;width:${this._lineNumbersWidth}px;">${m ?? ""}</div>`;
    }
  ...
  render(e, t) {
    if (!this._renderResult) {
      return this._renderLineNumberFallback(t);
    }
    const i = t - e;
...
HI((n, e) => {
  const t = n.getColor(gmh);
  const i = n.getColor(Imh);
  if (i) {
    e.addRule(`.monaco-editor .line-numbers.dimmed-line-number { color: ${i}; }`);
  } else if (t) {
    e.addRule(`.monaco-editor .line-numbers.dimmed-line-number { color: ${t.transparent(0.4)}; }`);
  }
});
