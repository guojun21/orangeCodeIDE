...
pyh = class Pad extends yW {
  ...
  constructor(e) {
    ...
    this._readOnly = t.get(96);
    this._cursorBlinking = t.get(26);
    this._cursorStyle = t.get(147);
    this._cursorSmoothCaretAnimation = t.get(27);
    this._experimentalEditContextEnabled = t.get(157);
    this._selectionIsEmpty = true;
    this._isComposingInput = false;
    this._isVisible = false;
    this._primaryCursor = new rIc(this._context, g9e.Single);
    ...
    this._startCursorBlinkAnimation = new O$();
    this._cursorFlatBlinkInterval = new D5e();
  ...
  _onCursorPositionChanged(e, t, i) {
    ...
    this._primaryCursor.setPlurality(t.length ? g9e.MultiPrimary : g9e.Single);
    this._primaryCursor.onCursorPositionChanged(e, r);
    this._updateBlinking();
    if (this._secondaryCursors.length < t.length) {
      const s = t.length - this._secondaryCursors.length;
      for (let o = 0; o < s; o++) {
        const a = new rIc(this._context, g9e.MultiSecondary);
        this._domNode.domNode.insertBefore(a.getDomNode().domNode, this._primaryCursor.getDomNode().domNode.nextSibling);
        this._secondaryCursors.push(a);
      }
    ...
    for (let s = 0; s < t.length; s++) {
      this._secondaryCursors[s].onCursorPositionChanged(t[s], r);
    }
  }
  onCursorStateChanged(e) {
    const t = [];
    for (let r = 0, s = e.selections.length; r < s; r++) {
  ...
  onTokensChanged(e) {
    const t = i => {
      for (let r = 0, s = e.ranges.length; r < s; r++) {
  ...
  _getCursorBlinking() {
    if (this._isComposingInput && !this._experimentalEditContextEnabled || !this._editorHasFocus || this._cursorBlinking === 0) {
      return 0;
  ...
  _updateBlinking() {
    ...
    if (!t && !i) {
      if (e === 1) {
        ...
        }, Pad.BLINK_INTERVAL, As(this._domNode.domNode));
      } else {
  ...
  _getClassName() {
    ...
    switch (this._cursorStyle) {
      ...
      case hT.LineThin:
        ...
        break;
      case hT.BlockOutline:
        e += " cursor-block-outline-style";
        break;
      case hT.UnderlineThin:
    ...
    if (this._cursorSmoothCaretAnimation === "on" || this._cursorSmoothCaretAnimation === "explicit") {
      e += " cursor-smooth-caret-animation";
    }
    return e;
  }
  _show() {
...
HI((n, e) => {
  ...
  for (const i of t) {
    ...
    if (r) {
      ...
      s ||= r.opposite();
      e.addRule(`.monaco-editor .cursors-layer ${i.class} { background-color: ${r}; border-color: ${r}; color: ${s}; }`);
      if (Poe(n.type)) {
        e.addRule(`.monaco-editor .cursors-layer.has-selection ${i.class} { border-left: 1px solid ${s}; border-right: 1px solid ${s}; }`);
      }
    }
  }
});
