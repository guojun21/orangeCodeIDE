...
x2 = class RO {
  ...
  withMargin(e, t, i, r) {
    ...
    return new RO(this.left - s, this.top - a, this.right + o, this.bottom + l);
  }
  intersectVertical(e) {
    const t = Math.max(this.top, e.start);
    const i = Math.min(this.bottom, e.endExclusive);
    return new RO(this.left, t, this.right, Math.max(t, i));
  }
  intersectHorizontal(e) {
    const t = Math.max(this.left, e.start);
    const i = Math.min(this.right, e.endExclusive);
  ...
  intersect(e) {
    ...
    const i = Math.min(this.right, e.right);
    const r = Math.max(this.top, e.top);
    const s = Math.min(this.bottom, e.bottom);
    if (!(t > i) && !(r > s)) {
      return new RO(t, r, i, s);
    }
  }
  union(e) {
  ...
  moveToBeContainedIn(e) {
    ...
    if (s < e.top) {
      s = e.top;
    } else if (s + i > e.bottom) {
      s = e.bottom - i;
    }
    return new RO(r, s, r + t, s + i);
  }
  withWidth(e) {
