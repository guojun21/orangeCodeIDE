...
Sxf = class extends at {
  ...
  constructor(n, e, t, i) {
    ...
    this.menu = this._register(t.createMenu(n, i));
  ...
  updateActions() {
    ...
    this._primaryActions = n.primary;
    this._secondaryActions = n.secondary;
  ...
  updateSubmenus(n, e) {
    const t = new Ut();
    for (const i of n) {
      if (i instanceof h2 && !e[i.item.submenu.id]) {
        const r = e[i.item.submenu.id] = t.add(this.menuService.createMenu(i.item.submenu, this.contextKeyService));
        t.add(r.onDidChange(() => this.updateActions()));
        t.add(this.updateSubmenus(i.actions, e));
      }
    }
    return t;
  }
...
u1t = class extends at {
  constructor(e, t, i, r, s) {
    ...
    this._onDidChange = this._register(new Qe());
    this.onDidChange = this._onDidChange.event;
    this.menuActions = this._register(new Sxf(e, this.options, s, r));
