...
function Bfn(n){
  ...
  ").replace(/'/g,"&#39;
  ")}function W1t(n){return n.replace(/\\/g,"\\\\").replace(/`/g,"\\`").replace(/\$\{/g,"\\${")}function FU(n){return W1t(n)}function MNf(n){return`(function() {
    
	if (window.__cursorPlaceholderInjected) return;
	window.__cursorPlaceholderInjected = true;

	var style = document.createElement('style');
	style.textContent = \`
		* { margin: 0; padding: 0; box-sizing: border-box; }
		body {
			background: ${FU(n.background)};
		...
		.browser-welcome-icon {
			color: ${FU(n.descriptionForeground)};
			opacity: 0.6;
			margin-bottom: 24px;
		}
	...
	document.body.innerHTML = \`
		<div class="browser-welcome-screen">
			<div class="browser-welcome-icon">${wvu}</div>
			<div class="browser-welcome-title">Browser</div>
			<div class="browser-welcome-subtitle">Enter a URL above, or instruct the Agent to navigate and use the browser</div>
		</div>
	\`;
})()`
  }
  function Jay(n, e){
    switch(n){
      case"http":return{
        title:"This page isn&#39;t working",subtitle:Bfn(e),buttonLabel:"Reload",buttonAction:"reload"
      };
      case"dns":return{
        title:"This site can&#39;t be reached",subtitle:Bfn(e),buttonLabel:"Restart Browser",buttonAction:"restart"
      };
      case"connection":return{
        title:"Connection Failed",subtitle:`Connection was refused. ${Bfn(e)}`,buttonLabel:"Restart Browser",buttonAction:"restart"
      };
      case"network":return{
        title:"Connection Failed",subtitle:Bfn(e),buttonLabel:"Restart Browser",buttonAction:"restart"
      };
      default:{
        const t=n;
        return{
          title:"Connection Failed",subtitle:Bfn(e),buttonLabel:"Reload",buttonAction:"reload"
        }
      }
    }
  }
  function Avu(n, e){
    const{
      title:t,subtitle:i,buttonLabel:r,buttonAction:s
    }
    =Jay(e.errorKind, e.errorMessage), o=e.errorDetails?`
		<span class="browser-error-details-toggle-group">
			<span class="browser-error-details-separator">\xB7</span>
			<button class="browser-error-details-toggle">Show Details</button>
		</span>
	`:"", a=e.errorDetails?`
		<div class="browser-error-details-box" style="display: none;">
			<div class="browser-error-details-content">
				<div class="browser-error-details-label">${Bfn(e.errorDetails)}</div>
				<button class="browser-error-copy-button" title="Copy error details">
					<span class="copy-icon">${FNf}</span>
					<span class="check-icon" style="display:none">${ONf}</span>
				</button>
			</div>
		</div>
	`:"", l=W1t(t), u=W1t(i), d=W1t(o), m=W1t(a), p=W1t(r), g=W1t(s);
    return`(function() {
	if (window.__cursorPlaceholderInjected) return;
	window.__cursorPlaceholderInjected = true;
	...
	style.textContent = \`
		* { margin: 0; padding: 0; box-sizing: border-box; }
		...
		.browser-error-details-toggle-group {
			display: inline-flex;
			align-items: baseline;
			gap: 6px;
		}
		.browser-error-details-separator {
			color: ${FU(n.textSecondary)};
			opacity: 0.5;
		}
		.browser-error-details-toggle {
			padding: 0;
		...
		.browser-error-details-toggle:focus-visible {
			outline: 1px solid ${FU(n.foreground)};
			outline-offset: -1px;
		}
		.browser-error-details-toggle:hover { opacity: 0.7; }
		.browser-error-details-box {
			width: 100%;
			max-width: 480px;
			background: color-mix(in srgb, ${FU(n.inputBackground)} 40%, transparent);
			border: 1px solid color-mix(in srgb, ${FU(n.strokeSecondary)} 40%, transparent);
			border-radius: 6px;
			padding: 6px 8px;
		...
		.browser-error-details-content {
			display: flex;
			align-items: center;
			gap: 8px;
		}
		.browser-error-details-label {
			flex: 1;
			min-width: 0;
			font-size: 11px;
			color: ${FU(n.textSecondary)};
		...
		.browser-error-copy-button:hover {
			opacity: 1;
			background: ${FU(n.buttonHoverBackground)};
		}
		.browser-error-copy-button .copy-icon {
			display: inline-flex;
			align-items: center;
			justify-content: center;
			width: 14px;
		...
		.browser-error-copy-button .check-icon {
			font-size: 12px;
			line-height: 1;
		}
		.browser-error-restart-button {
			padding: 4px 8px;
			background: transparent;
			color: ${FU(n.foreground)};
			border: 1px solid ${FU(n.strokeSecondary)};
			border-radius: 4px;
			cursor: pointer;
			font-size: 12px;
			font-family: inherit;
		...
		.browser-error-restart-button:hover {
			background: ${FU(n.buttonHoverBackground)};
		}
	\`;
	document.head.appendChild(style);
	document.body.innerHTML = \`
		<div class="browser-error-overlay">
			<div class="browser-error-content">
				<div class="browser-error-icon">${wvu.replace(/"/g,"'")}</div>
				<div class="browser-error-title">${l}</div>
				<div class="browser-error-subtitle">${u}${d}</div>
				${m}
				<button class="browser-error-restart-button">${p}</button>
			</div>
		</div>
	\`;

	var detailsToggle = document.querySelector('.browser-error-details-toggle');
	if (detailsToggle) {
		detailsToggle.addEventListener('click', function() {
			var box = document.querySelector('.browser-error-details-box');
			var btn = this;
			if (!box) { return; }
			if (box.style.display === 'none') { box.style.display = 'block'; btn.textContent = 'Hide Details'; }
			else { box.style.display = 'none'; btn.textContent = 'Show Details'; }
		});
	}

	var copyBtn = document.querySelector('.browser-error-copy-button');
	if (copyBtn) {
		copyBtn.addEventListener('click', function() {
			if (window.cursorBrowser) { window.cursorBrowser.send('browser-error-action', {action: 'copy-details'}); }
			this.title = 'Copied!';
			this.querySelector('.copy-icon').style.display = 'none';
			this.querySelector('.check-icon').style.display = 'inline';
			var btn = this;
			setTimeout(function() {
				btn.querySelector('.copy-icon').style.display = 'inline';
	...
	var restartBtn = document.querySelector('.browser-error-restart-button');
	if (restartBtn) {
		restartBtn.addEventListener('click', function() {
			if (window.cursorBrowser) { window.cursorBrowser.send('browser-error-action', {action: '${g}'}); }
		});
	}
})()`
  }
  function yvu(){
    const n=$c(), e=n.getComputedStyle(n.document.documentElement), t=(i, r)=>e.getPropertyValue(i).trim()||r;
    return{
      background:t("--vscode-editor-background","#1e1e1e"),foreground:t("--vscode-foreground","#cccccc"),descriptionForeground:t("--vscode-descriptionForeground","#8b8b8b"),textSecondary:t("--cursor-text-secondary","#8b8b8b"),strokeSecondary:t("--cursor-stroke-secondary","#3c3c3c"),inputBackground:t("--vscode-input-background","#2d2d2d"),buttonHoverBackground:t("--vscode-button-secondaryHoverBackground","#3c3c3c")
    }
  }
  var wvu, FNf, ONf, Gay=