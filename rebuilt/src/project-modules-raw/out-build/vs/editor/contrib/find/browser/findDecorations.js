...
ts(), xw(), bv(), Nl(), Io(), edn=class BTe{
  ...
  getCurrentMatchesPosition(e){
    ...
    for(const i of t){
      const r=i.options;
      if(r===BTe._FIND_MATCH_DECORATION||r===BTe._CURRENT_FIND_MATCH_DECORATION)return this._getDecorationIndex(i.id)
    }
    return 0
  }
  setCurrentFindMatch(e){
    let t=null, i=0;
    if(e)for(let r=0, s=this._decorations.length;
    r<s;
    r++){
    ...
    return(this._highlightedDecorationId!==null||t!==null)&&this._editor.changeDecorations(r=>{
      if(this._highlightedDecorationId!==null&&(r.changeDecorationOptions(this._highlightedDecorationId,BTe._FIND_MATCH_DECORATION),this._highlightedDecorationId=null),t!==null&&(this._highlightedDecorationId=t,r.changeDecorationOptions(this._highlightedDecorationId,BTe._CURRENT_FIND_MATCH_DECORATION)),this._rangeHighlightDecorationId!==null&&(r.removeDecoration(this._rangeHighlightDecorationId),this._rangeHighlightDecorationId=null),t!==null){
  ...
  set(e, t){
    this._editor.changeDecorations(i=>{
      ...
      if(e.length>1e3){
        ...
        const a=this._editor.getModel().getLineCount(),u=this._editor.getLayoutInfo().height/a,d=Math.max(2,Math.ceil(3/u));
        let m=e[0].range.startLineNumber,p=e[0].range.endLineNumber;
        for(let g=1,f=e.length;
        g<f;
        g++){
          const A=e[g].range;
          p+d>=A.startLineNumber?A.endLineNumber>p&&(p=A.endLineNumber):(s.push({
            range:new Zt(m,1,p,1),options:BTe._FIND_MATCH_ONLY_OVERVIEW_DECORATION
          }),m=A.startLineNumber,p=A.endLineNumber)
        }
        s.push({
          range:new Zt(m,1,p,1),options:BTe._FIND_MATCH_ONLY_OVERVIEW_DECORATION
        })
      ...
      a<l;
      a++)o[a]={
        range:e[a].range,options:r
      };
      this._decorations=i.deltaDecorations(this._decorations,o),this._overviewRulerApproximateDecorations=i.deltaDecorations(this._overviewRulerApproximateDecorations,s),this._rangeHighlightDecorationId&&(i.removeDecoration(this._rangeHighlightDecorationId),this._rangeHighlightDecorationId=null),this._findScopeDecorationIds.length&&(this._findScopeDecorationIds.forEach(a=>i.removeDecoration(a)),this._findScopeDecorationIds=[]),t?.length&&(this._findScopeDecorationIds=t.map(a=>i.addDecoration(a,BTe._FIND_SCOPE_DECORATION)))
    })
  }
  matchBeforePosition(e){
    if(this._decorations.length===0)return null;
    for(let t=this._decorations.length-1;
    t>=0;
    t--){
      const i=this._decorations[t],r=this._editor.getModel().getDecorationRange(i);
      if(!(!r||r.endLineNumber>e.lineNumber)){
        if(r.endLineNumber<e.lineNumber)return r;
        if(!(r.endColumn>e.column))return r
      }
    }
  ...
  matchAfterPosition(e){
    if(this._decorations.length===0)return null;
    for(let t=0, i=this._decorations.length;
    t<i;
    t++){
      const r=this._decorations[t],s=this._editor.getModel().getDecorationRange(r);
      if(!(!s||s.startLineNumber<e.lineNumber)){
        if(s.startLineNumber>e.lineNumber)return s;
        if(!(s.startColumn<e.column))return s
      }
    }
  ...
  static{
    this._CURRENT_FIND_MATCH_DECORATION=Zh.register({
      description:"current-find-match",stickiness:1,zIndex:13,className:"currentFindMatch",inlineClassName:"currentFindMatchInline",showIfCollapsed:!0,overviewRuler:{
        color:kC(W5e),position:Tx.Center
      },minimap:{
        color:kC(rOt),position:1
      }
    })
  }
  static{
    this._FIND_MATCH_DECORATION=Zh.register({
      description:"find-match",stickiness:1,zIndex:10,className:"findMatch",inlineClassName:"findMatchInline",showIfCollapsed:!0,overviewRuler:{
        color:kC(W5e),position:Tx.Center
      },minimap:{
        color:kC(rOt),position:1
      }
  ...
  static{
    this._FIND_MATCH_NO_OVERVIEW_DECORATION=Zh.register({
      description:"find-match-no-overview",stickiness:1,className:"findMatch",showIfCollapsed:!0
    })
  }
  static{
    this._FIND_MATCH_ONLY_OVERVIEW_DECORATION=Zh.register({
      description:"find-match-only-overview",stickiness:1,overviewRuler:{
        color:kC(W5e),position:Tx.Center
      }
    })
  }
  static{
    this._RANGE_HIGHLIGHT_DECORATION=Zh.register({
      description:"find-range-highlight",stickiness:1,className:"rangeHighlight",isWholeLine:!0
    })
  }
  static{
    this._FIND_SCOPE_DECORATION=Zh.register({
      description:"find-scope",className:"findScope",isWholeLine:!0
    })
  }
}
}
