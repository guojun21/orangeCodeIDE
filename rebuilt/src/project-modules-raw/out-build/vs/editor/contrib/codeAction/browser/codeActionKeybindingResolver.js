...
QY(), L0(), mve(), BW(), ka(), n$o=class{
  ...
  getResolver(){
    const e=new Ob(()=>this.keybindingService.getKeybindings().filter(t=>l5c.codeActionCommands.indexOf(t.command)>=0).filter(t=>t.resolvedKeybinding).map(t=>{
      let i=t.commandArgs;
      return t.command===d9o?i={
        kind:FA.SourceOrganizeImports.value
      }
      :t.command===h9o&&(i={
        kind:FA.SourceFixAll.value
      }),{
        resolvedKeybinding:t.resolvedKeybinding,...e5n.fromUser(i,{
          kind:p0.None,apply:"never"
        })
      }
    }));
    return t=>{
      if(t.kind)return this.bestKeybindingForCodeAction(t,e.value)?.resolvedKeybinding
    }
  }
  bestKeybindingForCodeAction(e, t){
    if(!e.kind)return;
    const i=new p0(e.kind);
    return t.filter(r=>r.kind.contains(i)).filter(r=>r.preferred?e.isPreferred:!0).reduceRight((r, s)=>r?r.kind.contains(s.kind)?s:r:s, void 0)
  }
