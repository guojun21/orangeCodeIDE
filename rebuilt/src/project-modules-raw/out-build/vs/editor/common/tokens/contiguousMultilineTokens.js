...
Vs(), Ql(), tl(), EVe(), Pgh(), Ix(), lxc=class KJb{
  static deserialize(e, t, i){
    const r=new Uint32Array(e.buffer), s=CY(e, t);
    t+=4;
    const o=CY(e, t);
    t+=4;
    const a=[];
    ...
    l++){
      const u=CY(e,t);
      t+=4,a.push(r.subarray(t/4,t/4+u/4)),t+=u
    }
    return i.push(new KJb(s, a)), t
  }
  get startLineNumber(){
  ...
  serialize(e, t){
    SY(e, this._startLineNumber, t), t+=4, SY(e, this._tokens.length, t), t+=4;
    for(let i=0;
    i<this._tokens.length;
    i++){
      const r=this._tokens[i];
      if(!(r instanceof Uint32Array))throw new Error("Not supported!");
      SY(e,r.byteLength,t),t+=4,e.set(new Uint8Array(r.buffer),t),t+=r.byteLength
    }
    return t
  }
  applyEdit(e, t){
    const[i, r]=Vbe(t);
    this._acceptDeleteRange(e), this._acceptInsertText(new ar(e.startLineNumber, e.startColumn), i, r)
  }
  _acceptDeleteRange(e){
    ...
    if(!(t>=this._tokens.length)){
      ...
      if(t>=0)if(this._tokens[t]=cz.deleteEnding(this._tokens[t],e.startColumn-1),i<this._tokens.length){
        const r=cz.deleteBeginning(this._tokens[i],e.endColumn-1);
        this._tokens[t]=cz.append(this._tokens[t],r),this._tokens.splice(t+1,i-t)
      }
      else this._tokens[t]=cz.append(this._tokens[t],null),this._tokens=this._tokens.slice(0,t+1);
      else{
        const r=-t;
        this._startLineNumber-=r,this._tokens[i]=cz.deleteBeginning(this._tokens[i],e.endColumn-1),this._tokens=this._tokens.slice(i)
      }
    }
  }
  _acceptInsertText(e, t, i){
    ...
    if(!(r>=this._tokens.length)){
      ...
      this._tokens[r]=cz.deleteEnding(this._tokens[r],e.column-1),this._tokens[r]=cz.insert(this._tokens[r],e.column-1,i),this._insertLines(e.lineNumber,t)
    }
  }
  _insertLines(e, t){
    if(t===0)return;
