...
function iMA(n){
  if(!ega[n]||Upa()==null)return null;
  const e=document.cookie.split(";");
  for(const t of e){
    const[i, r]=t.trim().split("=");
    if(i===f2g(n))return decodeURIComponent(r)
  }
  return null
}
function g2g(n, e){
  if(!ega[e]||!document)return;
  const t=new Date;
  t.setFullYear(t.getFullYear()+1), document.cookie=`${f2g(e)}=${encodeURIComponent(n)}; expires=${t.toUTCString()}; path=/`
}
function f2g(n){
  return`statsig.stable_id.${Mpa(n)}`
}
var Fhn, ega, Ktu, TNe, tga=