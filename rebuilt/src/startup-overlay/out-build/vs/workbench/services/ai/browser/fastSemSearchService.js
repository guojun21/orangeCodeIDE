...
oB();
zk();
a0u = xi("IFastSemSearchService");
WIa = class extends at {
  constructor(e, t, i, r, s, o, a, l, u) {
    super();
    this._cursorAuthenticationService = e;
    this.reactiveStorageService = t;
    this.productService = i;
    this.editorGroupsService = r;
    this._workspaceContextService = s;
    this._editorService = o;
    this._cursorCredsService = a;
    this.fastContextService = l;
    this._instantiationService = u;
    this.fastSearchClientPromise = this._instantiationService.createInstance(YS, {
      service: GIa
    });
  }
  async startFastSearch(e) {
    const t = await this.editorGroupsService.getGroups(1).map(a => a.editors.map(l => l.resource)).flat().filter(a => a !== undefined).map(a => this._workspaceContextService.asRelativePath(a)).map(a => new JIa({
      relativeWorkspacePath: a
    }));
    const s = (this.getLastActiveFileEditor()?.getControl()).getPosition() ?? new ar(1, 1);
    let o;
    try {
  ...
  getLastActiveFileEditor() {
    ...
    e?.input?.resource?.scheme !== _n.aiChat;
    return e;
  }
  ...
  async postProcessSearchResults(e) {
    const t = e.fileChunks.map(o => o.chunk?.relativeWorkspacePath).filter(o => o !== undefined);
    const i = new Wc();
    let r = new Map();
    await DIA(t, async o => {
  ...
  neverShowImports(e, t) {
    ...
    const s = i.filter(a => !r(a));
    let o = e;
    o.fileChunks = s;
    return o;
  }
  async internalPureFastSearch(e, t, i) {
    const r = await this.fastSearchClientPromise.get();
    const s = this.getAbortControllerFromCancellationToken(i);
    try {
...
WIa = __decorate([__param(0, wg), __param(1, ku), __param(2, za), __param(3, da), __param(4, Lr), __param(5, yi), __param(6, MJ), __param(7, wxe), __param(8, ln)], WIa);
Vi(a0u, WIa, 1);
