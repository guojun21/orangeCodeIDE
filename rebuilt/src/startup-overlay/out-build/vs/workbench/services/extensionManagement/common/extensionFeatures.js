...
Mme = xi("IExtensionFeaturesManagementService");
vtf = class {
  constructor() {
    this.extensionFeatures = new Map();
  }
  registerExtensionFeature(n) {
    if (this.extensionFeatures.has(n.id)) {
      throw new Error(`Extension feature with id '${n.id}' already exists`);
    }
    this.extensionFeatures.set(n.id, n);
    return {
