...
// Size: 1081 bytes
_s();
gif = class extends AbortController {
  constructor(n, e, t = 1, i) {
    super();
    this._logKey = n;
    this._structuredLogger = e;
    this._samplingRate = t;
    this._metadata = i;
    this.signal.addEventListener("abort", () => this._logAbortEvent());
  }
  abort(n) {
  ...
  _logAbortEvent() {
    if (Math.random() >= this._samplingRate) {
      return;
    }
    const n = {
...
bif = AbortController;
vif = class extends bif {
  abort(n) {
    if (QeA()) {
      const e = new Error().stack ?? "";
      if (V_c.some(t => e.includes(t))) {
        console.trace("[AbortController.abort]", n);
      }
    }
    super.abort(n);
  }
