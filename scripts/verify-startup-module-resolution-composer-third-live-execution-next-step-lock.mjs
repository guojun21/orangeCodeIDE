...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-third-live-execution-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-three-single-live-one-micro-batch-proven'
    ...
    decisionPinned:
      ...
      && report.decision?.composerLaneFreezeMayStartOnlyAfterThirdLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer third live freeze artifacts')
