...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-runtime-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-runtime-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'ninth-runtime-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-eight-single-live-one-micro-batch-ninth-no-op-proven'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-ninth-live-admission-plan'
    ...
    allowedNowScoped:
      allowedNow.includes('composer ninth dedicated no-op runtime artifacts')
      && allowedNow.includes('composer ninth live admission planning only'),
    blockedNowScoped:
      blockedNow.includes('composer ninth live execution before admission')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC9L execution until DC9 dedicated no-op runtime is frozen and locked')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion while composer continuation is still in ninth-candidate transition'),
  };
