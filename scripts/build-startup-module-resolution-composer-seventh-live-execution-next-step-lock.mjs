...
const SEVENTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-live-execution-next-step-lock.json');
...
function main() {
  const freezeReview = readJson(SEVENTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH);

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'composer-seventh-live-execution-next-step-lock',
    sources: {
      seventhLiveExecutionFreezeReview: normalizePath(path.relative(ROOT, SEVENTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH)),
    },
    baseline: {
    ...
    decision: {
      ...
      composerLaneFreezeMayStartOnlyAfterSeventhLiveFreeze: true,
