...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-live-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-live-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    contractPinned:
      report.twentyfirstLiveContract?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerWakelockManager.js'
      && report.twentyfirstLiveContract?.waveId === 'DC24L'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyfourth-live-contract-next-step-lock'
