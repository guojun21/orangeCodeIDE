...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-live-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-live-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'sixth-live-contract-next-step-lock',
    baselinePinned:
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-sixth-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('composer sixth live contract artifacts')
      && allowedNow.includes('composer sixth live execution only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer sixth live batch expansion'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC6L live is executed and frozen')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion before DC6L live outcome is frozen'),
  };
