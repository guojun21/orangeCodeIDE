...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-runtime-freeze-review.json');
...
function main() {
  ...
  const runtimeResult = eighteenthRuntimeGate.runtimeState?.resolution?.results?.[0]?.result ?? null;
  const diagnostics = eighteenthRuntimeGate.runtimeState?.resolution?.diagnostics ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eighteenth-runtime-freeze-review',
    sources: {
    ...
    eighteenthNoOp: {
      moduleId: eighteenthAdmission.approvedCandidate?.moduleId ?? null,
      waveId: eighteenthRuntimeGate.expectedWaveId ?? null,
      mode: eighteenthRuntimeGate.runtimeState?.resolution?.mode ?? null,
      canaryCount: eighteenthRuntimeGate.runtimeState?.resolution?.canaryCount ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-eleven-single-live-one-micro-batch-eighteenth-no-op-proven',
      nextApprovedStep: 'composer-eighteenth-runtime-next-step-lock',
      eighteenthNoOpProvenOnly: true,
      eighteenthLiveStillBlocked: true,
    ...
    blockedNow: [
      'composer eighteenth live execution before admission',
