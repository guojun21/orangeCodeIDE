...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-thirteenth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-live-admission-plan.json');
...
function main() {
  const thirteenthRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const thirteenthRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const thirteenthRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const thirteenthAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = thirteenthAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = thirteenthAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = thirteenthAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'thirteenth-live-admission-plan',
    sources: {
      thirteenthRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      thirteenthRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      thirteenthRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      thirteenthAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerAgent.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC13 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerAgent.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      thirteenthNoOpPassed: thirteenthRuntimeGate.passed === true,
      thirteenthNoOpWaveId: thirteenthRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: thirteenthRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: thirteenthRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC13L live contract',
      'run composer-thirteenth export-delta gate',
      'run composer-thirteenth fallback preflight',
      'run composer-thirteenth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
