...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourteenth-live-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourteenth-live-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'fourteenth-live-next-step-lock',
    baselinePinned:
    ...
    decisionPinned:
      ...
      && report.decision?.composerFourteenthRunnerUpSwitchBlocked === true
      && report.decision?.reviewChangesLaneStillFrozen === true
      && report.decision?.broadBrowserStillHeld === true
      && report.decision?.crossLaneExpansionBlocked === true
      && report.decision?.renameOnMainline === false,
    allowedNowPinned:
      allowedNow.includes('composer fourteenth live admission artifacts')
      && allowedNow.includes('composer fourteenth live contract planning only'),
    blockedNowPinned:
      ...
      && blockedNow.includes('composer fourteenth live execution before contract'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC14L live is executed and frozen')
      && stopConditions.includes('do not switch to composer runner-ups before DC14L live outcome is frozen'),
  };
