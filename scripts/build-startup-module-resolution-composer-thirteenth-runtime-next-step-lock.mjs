...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'thirteenth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-thirteenth-live-admission-plan',
    ...
    allowedNow: [
      'composer thirteenth dedicated no-op runtime artifacts',
      'composer thirteenth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC13L execution until DC13 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in thirteenth-candidate transition',
    ],
  };
