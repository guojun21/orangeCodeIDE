...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twentyfifth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twentyfirstSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js'
      && report.baseline?.twentyfirstSingleLiveWaveId === 'DC25L',
    decisionPinned:
