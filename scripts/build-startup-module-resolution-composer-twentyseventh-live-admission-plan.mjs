...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentyseventh-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-live-admission-plan.json');
...
function main() {
  ...
  const report = {
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerViews.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC27 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerViews.js',
      ],
    ...
    requiredBeforeLive: [
      'build DC27L live contract',
      'run composer-twentyseventh export-delta gate',
      'run composer-twentyseventh fallback preflight',
      'run composer-twentyseventh sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
    ...
    decision: {
      ...
      nextApprovedAction: 'composer-twentyseventh-live-contract',
      nextApprovedWaveId: 'DC27L',
