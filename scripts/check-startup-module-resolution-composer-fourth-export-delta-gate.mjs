...
const SOURCE_PATH = path.join(
  ...
  'composer',
  'browser',
  'composerFileChangeHandlerTypes.js',
);
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    assignmentShapePinned:
      sourceSummary.normalizedBody === 'Wt(); iit = xi("composerFileChangeHandlerService");'
      && inputSummary.normalizedBody === 'Wt(); iit = xi("composerFileChangeHandlerService");'
      && JSON.stringify(sourceSummary.topLevelCalls) === JSON.stringify(['Wt', 'xi'])
      && JSON.stringify(inputSummary.topLevelCalls) === JSON.stringify(['Wt', 'xi']),
  };
