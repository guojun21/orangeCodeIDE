...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-runtime-next-step-lock.json');
const ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-runtime-freeze-review.json');
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentysecond-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-live-admission-plan.json');
...
function main() {
  ...
  const report = {
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'browserScreenshotService.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC22 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than browserScreenshotService.js',
      ],
    ...
    requiredBeforeLive: [
      'build DC22L live contract',
      'run composer-twentysecond export-delta gate',
      'run composer-twentysecond fallback preflight',
      'run composer-twentysecond sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
    ...
    decision: {
      ...
      nextApprovedAction: 'composer-twentysecond-live-contract',
      nextApprovedWaveId: 'DC22L',
