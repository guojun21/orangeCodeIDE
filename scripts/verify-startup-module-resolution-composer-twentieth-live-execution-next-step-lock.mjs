...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twentieth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twentiethSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerStorageService.js'
      && report.baseline?.twentiethSingleLiveWaveId === 'DC20L',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer twentieth live freeze artifacts')
