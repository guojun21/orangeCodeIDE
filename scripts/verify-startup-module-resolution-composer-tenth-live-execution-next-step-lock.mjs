...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-tenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-tenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-tenth-live-execution-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-ten-single-live-one-micro-batch-proven'
    ...
    decisionPinned:
      ...
      && report.decision?.composerLaneFreezeMayStartOnlyAfterTenthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer tenth live freeze artifacts')
