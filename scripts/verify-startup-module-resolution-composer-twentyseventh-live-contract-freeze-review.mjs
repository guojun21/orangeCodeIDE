...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-live-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-live-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    contractPinned:
      report.twentyfirstLiveContract?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerViews.js'
      && report.twentyfirstLiveContract?.waveId === 'DC27L'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyseventh-live-contract-next-step-lock'
