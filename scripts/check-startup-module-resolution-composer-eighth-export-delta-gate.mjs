...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    viewStoreTokenPinned: sourceSummary.normalizedBody.includes('xi("browserViewStore")')
      && inputSummary.normalizedBody.includes('xi("browserViewStore")'),
    moduleShapePinned:
      sourceSummary.normalizedBody === 'Wt(); BC = xi("browserViewStore");'
      && inputSummary.normalizedBody === 'Wt(); BC = xi("browserViewStore");',
  };
