...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifteenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifteenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-fifteenth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.fifteenthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/bubbleComposerDataHandle.js'
      && report.baseline?.fifteenthSingleLiveWaveId === 'DC15L',
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-lane-freeze-review'
      && report.decision?.composerLaneFreezeMayStartOnlyAfterFifteenthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer fifteenth live freeze artifacts')
