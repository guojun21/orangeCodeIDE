...
const PROFILE = 'always-local-spike';
const RESULT_PATH = path.join(ROOT, 'mapped', 'always-local-spike-check.json');
const PHASE = 'rebuilt';
const BUILT_PATH = path.join(ROOT, 'recovered', PHASE, 'built', 'cursor-always-local-main.js');
const RUNTIME_PATH = path.join(ROOT, 'recovered', PHASE, 'runtime-app', 'extensions', 'cursor-always-local', 'dist', 'main.js');
...
try {
  ...
  const exthostLogPath = await waitForCondition(async () => {
    ...
    if (text.includes('Extension activated success: anysphere.cursor-always-local')) {
      return candidate;
    }

    if (exitCode !== null || signalCode !== null) {
      throw new Error(`runtime exited before always-local activation (code=${exitCode}, signal=${signalCode})`);
    }

    return null;
  }, {
    timeoutMs: 30000,
    description: 'always-local activation success logged',
  });

  const exthostLogText = fs.readFileSync(exthostLogPath, 'utf8');
  const outputLogPath = findLatestLog(
    userDataDir,
    'logs/*/window*/output_logging_*/*-Cursor-always-local debug logs.log'
  );
  ...
  const checks = [
    ...
    },
    {
      ...
      passed: exthostLogText.includes('Extension activated success: anysphere.cursor-always-local'),
    ...
    },
    {
      ...
      passed:
        !!outputLogPath &&
        outputLogText.includes('cursor-always-local extension activating') &&
        outputLogText.includes('cursor-always-local extension activated'),
      detail: { outputLogPath },
      advisory: true,
    },
    {
      id: 'exthost-log-present',
      passed: !!exthostLogPath,
      detail: { exthostLogPath },
    },
    {
      id: 'stderr-empty',
