...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifteenth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'fifteenth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-fifteenth-live-admission-plan',
    ...
    allowedNow: [
      'composer fifteenth dedicated no-op runtime artifacts',
      'composer fifteenth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC15L execution until DC15 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in fifteenth-candidate transition',
    ],
  };
