...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-admission.json');
...
function main() {
  ...
  const moduleId = 'out-build/vs/workbench/contrib/composer/browser/worktreeSetupRunner.js';
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/worktreeSetupRunner.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/worktreeSetupRunner.js';

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eighteenth-admission',
    sources: {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'worktreeSetupRunner.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places worktreeSetupRunner.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    blockedNow: [
      'composer eighteenth live execution',
    ...
    decision: {
      ...
      approvedWaveId: 'DC18A',
      nextApprovedAction: 'composer-eighteenth-dedicated-no-op-runtime-plan',
