...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-fourth-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    constantAssignmentShape:
      sourceSummary.topLevelCalls.length === 0
      && inputSummary.topLevelCalls.length === 0
      && sourceSummary.normalizedBody.startsWith('wCu = [')
      && inputSummary.normalizedBody.startsWith('wCu = ['),
  };
