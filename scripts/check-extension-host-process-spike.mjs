...
const PHASE = 'rebuilt';
const PROFILE = 'extension-host-process-spike';
const RESULT_PATH = path.join(ROOT, 'mapped', 'extension-host-process-spike-check.json');
const MARKER_OUTPUT_PATH = path.join(ROOT, 'mapped', 'extension-host-process-spike-marker.json');
const BUILT_PATH = path.join(ROOT, 'recovered', PHASE, 'built', 'extension-host-process-main.js');
const RUNTIME_PATH = path.join(ROOT, 'recovered', PHASE, 'runtime-app', 'out', 'vs', 'workbench', 'api', 'node', 'extensionHostProcess.js');
...
try {
  marker = await waitForCondition(async () => {
    if (!fs.existsSync(MARKER_OUTPUT_PATH)) {
      if (exitCode !== null || signalCode !== null) {
        throw new Error(`extension host process exited before marker matured (code=${exitCode}, signal=${signalCode})`);
      }
      return null;
    }

    const value = JSON.parse(fs.readFileSync(MARKER_OUTPUT_PATH, 'utf8'));
    if (value?.source === 'rebuilt/src/vs/workbench/api/node/extensionHostProcess/index.js' && typeof value?.importResolvedAt === 'string') {
      return value;
    }
    ...
    timeoutMs: 15000,
    description: 'extension host process marker file written',
  });
  ...
  checks = [
    ...
    {
      id: 'marker-source',
      passed: marker?.source === 'rebuilt/src/vs/workbench/api/node/extensionHostProcess/index.js',
      detail: marker ?? null,
    },
    {
      id: 'import-resolved',
      passed: typeof marker?.importResolvedAt === 'string' && Array.isArray(marker?.originalExportKeys) && marker.originalExportKeys.length > 0,
      detail: marker ?? null,
    },
    {
      id: 'argv-captured',
      passed: Array.isArray(marker?.argv),
      detail: marker ?? null,
    },
    {
      id: 'process-lifecycle-acceptable',
      passed: (exitCode === null && !child.killed) || exitCode === 0,
      detail: { exitCode, signalCode, killed: child.killed, acceptableExitCodes: [0, null] },
      advisory: true,
    },
    {
