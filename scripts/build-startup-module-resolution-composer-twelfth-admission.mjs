...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-admission.json');
...
function main() {
  ...
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/browserAnalytics.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/browserAnalytics.js';

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'twelfth-admission',
    sources: {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'browserAnalytics.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places browserAnalytics.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    decision: {
      ...
      approvedWaveId: 'DC12A',
      nextApprovedAction: 'composer-twelfth-dedicated-no-op-runtime-plan',
