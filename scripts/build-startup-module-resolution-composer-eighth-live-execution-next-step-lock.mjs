...
const EIGHTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-execution-next-step-lock.json');
...
function main() {
  const freezeReview = readJson(EIGHTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH);

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'composer-eighth-live-execution-next-step-lock',
    sources: {
      eighthLiveExecutionFreezeReview: normalizePath(path.relative(ROOT, EIGHTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH)),
    },
    baseline: {
    ...
    decision: {
      ...
      composerLaneFreezeMayStartOnlyAfterEighthLiveFreeze: true,
