...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composer.js'
      && report.twentyfirstNoOp?.waveId === 'DC26'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentysixth-runtime-next-step-lock'
