...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-seventeenth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-live-admission-plan.json');
...
function main() {
  const seventeenthRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const seventeenthRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const seventeenthRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const seventeenthAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = seventeenthAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = seventeenthAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = seventeenthAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'seventeenth-live-admission-plan',
    sources: {
      seventeenthRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      seventeenthRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      seventeenthRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      seventeenthAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerContextKeys.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC17 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerContextKeys.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      seventeenthNoOpPassed: seventeenthRuntimeGate.passed === true,
      seventeenthNoOpWaveId: seventeenthRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: seventeenthRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: seventeenthRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC17L live contract',
      'run composer-seventeenth export-delta gate',
      'run composer-seventeenth fallback preflight',
      'run composer-seventeenth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
