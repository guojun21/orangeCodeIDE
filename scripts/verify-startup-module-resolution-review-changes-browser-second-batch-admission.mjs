...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-admission.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-admission-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-second-batch-admission-plan',
    batchWavePinned:
      report.batchCandidate?.batchWaveId === 'DBRB2A'
      && report.batchCandidate?.batchSize === 4,
    batchModulesPinned: arrayEquals(report.batchCandidate?.selectedModules, EXPECTED_MODULES),
    selectedWavesPinned: arrayEquals(report.batchCandidate?.selectedWaves, ['DBR1L', 'DBR2L', 'DBR3L', 'DBR4L']),
    blockedCandidatesPinned:
      Array.isArray(report.batchCandidate?.blockedCandidates)
      && arrayEquals(report.batchCandidate.blockedCandidates, ['out-build/vs/workbench/contrib/reviewChanges/browser/utils/ciMessageUtils.js']),
    prerequisitesPinned:
      report.prerequisites?.secondBatchAdmissionUnlocked === true
      && report.prerequisites?.firstBatchLiveProven === true
      && report.prerequisites?.fourthSingleLiveProven === true
      ...
      && report.prerequisites?.fifthCandidateStillBlocked === true,
    ...
    blockedSurfacesPinned:
      ...
      && report.blockedSurfaces.includes('browser second batch live')
      && report.blockedSurfaces.includes('browser component/widget/template')
      && report.blockedSurfaces.includes('browser fifth candidate admission')
    ...
    minimumWinPinned:
      ...
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-second-batch-admission-verify.json'),
    scopeBoundaryPinned:
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedAction === 'browser-second-batch-contract-plan'
      && report.decision?.browserSecondBatchExecutionStillBlocked === true
      && report.decision?.browserFifthCandidateStillBlocked === true,
  };
