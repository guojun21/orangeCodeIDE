...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-execution-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-third-live-execution-freeze-review',
    baselinePinned:
    ...
    thirdLivePinned:
      report.proven?.thirdSingleLiveModuleId === moduleId
    ...
    decisionPinned:
      ...
      && report.decision?.laneState === 'contrib-composer-three-single-live-one-micro-batch-proven'
      && report.decision?.nextApprovedStep === 'composer-third-live-execution-next-step-lock'
