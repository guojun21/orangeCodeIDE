...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-broader-editor-widget-view-zone-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-broader-editor-widget-view-zone-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-broader-editor-widget-view-zone-next-step-lock',
    baselinePinned:
      ...
      && report.baseline.approvedSurface.length === 3,
    decisionPinned:
      ...
      && report.decision.approvedSurfaceOnly.length === 3
    ...
    allowedNowPinned:
      allowedNow.includes('browser broader editor/widget/view-zone admission freeze artifacts')
      && allowedNow.includes('browser broader editor/widget/view-zone contract planning only')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not execute broader editor/widget/view-zone live before a dedicated contract, preflight, and freeze chain is completed')
      && stopConditions.includes('do not widen broader editor/widget/view-zone admission beyond DBEWV1A before contract outcome is frozen')
