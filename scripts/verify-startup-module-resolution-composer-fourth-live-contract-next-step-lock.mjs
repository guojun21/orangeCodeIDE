...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-fourth-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('composer fourth live contract artifacts')
      && allowedNow.includes('composer fourth live execution only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer fourth live batch expansion'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC4L live is executed and frozen')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion before DC4L live outcome is frozen'),
  };
