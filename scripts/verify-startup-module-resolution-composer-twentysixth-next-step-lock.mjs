...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/composer.js'
      && report.baseline?.nextCandidateWaveId === 'DC26A',
    decisionPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC26L until DC26A dedicated no-op runtime gate completes')
