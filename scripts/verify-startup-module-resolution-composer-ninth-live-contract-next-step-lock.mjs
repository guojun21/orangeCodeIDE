...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-live-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-live-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'ninth-live-contract-next-step-lock',
    baselinePinned:
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-ninth-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('composer ninth live contract artifacts')
      && allowedNow.includes('composer ninth live execution only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer ninth live batch expansion'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC9L live is executed and frozen')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion before DC9L live outcome is frozen'),
  };
