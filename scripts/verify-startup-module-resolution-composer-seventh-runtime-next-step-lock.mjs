...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-runtime-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-runtime-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'seventh-runtime-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-six-single-live-one-micro-batch-seventh-no-op-proven'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-seventh-live-admission-plan'
    ...
    allowedNowScoped:
      allowedNow.includes('composer seventh dedicated no-op runtime artifacts')
      && allowedNow.includes('composer seventh live admission planning only'),
    blockedNowScoped:
      blockedNow.includes('composer seventh live execution before admission')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC7L execution until DC7 dedicated no-op runtime is frozen and locked')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion while composer continuation is still in seventh-candidate transition'),
  };
