...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerWakelockManager.js'
      && report.twentyfirstNoOp?.waveId === 'DC24'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyfourth-runtime-next-step-lock'
