...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-export-delta-gate.json');
...
function main() {
  ...
  const expectedBody = 'IVg = ".cursor/debug.log"; DVg = /^\\.cursor\\/debug-[a-zA-Z0-9-]+\\.log$/; BVg = 6;';
  ...
  const checks = {
    ...
    assignmentShapePinned:
      sourceSummary.normalizedBody === expectedBody
      && inputSummary.normalizedBody === expectedBody
      && JSON.stringify(sourceSummary.topLevelCalls) === JSON.stringify([])
      && JSON.stringify(inputSummary.topLevelCalls) === JSON.stringify([]),
  };
