...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-live-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-live-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    contractPinned:
      report.twentyfirstLiveContract?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/browserScreenshotService.js'
      && report.twentyfirstLiveContract?.waveId === 'DC22L'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentysecond-live-contract-next-step-lock'
