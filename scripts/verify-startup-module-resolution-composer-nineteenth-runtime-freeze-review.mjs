...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'nineteenth-runtime-freeze-review',
    baselinePinned:
    ...
    nineteenthNoOpPinned:
      report.nineteenthNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/asyncOperationRegistry.js'
      && report.nineteenthNoOp?.waveId === 'DC19'
      && report.nineteenthNoOp?.mode === 'no-op-observable'
      && report.nineteenthNoOp?.result === 'original-pass'
      && report.nineteenthNoOp?.fallbackReason === 'disabled-global'
      && report.nineteenthNoOp?.originalPassCount === 1
      && report.nineteenthNoOp?.overlayHitCount === 0
      && report.nineteenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-nineteenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-nineteenth-runtime-next-step-lock'
      && report.decision?.nineteenthNoOpProvenOnly === true
      && report.decision?.nineteenthLiveStillBlocked === true
