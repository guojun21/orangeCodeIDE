...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-live-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-live-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    contractPinned:
      report.twentyfirstLiveContract?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerMultiDiffContentProvider.js'
      && report.twentyfirstLiveContract?.waveId === 'DC23L'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentythird-live-contract-next-step-lock'
