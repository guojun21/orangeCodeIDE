...
const SIXTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-sixth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(SIXTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(SIXTH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    phase: 'composer-sixth-live-execution-freeze-review',
    sources: {
      sixthLiveContractFreezeReview: normalizePath(path.relative(ROOT, SIXTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      sixthLiveGate: normalizePath(path.relative(ROOT, SIXTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      sixthSingleLiveModuleId: moduleId,
      sixthSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-six-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-sixth-live-execution-next-step-lock',
