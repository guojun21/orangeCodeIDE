...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-tenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-tenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'tenth-runtime-freeze-review',
    baselinePinned:
    ...
    tenthNoOpPinned:
      ...
      && report.tenthNoOp?.result === 'original-pass'
      && report.tenthNoOp?.fallbackReason === 'disabled-global'
      && report.tenthNoOp?.originalPassCount === 1
      && report.tenthNoOp?.overlayHitCount === 0
      && report.tenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-nine-single-live-one-micro-batch-tenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-tenth-runtime-next-step-lock'
      && report.decision?.tenthNoOpProvenOnly === true
      && report.decision?.tenthLiveStillBlocked === true
