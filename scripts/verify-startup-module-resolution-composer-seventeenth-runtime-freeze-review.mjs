...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'seventeenth-runtime-freeze-review',
    baselinePinned:
    ...
    seventeenthNoOpPinned:
      report.seventeenthNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerContextKeys.js'
      && report.seventeenthNoOp?.waveId === 'DC17'
      && report.seventeenthNoOp?.mode === 'no-op-observable'
      && report.seventeenthNoOp?.result === 'original-pass'
      && report.seventeenthNoOp?.fallbackReason === 'disabled-global'
      && report.seventeenthNoOp?.originalPassCount === 1
      && report.seventeenthNoOp?.overlayHitCount === 0
      && report.seventeenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-seventeenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-seventeenth-runtime-next-step-lock'
      && report.decision?.seventeenthNoOpProvenOnly === true
      && report.decision?.seventeenthLiveStillBlocked === true
