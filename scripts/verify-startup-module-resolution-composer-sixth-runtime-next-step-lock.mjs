...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-runtime-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-runtime-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'sixth-runtime-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-five-single-live-one-micro-batch-sixth-no-op-proven'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-sixth-live-admission-plan'
    ...
    allowedNowScoped:
      allowedNow.includes('composer sixth dedicated no-op runtime artifacts')
      && allowedNow.includes('composer sixth live admission planning only'),
    blockedNowScoped:
      blockedNow.includes('composer sixth live execution before admission')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC6L execution until DC6 dedicated no-op runtime is frozen and locked')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion while composer continuation is still in sixth-candidate transition'),
  };
