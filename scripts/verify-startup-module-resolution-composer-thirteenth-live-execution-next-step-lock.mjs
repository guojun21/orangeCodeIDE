...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-thirteenth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.thirteenthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerAgent.js'
      && report.baseline?.thirteenthSingleLiveWaveId === 'DC13L',
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-lane-freeze-review'
      && report.decision?.composerLaneFreezeMayStartOnlyAfterThirteenthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer thirteenth live freeze artifacts')
