...
const CONTRACT_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-manager-module-resolution-contract.json');
const RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-manager-module-resolution-runtime-gate.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-manager-fallback-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    reasonCodePinned: Array.isArray(reasonCodes) && reasonCodes.includes('disabled-global'),
    runtimeObservedOriginalPass: result?.result === 'original-pass',
    runtimeObservedExpectedFallbackReason: result?.fallbackReason === 'disabled-global',
    noUnexpectedFallbackCount: (diagnostics?.counters?.fallbackCount ?? -1) === 0,
    noStickyDisableYet: (diagnostics?.counters?.stickyDisabledCount ?? -1) === 0,
  ...
  const report = {
    ...
    runtimeGatePath: normalizePath(path.relative(ROOT, RUNTIME_GATE_PATH)),
    checks,
    failedChecks,
