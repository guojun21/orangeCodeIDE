...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-fifth-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    initializerCallShape:
      sourceSummary.normalizedBody === 'Hl(); Yn(); Bc(); OJ();'
      && inputSummary.normalizedBody === 'Hl(); Yn(); Bc(); OJ();'
      && JSON.stringify(sourceSummary.topLevelCalls) === JSON.stringify(['Hl', 'Yn', 'Bc', 'OJ'])
      && JSON.stringify(inputSummary.topLevelCalls) === JSON.stringify(['Hl', 'Yn', 'Bc', 'OJ']),
  };
