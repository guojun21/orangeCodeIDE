...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifteenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifteenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'fifteenth-runtime-freeze-review',
    baselinePinned:
    ...
    fifteenthNoOpPinned:
      ...
      && report.fifteenthNoOp?.result === 'original-pass'
      && report.fifteenthNoOp?.fallbackReason === 'disabled-global'
      && report.fifteenthNoOp?.originalPassCount === 1
      && report.fifteenthNoOp?.overlayHitCount === 0
      && report.fifteenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-fifteenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-fifteenth-runtime-next-step-lock'
      && report.decision?.fifteenthNoOpProvenOnly === true
      && report.decision?.fifteenthLiveStillBlocked === true
