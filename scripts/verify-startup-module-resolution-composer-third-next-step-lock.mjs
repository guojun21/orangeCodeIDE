...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-two-single-live-one-micro-batch-third-candidate-planned'
      ...
      && report.baseline?.plannedModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js'
      && report.baseline?.plannedWaveId === 'DC3A',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer third candidate admission artifacts')
      && allowedNow.includes('composer third dedicated no-op runtime planning only'),
    blockedNowScoped:
      blockedNow.includes('composer third candidate live execution')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC3L until DC3A dedicated no-op runtime gate completes')
