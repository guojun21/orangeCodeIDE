...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    plannedCandidatePinned:
      report.plannedCandidate?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js'
      && report.plannedCandidate?.waveId === 'DC3A'
      && report.plannedCandidate?.riskClass === 'tiny-browser-filter-planning-only',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-two-single-live-one-micro-batch-third-candidate-planned'
      && report.decision?.nextApprovedStep === 'composer-third-next-step-lock'
