...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-live-execution-freeze-review-verify.json');
...
function main() {
  ...
  const thirdModuleId = 'out-build/vs/workbench/contrib/reviewChanges/browser/utils/diffMentionUtils.js';

  const checks = {
    lanePinned: report.lane === 'browser',
    phasePinned: report.phase === 'third-live-execution-freeze-review',
    baselinePinned:
    ...
    thirdLivePinned:
      report.proven?.thirdSingleLiveModuleId === thirdModuleId
      && report.proven?.thirdSingleLiveWaveId === 'DBR3L'
      && (report.proven?.overlayProbeIds ?? []).includes(thirdModuleId)
      && (report.proven?.factoryHitIds ?? []).includes(thirdModuleId)
      && (report.proven?.diagnostics?.overlayHitCount ?? 0) >= 1,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'three-single-live-proven-still-no-batch'
      && report.decision?.nextApprovedStep === 'browser-third-live-execution-next-step-lock'
