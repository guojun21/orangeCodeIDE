...
const COMPOSER_ELEVENTH_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'twentieth-next-step-lock',
    sources: {
    ...
    allowedNow: [
      'composer twentieth candidate admission artifacts',
      'composer twentieth dedicated no-op runtime planning only',
      'composer-specific diagnostics refinement',
    ],
    blockedNow: [
      'composer twentieth candidate live execution',
    ...
    stopConditions: [
      'do not discuss DC20L until DC20A dedicated no-op runtime gate completes',
