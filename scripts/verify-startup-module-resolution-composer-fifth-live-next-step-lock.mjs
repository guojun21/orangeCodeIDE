...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-four-single-live-one-micro-batch-fifth-live-admission-planned'
    ...
    decisionPinned:
      ...
      && report.decision?.composerFifthRunnerUpSwitchBlocked === true
      && report.decision?.reviewChangesLaneStillFrozen === true
      && report.decision?.broadBrowserStillHeld === true
      && report.decision?.crossLaneExpansionBlocked === true
      && report.decision?.renameOnMainline === false,
    allowedNowScoped:
      allowedNow.includes('composer fifth live admission artifacts')
      && allowedNow.includes('composer fifth live contract planning only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer fifth live execution before contract'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC5L live is executed and frozen')
      && stopConditions.includes('do not switch to composer runner-ups before DC5L live outcome is frozen'),
  };
