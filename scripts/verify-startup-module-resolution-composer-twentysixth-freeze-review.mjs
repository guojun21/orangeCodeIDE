...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    currentStatePinned:
      ...
      && report.currentState?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/composer.js'
      && report.currentState?.nextCandidateWaveId === 'DC26A',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-ten-single-live-one-micro-batch-twentyfirst-candidate-planned'
      && report.decision?.nextApprovedStep === 'composer-twentysixth-next-step-lock'
