...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'twelfth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-twelfth-live-admission-plan',
    ...
    allowedNow: [
      'composer twelfth dedicated no-op runtime artifacts',
      'composer twelfth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC12L execution until DC12 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in twelfth-candidate transition',
    ],
  };
