...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    assignmentShapePinned:
      sourceSummary.topLevelCalls.length === 0
      && inputSummary.topLevelCalls.length === 0
      && sourceSummary.normalizedBody === 'esu = "This model is not available in plan mode";'
      && inputSummary.normalizedBody === 'esu = "This model is not available in plan mode";',
  };
