...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-runtime-next-step-lock.json');
const ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-runtime-freeze-review.json');
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentyfifth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-live-admission-plan.json');
...
function main() {
  ...
  const report = {
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'browserPlaceholderPages.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC25 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than browserPlaceholderPages.js',
      ],
    ...
    requiredBeforeLive: [
      'build DC25L live contract',
      'run composer-twentyfifth export-delta gate',
      'run composer-twentyfifth fallback preflight',
      'run composer-twentyfifth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
    ...
    decision: {
      ...
      nextApprovedAction: 'composer-twentyfifth-live-contract',
      nextApprovedWaveId: 'DC25L',
