...
const NINTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-ninth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(NINTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(NINTH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    phase: 'composer-ninth-live-execution-freeze-review',
    sources: {
      ninthLiveContractFreezeReview: normalizePath(path.relative(ROOT, NINTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      ninthLiveGate: normalizePath(path.relative(ROOT, NINTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      ninthSingleLiveModuleId: moduleId,
      ninthSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-nine-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-ninth-live-execution-next-step-lock',
