...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-nineteenth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-live-admission-plan.json');
...
function main() {
  const nineteenthRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const nineteenthRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const nineteenthRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const nineteenthAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = nineteenthAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = nineteenthAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = nineteenthAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'nineteenth-live-admission-plan',
    sources: {
      nineteenthRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      nineteenthRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      nineteenthRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      nineteenthAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'asyncOperationRegistry.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC19 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than asyncOperationRegistry.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      nineteenthNoOpPassed: nineteenthRuntimeGate.passed === true,
      nineteenthNoOpWaveId: nineteenthRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: nineteenthRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: nineteenthRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC19L live contract',
      'run composer-nineteenth export-delta gate',
      'run composer-nineteenth fallback preflight',
      'run composer-nineteenth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
