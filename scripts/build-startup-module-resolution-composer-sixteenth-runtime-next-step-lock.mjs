...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixteenth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'sixteenth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-sixteenth-live-admission-plan',
    ...
    allowedNow: [
      'composer sixteenth dedicated no-op runtime artifacts',
      'composer sixteenth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC16L execution until DC16 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in sixteenth-candidate transition',
    ],
  };
