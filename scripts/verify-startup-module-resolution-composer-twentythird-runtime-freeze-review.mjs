...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'twentyfirst-runtime-freeze-review',
    baselinePinned:
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerMultiDiffContentProvider.js'
      && report.twentyfirstNoOp?.waveId === 'DC23'
      && report.twentyfirstNoOp?.mode === 'no-op-observable'
      && report.twentyfirstNoOp?.result === 'original-pass'
      && report.twentyfirstNoOp?.fallbackReason === 'disabled-global'
      && report.twentyfirstNoOp?.originalPassCount === 1
      && report.twentyfirstNoOp?.overlayHitCount === 0
      && report.twentyfirstNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-twentyfirst-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-twentythird-runtime-next-step-lock'
      && report.decision?.twentyfirstNoOpProvenOnly === true
      && report.decision?.twentyfirstLiveStillBlocked === true
