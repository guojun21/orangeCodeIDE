...
const LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-browser-heavier-ui-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-heavier-ui-live-execution-freeze-review.json');
...
function main() {
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-heavier-ui-live-execution-freeze-review',
    sources: {
      browserHeavierUiContractFreezeReview: normalizePath(path.relative(ROOT, CONTRACT_FREEZE_REVIEW_PATH)),
      browserHeavierUiLiveGate: normalizePath(path.relative(ROOT, LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      heavierUiWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'five-single-live-proven-first-and-second-and-third-batch-live-proven-component-widget-template-live-proven-heavier-ui-live-proven',
      nextApprovedStep: 'browser-heavier-ui-live-execution-next-step-lock',
      broaderUiDiscussionUnlocked: true,
