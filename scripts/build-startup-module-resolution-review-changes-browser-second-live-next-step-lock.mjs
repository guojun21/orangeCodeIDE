...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-live-next-step-lock.json');
...
function main() {
  const freezeReview = readJson(SECOND_LIVE_FREEZE_REVIEW_PATH);

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'browser',
    phase: 'second-live-next-step-lock',
    sources: {
    ...
    baseline: {
      ...
      secondLiveModuleId: freezeReview.secondLivePlan?.moduleId ?? null,
      secondLiveWaveId: freezeReview.secondLivePlan?.waveId ?? null,
    },
    decision: {
    ...
    allowedNow: [
      'browser second live admission artifacts',
      'browser second live contract planning only',
    ...
    blockedNow: [
      ...
      'rename-driven work',
      'browser second live execution before contract',
    ],
    stopConditions: [
