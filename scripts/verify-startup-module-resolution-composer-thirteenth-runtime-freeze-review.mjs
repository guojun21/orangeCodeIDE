...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'thirteenth-runtime-freeze-review',
    baselinePinned:
    ...
    thirteenthNoOpPinned:
      report.thirteenthNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerAgent.js'
      && report.thirteenthNoOp?.waveId === 'DC13'
      && report.thirteenthNoOp?.mode === 'no-op-observable'
      && report.thirteenthNoOp?.result === 'original-pass'
      && report.thirteenthNoOp?.fallbackReason === 'disabled-global'
      && report.thirteenthNoOp?.originalPassCount === 1
      && report.thirteenthNoOp?.overlayHitCount === 0
      && report.thirteenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-thirteenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-thirteenth-runtime-next-step-lock'
      && report.decision?.thirteenthNoOpProvenOnly === true
      && report.decision?.thirteenthLiveStillBlocked === true
      && report.decision?.reviewChangesLaneStillFrozen === true
      && report.decision?.broadBrowserStillHeld === true
      && report.decision?.crossLaneExpansionBlocked === true
      && report.decision?.renameOnMainline === false,
    blockedNowPinned:
      blockedNow.includes('composer thirteenth live execution before admission')
