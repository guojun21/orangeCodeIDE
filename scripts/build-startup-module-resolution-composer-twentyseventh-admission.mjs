...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-admission.json');
...
function main() {
  ...
  const moduleId = 'out-build/vs/workbench/contrib/composer/browser/composerViews.js';
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/composerViews.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/composerViews.js';
  ...
  const report = {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerViews.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places composerViews.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    decision: {
      ...
      approvedWaveId: 'DC27A',
      nextApprovedAction: 'composer-twentyseventh-dedicated-no-op-runtime-plan',
