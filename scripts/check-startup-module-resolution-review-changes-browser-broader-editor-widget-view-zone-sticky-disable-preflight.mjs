...
const HEAVIER_UI_LIVE_GATE_PATH = path.join(
  ROOT,
  'mapped',
  'contrib-review-changes-browser-heavier-ui-module-resolution-live-gate.json'
);
...
function main() {
  ...
  const heavierUiLiveGate = readJson(HEAVIER_UI_LIVE_GATE_PATH);
  ...
  const modules = selectedModules.map((moduleId) => {
    ...
    const checks = {
      ...
      notAlreadyInPriorSurface:
        !(Array.isArray(heavierUiLiveGate.enabledIds) && heavierUiLiveGate.enabledIds.includes(moduleId)),
    };
  ...
  const crossModuleChecks = {
    ...
    browserLaneStickyBaselineClean:
      (heavierUiLiveGate.runtimeState?.resolution?.diagnostics?.counters?.stickyDisabledCount ?? -1) === 0,
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-broader-editor-widget-view-zone-sticky-disable-preflight',
    contractPlanPath: normalizePath(path.relative(ROOT, CONTRACT_PLAN_PATH)),
    heavierUiLiveGatePath: normalizePath(path.relative(ROOT, HEAVIER_UI_LIVE_GATE_PATH)),
