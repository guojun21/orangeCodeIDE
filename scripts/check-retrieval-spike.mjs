...
const PROFILE = 'retrieval-spike';
const RESULT_PATH = path.join(ROOT, 'mapped', 'retrieval-spike-check.json');
const PHASE = 'rebuilt';
const BUILT_PATH = path.join(ROOT, 'recovered', PHASE, 'built', 'cursor-retrieval-main.js');
const RUNTIME_PATH = path.join(ROOT, 'recovered', PHASE, 'runtime-app', 'extensions', 'cursor-retrieval', 'dist', 'main.js');
...
try {
  ...
  const exthostLogPath = await waitForCondition(async () => {
    ...
    if (text.includes('Extension activated success: anysphere.cursor-retrieval')) {
      return candidate;
    }

    if (exitCode !== null || signalCode !== null) {
      throw new Error(`runtime exited before retrieval activation (code=${exitCode}, signal=${signalCode})`);
    }

    return null;
  }, {
    timeoutMs: 30000,
    description: 'retrieval activation success logged',
  });

  const exthostLogText = fs.readFileSync(exthostLogPath, 'utf8');
  const outputLogPath = findLatestLog(
    userDataDir,
    'logs/*/window*/output_logging_*/*-Cursor-retrieval debug logs.log'
  );
  ...
  const checks = [
    ...
    },
    {
      ...
      passed: exthostLogText.includes('Extension activated success: anysphere.cursor-retrieval'),
      detail: { exthostLogPath },
    },
    {
      id: 'debug-log-written',
      passed:
        !!outputLogPath &&
        outputLogText.includes('cursor-retrieval extension activating') &&
        outputLogText.includes('cursor-retrieval extension activated'),
