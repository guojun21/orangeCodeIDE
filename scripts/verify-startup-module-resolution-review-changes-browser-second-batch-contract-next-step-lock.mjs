...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-second-batch-contract-next-step-lock',
    baselinePinned:
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'browser-second-batch-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('browser second-batch contract artifacts')
      && allowedNow.includes('browser second-batch live execution only')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not widen browser second batch beyond DBR1L, DBR2L, DBR3L, and DBR4L before second-batch execution outcome is frozen')
      && stopConditions.includes('do not switch to ciMessageUtils.js before second-batch execution outcome is frozen')
