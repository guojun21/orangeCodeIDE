...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    thirdLivePlanPinned:
      report.thirdLivePlan?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js'
      && report.thirdLivePlan?.waveId === 'DC3L'
      && report.thirdLivePlan?.liveShape === 'single-module-live',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-two-single-live-one-micro-batch-third-live-admission-planned'
      && report.decision?.nextApprovedStep === 'composer-third-live-next-step-lock'
