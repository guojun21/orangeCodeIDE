...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-fifth-live-execution-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-five-single-live-one-micro-batch-proven'
    ...
    decisionPinned:
      ...
      && report.decision?.composerLaneFreezeMayStartOnlyAfterFifthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer fifth live freeze artifacts')
