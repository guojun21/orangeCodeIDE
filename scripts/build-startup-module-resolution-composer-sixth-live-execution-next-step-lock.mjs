...
const SIXTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixth-live-execution-next-step-lock.json');
...
function main() {
  const freezeReview = readJson(SIXTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH);

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'composer-sixth-live-execution-next-step-lock',
    sources: {
      sixthLiveExecutionFreezeReview: normalizePath(path.relative(ROOT, SIXTH_LIVE_EXECUTION_FREEZE_REVIEW_PATH)),
    },
    baseline: {
    ...
    decision: {
      ...
      composerLaneFreezeMayStartOnlyAfterSixthLiveFreeze: true,
