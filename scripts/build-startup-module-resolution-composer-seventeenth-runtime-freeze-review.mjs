...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-runtime-freeze-review.json');
...
function main() {
  ...
  const runtimeResult = seventeenthRuntimeGate.runtimeState?.resolution?.results?.[0]?.result ?? null;
  const diagnostics = seventeenthRuntimeGate.runtimeState?.resolution?.diagnostics ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'seventeenth-runtime-freeze-review',
    sources: {
    ...
    seventeenthNoOp: {
      moduleId: seventeenthAdmission.approvedCandidate?.moduleId ?? null,
      waveId: seventeenthRuntimeGate.expectedWaveId ?? null,
      mode: seventeenthRuntimeGate.runtimeState?.resolution?.mode ?? null,
      canaryCount: seventeenthRuntimeGate.runtimeState?.resolution?.canaryCount ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-eleven-single-live-one-micro-batch-seventeenth-no-op-proven',
      nextApprovedStep: 'composer-seventeenth-runtime-next-step-lock',
      seventeenthNoOpProvenOnly: true,
      seventeenthLiveStillBlocked: true,
