...
const TENTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-tenth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-tenth-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(TENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(TENTH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    phase: 'composer-tenth-live-execution-freeze-review',
    sources: {
      tenthLiveContractFreezeReview: normalizePath(path.relative(ROOT, TENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      tenthLiveGate: normalizePath(path.relative(ROOT, TENTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      tenthSingleLiveModuleId: moduleId,
      tenthSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-ten-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-tenth-live-execution-next-step-lock',
