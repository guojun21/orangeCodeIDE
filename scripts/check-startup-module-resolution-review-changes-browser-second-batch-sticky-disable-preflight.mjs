...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-sticky-disable-preflight.json');
...
function main() {
  ...
  const crossModuleChecks = {
    expectedModuleCountMatches: selectedModules.length === 4 && selectedWaves.length === 4,
    observedModuleCountMatches: modules.length === 4,
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-second-batch-sticky-disable-preflight',
