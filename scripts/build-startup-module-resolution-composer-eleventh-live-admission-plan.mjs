...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-eleventh-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-live-admission-plan.json');
...
function main() {
  const eleventhRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const eleventhRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const eleventhRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const eleventhAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = eleventhAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = eleventhAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = eleventhAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eleventh-live-admission-plan',
    sources: {
      eleventhRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      eleventhRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      eleventhRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      eleventhAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerDataCreation.js is the next narrow composer browser data-creation candidate after worktreeGate.js is frozen into the proven set',
        'it stays in a small browser data-shaping and serialization boundary rather than dropping directly into widget or render execution',
        'source and startup-loader input remain aligned after DC11 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, analytics, or agent coordination than composerDataCreation.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      eleventhNoOpPassed: eleventhRuntimeGate.passed === true,
      eleventhNoOpWaveId: eleventhRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: eleventhRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: eleventhRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC11L live contract',
      'run composer-eleventh export-delta gate',
      'run composer-eleventh fallback preflight',
      'run composer-eleventh sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
