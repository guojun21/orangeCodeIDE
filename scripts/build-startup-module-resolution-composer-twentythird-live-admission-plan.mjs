...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-runtime-next-step-lock.json');
const ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-runtime-freeze-review.json');
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentythird-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-live-admission-plan.json');
...
function main() {
  const twentyfirstRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const twentyfirstRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  ...
  const moduleId = twentyfirstAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = twentyfirstAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = twentyfirstAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'twentyfirst-live-admission-plan',
    sources: {
      twentyfirstRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerMultiDiffContentProvider.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC23 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerMultiDiffContentProvider.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      twentyfirstNoOpPassed: twentyfirstRuntimeGate.passed === true,
      twentyfirstNoOpWaveId: twentyfirstRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: twentyfirstRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: twentyfirstRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC23L live contract',
      'run composer-twentythird export-delta gate',
      'run composer-twentythird fallback preflight',
      'run composer-twentythird sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
    ...
    decision: {
      ...
      nextApprovedAction: 'composer-twentythird-live-contract',
      nextApprovedWaveId: 'DC23L',
