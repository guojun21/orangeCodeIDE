...
const COMPOSER_ELEVENTH_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'nineteenth-next-step-lock',
    sources: {
    ...
    allowedNow: [
      'composer nineteenth candidate admission artifacts',
      'composer nineteenth dedicated no-op runtime planning only',
      'composer-specific diagnostics refinement',
    ],
    blockedNow: [
      'composer nineteenth candidate live execution',
    ...
    stopConditions: [
      'do not discuss DC19L until DC19A dedicated no-op runtime gate completes',
