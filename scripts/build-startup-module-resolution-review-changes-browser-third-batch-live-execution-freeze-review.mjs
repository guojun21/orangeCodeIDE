...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-batch-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(THIRD_BATCH_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(THIRD_BATCH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-third-batch-live-execution-freeze-review',
    sources: {
      browserThirdBatchContractFreezeReview: normalizePath(path.relative(ROOT, THIRD_BATCH_CONTRACT_FREEZE_REVIEW_PATH)),
      browserThirdBatchLiveGate: normalizePath(path.relative(ROOT, THIRD_BATCH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      thirdBatchWaveId: liveGate.expectedWaveId ?? null,
      approvedThirdBatch: liveGate.enabledIds ?? [],
    ...
    decision: {
      laneFrozen: true,
      laneState: 'five-single-live-proven-first-and-second-and-third-batch-live-proven',
      nextApprovedStep: 'browser-third-batch-live-execution-next-step-lock',
      componentWidgetTemplateDiscussionUnlocked: true,
