...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-admission.json');
...
function main() {
  ...
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js';
  ...
  const report = {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'browserPlaceholderPages.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places browserPlaceholderPages.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    decision: {
      ...
      approvedWaveId: 'DC25A',
      nextApprovedAction: 'composer-twentyfifth-dedicated-no-op-runtime-plan',
