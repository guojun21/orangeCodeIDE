...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventeenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-seventeenth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.seventeenthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerContextKeys.js'
      && report.baseline?.seventeenthSingleLiveWaveId === 'DC17L',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer seventeenth live freeze artifacts')
