...
const CONTRACT_PLAN_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-batch-contract-plan.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-batch-sticky-disable-preflight.json');
...
function main() {
  ...
  const crossModuleChecks = {
    expectedModuleCountMatches: selectedModules.length === 3 && selectedWaves.length === 3,
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-batch-sticky-disable-preflight',
