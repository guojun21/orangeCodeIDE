...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'eighth-live-contract-next-step-lock',
    baselinePinned:
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-eighth-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('composer eighth live contract artifacts')
      && allowedNow.includes('composer eighth live execution only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer eighth live batch expansion'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC8L live is executed and frozen')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion before DC8L live outcome is frozen'),
  };
