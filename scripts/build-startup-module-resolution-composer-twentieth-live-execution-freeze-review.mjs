...
const ELEVENTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentieth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-live-execution-freeze-review.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'composer-twentieth-live-execution-freeze-review',
    sources: {
      twentiethLiveContractFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      twentiethLiveGate: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      twentiethSingleLiveModuleId: moduleId,
      twentiethSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      ...
      nextApprovedStep: 'composer-twentieth-live-execution-next-step-lock',
