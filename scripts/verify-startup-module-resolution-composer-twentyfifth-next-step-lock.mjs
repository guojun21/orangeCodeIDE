...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js'
      && report.baseline?.nextCandidateWaveId === 'DC25A',
    decisionPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC25L until DC25A dedicated no-op runtime gate completes')
