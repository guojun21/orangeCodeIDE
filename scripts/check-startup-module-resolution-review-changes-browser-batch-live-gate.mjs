...
const CONTRACT_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-browser-batch-module-resolution-live-contract.json');
const PATCH_REPORT_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-browser-batch-module-resolution-wrapper-patch.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'contrib-review-changes-browser-batch-module-resolution-live-gate.json');
const SHORT_PROBE_ROOT = path.join('/tmp', 'shc-rcbb');
...
function parseArgs(argv) {
  ...
  let expectedWaveId = 'DBRB1';
  let minEnabledCount = 3;
...
async function main() {
  ...
  console.log(`Browser batch live gate: ${normalizePath(path.relative(ROOT, outputPath))}`);
