...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-twentieth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-live-admission-plan.json');
...
function main() {
  const twentiethRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const twentiethRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const twentiethRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const twentiethAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = twentiethAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = twentiethAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = twentiethAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'twentieth-live-admission-plan',
    sources: {
      twentiethRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      twentiethRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      twentiethRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      twentiethAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerStorageService.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC20 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerStorageService.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      twentiethNoOpPassed: twentiethRuntimeGate.passed === true,
      twentiethNoOpWaveId: twentiethRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: twentiethRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: twentiethRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC20L live contract',
      'run composer-twentieth export-delta gate',
      'run composer-twentieth fallback preflight',
      'run composer-twentieth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
