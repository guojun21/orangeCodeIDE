...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js'
      && report.twentyfirstNoOp?.waveId === 'DC25'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyfifth-runtime-next-step-lock'
