...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-runtime-next-step-lock.json');
const ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-runtime-freeze-review.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-live-admission-plan.json');
...
function main() {
  ...
  const report = {
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerTextModelService.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC21 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than composerTextModelService.js',
      ],
    ...
    requiredBeforeLive: [
      'build DC21L live contract',
      'run composer-twentyfirst export-delta gate',
      'run composer-twentyfirst fallback preflight',
      'run composer-twentyfirst sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
