...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-admission.json');
...
function main() {
  ...
  const moduleId = 'out-build/vs/workbench/contrib/composer/browser/asyncOperationRegistry.js';
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/asyncOperationRegistry.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/asyncOperationRegistry.js';

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'nineteenth-admission',
    sources: {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'asyncOperationRegistry.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places asyncOperationRegistry.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    decision: {
      ...
      approvedWaveId: 'DC19A',
      nextApprovedAction: 'composer-nineteenth-dedicated-no-op-runtime-plan',
