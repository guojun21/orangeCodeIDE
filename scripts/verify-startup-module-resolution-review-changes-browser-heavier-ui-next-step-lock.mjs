...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-heavier-ui-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-heavier-ui-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-heavier-ui-next-step-lock',
    baselinePinned:
    ...
    allowedNowPinned:
      allowedNow.includes('browser heavier UI admission freeze artifacts')
      && allowedNow.includes('browser heavier UI contract planning only')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not execute heavier UI live before a dedicated contract, preflight, and freeze chain is completed')
