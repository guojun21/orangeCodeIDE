...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'twentieth-runtime-freeze-review',
    baselinePinned:
    ...
    twentiethNoOpPinned:
      report.twentiethNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerStorageService.js'
      && report.twentiethNoOp?.waveId === 'DC20'
      && report.twentiethNoOp?.mode === 'no-op-observable'
      && report.twentiethNoOp?.result === 'original-pass'
      && report.twentiethNoOp?.fallbackReason === 'disabled-global'
      && report.twentiethNoOp?.originalPassCount === 1
      && report.twentiethNoOp?.overlayHitCount === 0
      && report.twentiethNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-twentieth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-twentieth-runtime-next-step-lock'
      && report.decision?.twentiethNoOpProvenOnly === true
      && report.decision?.twentiethLiveStillBlocked === true
      && report.decision?.reviewChangesLaneStillFrozen === true
      && report.decision?.broadBrowserStillHeld === true
      && report.decision?.crossLaneExpansionBlocked === true
      && report.decision?.renameOnMainline === false,
    blockedNowPinned:
      blockedNow.includes('composer twentieth live execution before admission')
