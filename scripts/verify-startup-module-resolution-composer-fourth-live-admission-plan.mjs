...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-admission-plan.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-admission-plan-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    candidatePinned:
      report.candidate?.waveId === 'DC4L'
      && report.candidate?.moduleId === moduleId
      && report.candidate?.sourceFile === 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/composerFileChangeHandlerTypes.js'
      && report.candidate?.runtimeInputFile === 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/composerFileChangeHandlerTypes.js'
      && report.candidate?.liveShape === 'single-module-live',
    rationalePinned:
      Array.isArray(rationale)
      && rationale.includes('tiny browser-scope handler-types module that remains non-DOM and non-component at live-admission time')
      && rationale.includes('no manager/stateful resource-holder behavior is exposed in the recovered slice')
      && rationale.includes('source and startup-loader input remain aligned after DC4 dedicated no-op runtime freeze')
      && rationale.includes('runner-ups still carry broader chat-service, debug-log, or blob-store semantics than composerFileChangeHandlerTypes.js'),
    prerequisitesPinned:
      prerequisites.fourthNoOpPassed === true
      && prerequisites.fourthNoOpWaveId === 'DC4'
    ...
    requiredBeforeLivePresent:
      requiredBeforeLive.includes('build DC4L live contract')
      && requiredBeforeLive.includes('run composer-fourth export-delta gate')
      && requiredBeforeLive.includes('run composer-fourth fallback preflight')
      && requiredBeforeLive.includes('run composer-fourth sticky-disable preflight')
    ...
    stopConditionsPresent:
      ...
      && stopConditions.includes('DC4L single-live gate does not record overlay-hit')
      && stopConditions.includes('do not discuss wider composer batch while DC4 is only no-op proven'),
    runnerUpLockPinned:
    ...
    widerBatchEligibilityGatePinned:
      ...
      && widerBatchEligibilityGate.currentProofState === 'three-single-live-plus-first-micro-batch-plus-fourth-no-op'
      ...
      && widerBatchEligibilityGate.requiredMilestonesBeforeDiscussion.includes('DC4L single-live proven')
      && widerBatchEligibilityGate.requiredMilestonesBeforeDiscussion.includes('post-DC4L composer lane freeze review'),
    minimumWinPinned:
      minimumWin.definition === 'DC4L admission plan / freeze / next-step lock all verify green and nextApprovedAction uniquely locked to composer-fourth-live-contract'
      && Array.isArray(minimumWin.mustVerify)
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-fourth-live-admission-plan-verify.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-fourth-live-freeze-review-verify.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-fourth-live-next-step-lock-verify.json'),
    scopeBoundaryPinned:
      Array.isArray(scopeBoundary.allowOnly)
      && scopeBoundary.allowOnly.includes('DC4L admission artifacts')
      && scopeBoundary.allowOnly.includes('composer fourth live freeze artifacts')
      && scopeBoundary.allowOnly.includes('composer fourth live next-step lock artifacts')
    ...
    reachabilityAssessmentPinned:
      ...
      && reachabilityAssessment.safestShortPath === 'DC4L admission -> fourth-live freeze review -> fourth-live next-step lock -> composer-fourth-live-contract'
      && typeof reachabilityAssessment.biggestGap === 'string',
    followUpPriorityPinned:
      followUpPriority.next === 'composer-fourth-live-contract'
    ...
    nextActionPinned:
      ...
      && report.decision?.nextApprovedWaveId === 'DC4L'
