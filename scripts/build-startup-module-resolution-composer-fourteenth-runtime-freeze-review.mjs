...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourteenth-runtime-freeze-review.json');
...
function main() {
  ...
  const runtimeResult = fourteenthRuntimeGate.runtimeState?.resolution?.results?.[0]?.result ?? null;
  const diagnostics = fourteenthRuntimeGate.runtimeState?.resolution?.diagnostics ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'fourteenth-runtime-freeze-review',
    sources: {
    ...
    fourteenthNoOp: {
      moduleId: fourteenthAdmission.approvedCandidate?.moduleId ?? null,
      waveId: fourteenthRuntimeGate.expectedWaveId ?? null,
      mode: fourteenthRuntimeGate.runtimeState?.resolution?.mode ?? null,
      canaryCount: fourteenthRuntimeGate.runtimeState?.resolution?.canaryCount ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-eleven-single-live-one-micro-batch-fourteenth-no-op-proven',
      nextApprovedStep: 'composer-fourteenth-runtime-next-step-lock',
      fourteenthNoOpProvenOnly: true,
      fourteenthLiveStillBlocked: true,
