...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    freezeDecisionPinned:
      ...
      && report.decision?.laneState === 'contrib-composer-four-single-live-one-micro-batch-fifth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-fifth-runtime-next-step-lock'
    ...
    blockedNowScoped:
      ...
      && report.blockedNow.includes('composer fifth live execution before admission')
