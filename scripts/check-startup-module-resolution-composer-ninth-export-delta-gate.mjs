...
const SOURCE_PATH = path.join(
  ...
  'browser',
  'capabilities',
  'serializeToolformerBubbleData.js',
);
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-ninth-export-delta-gate.json');
...
function main() {
  ...
  const checks = {
    ...
    serializerCallShapePinned:
      sourceSummary.normalizedBody.includes('P5n();')
      && sourceSummary.normalizedBody.includes('ghn();')
      && inputSummary.normalizedBody.includes('P5n();')
      && inputSummary.normalizedBody.includes('ghn();'),
    moduleShapePinned:
      sourceSummary.normalizedBody === 'Vg(); qp(); jY(); Jk(); P5n(); a5t(); u5t(); HRc(); Mbt(); Ql(); vhn(); ghn();'
      && inputSummary.normalizedBody === 'Vg(); qp(); jY(); Jk(); P5n(); a5t(); u5t(); HRc(); Mbt(); Ql(); vhn(); ghn();',
  };
