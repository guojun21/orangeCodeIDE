...
const PHASE = 'rebuilt';
const PROFILE = 'main-spike';
const RESULT_PATH = path.join(ROOT, 'mapped', 'main-spike-check.json');
const MARKER_OUTPUT_PATH = path.join(ROOT, 'mapped', 'main-spike-marker.json');
const BUILT_PATH = path.join(ROOT, 'recovered', PHASE, 'built', 'main-runtime.js');
const RUNTIME_PATH = path.join(ROOT, 'recovered', PHASE, 'runtime-app', 'out', 'main.js');
...
try {
  ...
  marker = await waitForCondition(async () => {
    if (fs.existsSync(MARKER_OUTPUT_PATH)) {
      ...
      if (
        value?.source === 'rebuilt/src/main/index.js' &&
    ...
    if (exitCode !== null || signalCode !== null) {
      throw new Error(`runtime exited before main marker matured (code=${exitCode}, signal=${signalCode})`);
    }

    return null;
  }, {
    timeoutMs: 30000,
    description: 'main marker matured',
  });
  ...
  checks = [
    ...
    {
      id: 'marker-source',
      passed: marker?.source === 'rebuilt/src/main/index.js',
      detail: marker ?? null,
    },
    {
      id: 'import-resolved',
