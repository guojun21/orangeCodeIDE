...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentieth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'twentieth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-twentieth-live-admission-plan',
    ...
    allowedNow: [
      'composer twentieth dedicated no-op runtime artifacts',
      'composer twentieth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC20L execution until DC20 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in twentieth-candidate transition',
    ],
  };
