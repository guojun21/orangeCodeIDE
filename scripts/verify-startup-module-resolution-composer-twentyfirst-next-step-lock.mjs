...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerTextModelService.js'
      && report.baseline?.nextCandidateWaveId === 'DC21A',
    decisionPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC21L until DC21A dedicated no-op runtime gate completes')
