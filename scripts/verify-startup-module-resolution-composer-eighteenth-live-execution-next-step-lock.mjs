...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-eighteenth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.eighteenthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/worktreeSetupRunner.js'
      && report.baseline?.eighteenthSingleLiveWaveId === 'DC18L',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer eighteenth live freeze artifacts')
