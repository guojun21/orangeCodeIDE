...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerViews.js'
      && report.twentyfirstNoOp?.waveId === 'DC27'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyseventh-runtime-next-step-lock'
