...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-admission.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-admission-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextExecutableStep === 'composer-fourth-candidate-admission-plan'
    ...
    candidatePinned:
      report.approvedCandidate?.waveId === 'DC4A'
      && report.approvedCandidate?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerFileChangeHandlerTypes.js'
      && report.approvedCandidate?.riskClass === 'tiny-browser-handler-types-planning-only',
    runnerUpsPinned:
      runnerUpIds.length === 3
      && runnerUpIds[0] === 'out-build/vs/workbench/contrib/composer/browser/composerChatServiceInterface.js'
      && runnerUpIds[1] === 'out-build/vs/workbench/contrib/composer/browser/utils/debugLogFileUtils.js'
      && runnerUpIds[2] === 'out-build/vs/workbench/contrib/composer/browser/composerBlobStore.js',
    blockedGroupsPinned:
      Array.isArray(report.blockedGroups?.alreadyProvenComposerSingles)
      && report.blockedGroups.alreadyProvenComposerSingles.includes('out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js')
    ...
    admissionOnly:
      ...
      && report.decision?.nextApprovedAction === 'composer-fourth-dedicated-no-op-runtime-plan'
    ...
    comparisonReasonsPresent:
      ...
      && typeof report.comparisons?.notComposerBlobStore === 'string'
