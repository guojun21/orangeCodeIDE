...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfourth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twentyfourth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twentyfirstSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerWakelockManager.js'
      && report.baseline?.twentyfirstSingleLiveWaveId === 'DC24L',
    decisionPinned:
