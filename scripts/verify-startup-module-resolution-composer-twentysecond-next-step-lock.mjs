...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserScreenshotService.js'
      && report.baseline?.nextCandidateWaveId === 'DC22A',
    decisionPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC22L until DC22A dedicated no-op runtime gate completes')
