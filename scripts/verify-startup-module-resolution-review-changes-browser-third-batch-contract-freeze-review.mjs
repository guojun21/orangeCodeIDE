...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-batch-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-batch-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-third-batch-contract-freeze-review',
    baselinePinned:
    ...
    provenPinned:
      ...
      && report.proven?.fifthSingleLiveModuleId === EXPECTED_MODULES[4]
    ...
    batchContractPinned:
      batchContract.batchWaveId === 'DBRB3A'
      && arrayEquals(batchContract.selectedModules, EXPECTED_MODULES)
      && arrayEquals(batchContract.selectedWaves, EXPECTED_WAVES)
      && batchContract.output === 'mapped/contrib-review-changes-browser-third-batch-module-resolution-live-contract.json'
      && batchContract.runtimeCopy === 'mapped/contrib-review-changes-browser-third-batch-module-resolution-live-contract.runtime.json'
      ...
      && batchContract.expectedModuleCount === 5
      && batchContract.observedModuleCount === 5,
    runnerUpLockPinned:
    ...
    minimumWinPinned:
      ...
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-third-batch-export-delta-preflight.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-third-batch-fallback-preflight.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-third-batch-sticky-disable-preflight.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-third-batch-contract-freeze-review-verify.json')
    ...
    decisionPinned:
      ...
      && report.decision?.laneState === 'five-single-live-proven-first-and-second-batch-live-proven-third-batch-contract-ready'
      && report.decision?.nextApprovedStep === 'browser-third-batch-contract-next-step-lock'
    ...
    blockedNowScoped:
      blockedNow.includes('browser third-batch live before explicit run step')
