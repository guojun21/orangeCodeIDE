...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-heavier-ui-export-delta-preflight.json');
...
function main() {
  ...
  const selectedModules = contractPlan.heavierUiContractPlan?.selectedModules ?? [];
  ...
  const report = {
    ...
    lane: 'browser',
    phase: 'browser-heavier-ui-export-delta-preflight',
