...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfifth-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    currentStatePinned:
      ...
      && report.currentState?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserPlaceholderPages.js'
      && report.currentState?.nextCandidateWaveId === 'DC25A',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-ten-single-live-one-micro-batch-twentyfirst-candidate-planned'
      && report.decision?.nextApprovedStep === 'composer-twentyfifth-next-step-lock'
