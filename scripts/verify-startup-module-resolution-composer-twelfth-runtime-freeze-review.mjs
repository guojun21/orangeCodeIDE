...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'twelfth-runtime-freeze-review',
    baselinePinned:
    ...
    twelfthNoOpPinned:
      report.twelfthNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/browserAnalytics.js'
      && report.twelfthNoOp?.waveId === 'DC12'
      && report.twelfthNoOp?.mode === 'no-op-observable'
      && report.twelfthNoOp?.result === 'original-pass'
      && report.twelfthNoOp?.fallbackReason === 'disabled-global'
      && report.twelfthNoOp?.originalPassCount === 1
      && report.twelfthNoOp?.overlayHitCount === 0
      && report.twelfthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-twelfth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-twelfth-runtime-next-step-lock'
      && report.decision?.twelfthNoOpProvenOnly === true
      && report.decision?.twelfthLiveStillBlocked === true
