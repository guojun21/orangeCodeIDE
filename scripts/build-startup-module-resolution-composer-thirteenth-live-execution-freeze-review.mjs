...
const ELEVENTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-thirteenth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-thirteenth-live-execution-freeze-review.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'composer-thirteenth-live-execution-freeze-review',
    sources: {
      thirteenthLiveContractFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      thirteenthLiveGate: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      thirteenthSingleLiveModuleId: moduleId,
      thirteenthSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      ...
      nextApprovedStep: 'composer-thirteenth-live-execution-next-step-lock',
