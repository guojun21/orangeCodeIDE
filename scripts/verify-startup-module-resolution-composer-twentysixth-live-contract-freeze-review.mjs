...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-live-contract-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysixth-live-contract-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    contractPinned:
      report.twentyfirstLiveContract?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composer.js'
      && report.twentyfirstLiveContract?.waveId === 'DC26L'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentysixth-live-contract-next-step-lock'
