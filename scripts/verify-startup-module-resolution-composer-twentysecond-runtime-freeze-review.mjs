...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/browserScreenshotService.js'
      && report.twentyfirstNoOp?.waveId === 'DC22'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentysecond-runtime-next-step-lock'
