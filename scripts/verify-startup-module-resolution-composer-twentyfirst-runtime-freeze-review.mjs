...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyfirst-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    twentyfirstNoOpPinned:
      report.twentyfirstNoOp?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerTextModelService.js'
      && report.twentyfirstNoOp?.waveId === 'DC21'
    ...
    decisionPinned:
      ...
      && report.decision?.nextApprovedStep === 'composer-twentyfirst-runtime-next-step-lock'
