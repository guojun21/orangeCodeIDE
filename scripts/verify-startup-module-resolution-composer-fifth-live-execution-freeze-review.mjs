...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-execution-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fifth-live-execution-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-fifth-live-execution-freeze-review',
    baselinePinned:
    ...
    fifthLivePinned:
      report.proven?.fifthSingleLiveModuleId === moduleId
    ...
    decisionPinned:
      ...
      && report.decision?.laneState === 'contrib-composer-five-single-live-one-micro-batch-proven'
      && report.decision?.nextApprovedStep === 'composer-fifth-live-execution-next-step-lock'
