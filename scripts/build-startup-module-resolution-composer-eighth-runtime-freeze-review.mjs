...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-runtime-freeze-review.json');
...
function main() {
  ...
  const runtimeResult = eighthRuntimeGate.runtimeState?.resolution?.results?.[0]?.result ?? null;
  const diagnostics = eighthRuntimeGate.runtimeState?.resolution?.diagnostics ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eighth-runtime-freeze-review',
    sources: {
    ...
    eighthNoOp: {
      moduleId: eighthAdmission.approvedCandidate?.moduleId ?? null,
      waveId: eighthRuntimeGate.expectedWaveId ?? null,
      mode: eighthRuntimeGate.runtimeState?.resolution?.mode ?? null,
      canaryCount: eighthRuntimeGate.runtimeState?.resolution?.canaryCount ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-seven-single-live-one-micro-batch-eighth-no-op-proven',
      nextApprovedStep: 'composer-eighth-runtime-next-step-lock',
      eighthNoOpProvenOnly: true,
      eighthLiveStillBlocked: true,
