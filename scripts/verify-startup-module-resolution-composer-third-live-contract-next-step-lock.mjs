...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-contract-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-contract-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-two-single-live-one-micro-batch-third-live-contract-ready'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-third-live-execution'
    ...
    allowedNowScoped:
      allowedNow.includes('composer third live contract artifacts')
      && allowedNow.includes('composer third live execution only'),
    blockedNowScoped:
      ...
      && blockedNow.includes('composer third live batch expansion'),
    stopConditionsPinned:
      stopConditions.includes('do not discuss wider composer batch until DC3L live is executed and frozen')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion before DC3L live outcome is frozen'),
  };
