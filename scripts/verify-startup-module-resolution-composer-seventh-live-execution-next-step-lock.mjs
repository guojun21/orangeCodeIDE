...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-seventh-live-execution-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-seven-single-live-one-micro-batch-proven'
    ...
    decisionPinned:
      ...
      && report.decision?.composerLaneFreezeMayStartOnlyAfterSeventhLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer seventh live freeze artifacts')
