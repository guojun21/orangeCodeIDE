...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-second-batch-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-second-batch-next-step-lock',
    baselinePinned:
    ...
    allowedNowPinned:
      allowedNow.includes('browser second-batch admission freeze artifacts')
      && allowedNow.includes('browser second-batch contract planning only'),
    blockedNowPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not execute browser second batch before a dedicated browser-second-batch contract, preflight, and freeze are completed')
      && stopConditions.includes('do not switch to ciMessageUtils.js before second-batch planning outcome is frozen')
      && stopConditions.includes('do not discuss browser component/widget/template or a third browser batch before second-batch execution outcome is frozen'),
    runnerUpLockPinned:
    ...
    minimumWinPinned:
      ...
      && minimumWin.mustVerify.includes('startup-module-resolution-review-changes-browser-second-batch-next-step-lock-verify.json'),
    scopeBoundaryPinned:
      ...
      && scopeBoundary.allowOnly.includes('browser second-batch next-step lock artifacts')
