...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-three-single-live-one-micro-batch-fourth-candidate-planned'
      ...
      && report.baseline?.plannedModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerFileChangeHandlerTypes.js'
      && report.baseline?.plannedWaveId === 'DC4A',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer fourth candidate admission artifacts')
      && allowedNow.includes('composer fourth dedicated no-op runtime planning only'),
    blockedNowScoped:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC4L until DC4A dedicated no-op runtime gate completes')
