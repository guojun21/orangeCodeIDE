...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-runtime-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-runtime-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-two-single-live-one-micro-batch-third-no-op-proven'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-third-live-admission-plan'
    ...
    allowedNowScoped:
      allowedNow.includes('composer third dedicated no-op runtime artifacts')
      && allowedNow.includes('composer third live admission planning only'),
    blockedNowScoped:
      blockedNow.includes('composer third live execution before admission')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC3L execution until DC3 dedicated no-op runtime is frozen and locked')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion while composer continuation is still in third-candidate transition'),
  };
