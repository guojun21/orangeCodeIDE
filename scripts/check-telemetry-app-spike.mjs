...
const PHASE = 'rebuilt';
const PROFILE = 'telemetry-app-spike';
const RESULT_PATH = path.join(ROOT, 'mapped', 'telemetry-app-spike-check.json');
const MARKER_OUTPUT_PATH = path.join(ROOT, 'mapped', 'telemetry-app-spike-marker.json');
const BUILT_PATH = path.join(ROOT, 'recovered', PHASE, 'built', 'telemetry-app.js');
const RUNTIME_PATH = path.join(ROOT, 'recovered', PHASE, 'runtime-app', 'out', 'vs', 'workbench', 'contrib', 'debug', 'node', 'telemetryApp.js');
...
const child = spawn(process.execPath, [RUNTIME_PATH, 'telemetryAppTest', '{}', 'mockAuthId'], {
  cwd: ROOT,
  env: {
    ...process.env,
    SHOPEECODE_REBUILT_PROFILE: PROFILE,
...
try {
  marker = await waitForCondition(async () => {
    ...
    timeoutMs: 15000,
    description: 'telemetry app marker file written',
  });
  ...
  checks = [
    ...
    {
      id: 'marker-source',
      passed: marker?.source === 'rebuilt/src/vs/workbench/contrib/debug/node/telemetryApp/index.js',
      detail: marker ?? null,
    },
    {
      id: 'argv-forwarded',
      passed: Array.isArray(marker?.argv) && marker.argv[0] === 'telemetryAppTest' && marker.argv[1] === '{}',
      detail: marker ?? null,
    },
    {
      id: 'import-resolved',
