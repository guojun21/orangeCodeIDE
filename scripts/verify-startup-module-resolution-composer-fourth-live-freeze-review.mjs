...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourth-live-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    fourthLivePlanPinned:
      report.fourthLivePlan?.moduleId === 'out-build/vs/workbench/contrib/composer/browser/composerFileChangeHandlerTypes.js'
      && report.fourthLivePlan?.waveId === 'DC4L'
      && report.fourthLivePlan?.liveShape === 'single-module-live',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-three-single-live-one-micro-batch-fourth-live-admission-planned'
      && report.decision?.nextApprovedStep === 'composer-fourth-live-next-step-lock'
