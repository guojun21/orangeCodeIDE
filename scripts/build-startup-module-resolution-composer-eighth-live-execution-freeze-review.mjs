...
const EIGHTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-eighth-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(EIGHTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(EIGHTH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    phase: 'composer-eighth-live-execution-freeze-review',
    sources: {
      eighthLiveContractFreezeReview: normalizePath(path.relative(ROOT, EIGHTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      eighthLiveGate: normalizePath(path.relative(ROOT, EIGHTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      eighthSingleLiveModuleId: moduleId,
      eighthSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-eight-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-eighth-live-execution-next-step-lock',
