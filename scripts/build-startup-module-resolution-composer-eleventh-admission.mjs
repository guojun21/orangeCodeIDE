...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-admission.json');
...
function main() {
  ...
  const moduleId = 'out-build/vs/workbench/contrib/composer/browser/composerDataCreation.js';
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/composerDataCreation.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/composerDataCreation.js';

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eleventh-admission',
    sources: {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerDataCreation.js is now the top-ranked remaining staged composer browser candidate once worktreeGate.js is frozen into the proven set',
        ...
        'current canary ranking places composerDataCreation.js ahead of the broader composer browser surface after ten single-live waves and one micro-batch are frozen',
      ],
      riskClass: 'browser-composer-data-creation-planning-only',
    },
    currentState: {
    ...
    blockedNow: [
      'composer eleventh live execution',
    ...
    decision: {
      ...
      approvedWaveId: 'DC11A',
      nextApprovedAction: 'composer-eleventh-dedicated-no-op-runtime-plan',
