...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-runtime-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-runtime-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'eighth-runtime-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-seven-single-live-one-micro-batch-eighth-no-op-proven'
    ...
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-eighth-live-admission-plan'
    ...
    allowedNowScoped:
      allowedNow.includes('composer eighth dedicated no-op runtime artifacts')
      && allowedNow.includes('composer eighth live admission planning only'),
    blockedNowScoped:
      blockedNow.includes('composer eighth live execution before admission')
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC8L execution until DC8 dedicated no-op runtime is frozen and locked')
      && stopConditions.includes('do not reopen reviewChanges or browser lane expansion while composer continuation is still in eighth-candidate transition'),
  };
