...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-third-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'third-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'two-single-live-proven-third-candidate-planned-still-no-batch'
      ...
      && report.baseline?.plannedModuleId === 'out-build/vs/workbench/contrib/reviewChanges/browser/utils/diffMentionUtils.js'
      && report.baseline?.plannedWaveId === 'DBR3A',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('browser third candidate admission artifacts')
      && allowedNow.includes('browser third dedicated no-op runtime planning only'),
    blockedNowScoped:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DBR3L until DBR3A dedicated no-op runtime gate completes')
      && stopConditions.includes('do not approve browser batch while only two browser single-live and one third admission candidate are frozen'),
  };
