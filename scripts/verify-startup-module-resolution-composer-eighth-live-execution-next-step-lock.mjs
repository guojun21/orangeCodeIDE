...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-eighth-live-execution-next-step-lock',
    baselinePinned:
      report.baseline?.laneState === 'contrib-composer-eight-single-live-one-micro-batch-proven'
    ...
    decisionPinned:
      ...
      && report.decision?.composerLaneFreezeMayStartOnlyAfterEighthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer eighth live freeze artifacts')
