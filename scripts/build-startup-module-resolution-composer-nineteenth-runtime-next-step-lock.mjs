...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-nineteenth-runtime-next-step-lock.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'nineteenth-runtime-next-step-lock',
    sources: {
    ...
    decision: {
      nextApprovedAction: 'composer-nineteenth-live-admission-plan',
    ...
    allowedNow: [
      'composer nineteenth dedicated no-op runtime artifacts',
      'composer nineteenth live admission planning only',
    ...
    stopConditions: [
      'do not discuss DC19L execution until DC19 dedicated no-op runtime is frozen and locked',
      'do not reopen reviewChanges or browser lane expansion while composer continuation is still in nineteenth-candidate transition',
    ],
  };
