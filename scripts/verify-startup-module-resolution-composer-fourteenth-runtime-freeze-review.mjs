...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourteenth-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-fourteenth-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'fourteenth-runtime-freeze-review',
    baselinePinned:
    ...
    fourteenthNoOpPinned:
      ...
      && report.fourteenthNoOp?.result === 'original-pass'
      && report.fourteenthNoOp?.fallbackReason === 'disabled-global'
      && report.fourteenthNoOp?.originalPassCount === 1
      && report.fourteenthNoOp?.overlayHitCount === 0
      && report.fourteenthNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-eleven-single-live-one-micro-batch-fourteenth-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-fourteenth-runtime-next-step-lock'
      && report.decision?.fourteenthNoOpProvenOnly === true
      && report.decision?.fourteenthLiveStillBlocked === true
