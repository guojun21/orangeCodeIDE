...
const ELEVENTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-eleventh-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-live-execution-freeze-review.json');
...
function main() {
  ...
  const report = {
    ...
    phase: 'composer-eleventh-live-execution-freeze-review',
    sources: {
      eleventhLiveContractFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      eleventhLiveGate: normalizePath(path.relative(ROOT, ELEVENTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      eleventhSingleLiveModuleId: moduleId,
      eleventhSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-eleven-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-eleventh-live-execution-next-step-lock',
