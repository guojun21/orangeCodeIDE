...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twelfth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twelfth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twelfthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserAnalytics.js'
      && report.baseline?.twelfthSingleLiveWaveId === 'DC12L',
    decisionPinned:
      report.decision?.nextApprovedAction === 'composer-lane-freeze-review'
      && report.decision?.composerLaneFreezeMayStartOnlyAfterTwelfthLiveFreeze === true
    ...
    allowedNowScoped:
      allowedNow.includes('composer twelfth live freeze artifacts')
