...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentyseventh-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twentyseventh-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twentyfirstSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerViews.js'
      && report.baseline?.twentyfirstSingleLiveWaveId === 'DC27L',
    decisionPinned:
