...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    currentStatePinned:
      ...
      && report.currentState?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerMultiDiffContentProvider.js'
      && report.currentState?.nextCandidateWaveId === 'DC23A',
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-ten-single-live-one-micro-batch-twentyfirst-candidate-planned'
      && report.decision?.nextApprovedStep === 'composer-twentythird-next-step-lock'
