...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-batch-contract-plan.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-review-changes-browser-batch-contract-plan-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'browser-batch-contract-plan',
    baselinePinned:
    ...
    contractPlanPinned:
      plan.batchWaveId === 'DBRB1A'
      && arrayEquals(plan.selectedModules, EXPECTED_MODULES)
      && arrayEquals(plan.selectedWaves, EXPECTED_WAVES)
      && plan.output === 'mapped/contrib-review-changes-browser-batch-module-resolution-live-contract.json'
      && plan.runtimeCopy === 'mapped/contrib-review-changes-browser-batch-module-resolution-live-contract.runtime.json'
    ...
    minimumWinPinned:
      minimumWin.definition === 'browser batch contract plan verify green and nextApprovedAction uniquely locked to browser-batch-contract-preflight'
    ...
    followUpPinned:
      followUpPriority.next === 'browser-batch-contract-preflight'
      && followUpPriority.afterThat === 'browser-batch-contract-freeze-review',
    decisionPinned:
      report.decision?.contractPlanReady === true
      && report.decision?.nextApprovedAction === 'browser-batch-contract-preflight'
