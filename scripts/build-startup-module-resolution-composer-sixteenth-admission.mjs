...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixteenth-admission.json');
...
function main() {
  ...
  const moduleId = 'out-build/vs/workbench/contrib/composer/browser/composerAgentProviderRouter.js';
  const sourceFile = 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/composerAgentProviderRouter.js';
  const runtimeInputFile = 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/composerAgentProviderRouter.js';

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'sixteenth-admission',
    sources: {
    ...
    approvedCandidate: {
      ...
      runtimeInputFile,
      rationale: [
        'composerAgentProviderRouter.js is now the top-ranked remaining staged composer browser candidate once composerDataCreation.js is frozen into the proven set',
        ...
        'current canary ranking places composerAgentProviderRouter.js ahead of the broader composer browser surface after eleven single-live waves and one micro-batch are frozen',
      ],
    ...
    blockedNow: [
      'composer sixteenth live execution',
    ...
    decision: {
      ...
      approvedWaveId: 'DC16A',
      nextApprovedAction: 'composer-sixteenth-dedicated-no-op-runtime-plan',
