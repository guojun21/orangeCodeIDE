...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-runtime-freeze-review.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eleventh-runtime-freeze-review-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'eleventh-runtime-freeze-review',
    baselinePinned:
    ...
    eleventhNoOpPinned:
      ...
      && report.eleventhNoOp?.result === 'original-pass'
      && report.eleventhNoOp?.fallbackReason === 'disabled-global'
      && report.eleventhNoOp?.originalPassCount === 1
      && report.eleventhNoOp?.overlayHitCount === 0
      && report.eleventhNoOp?.stickyDisabledCount === 0,
    decisionPinned:
      report.decision?.laneFrozen === true
      && report.decision?.laneState === 'contrib-composer-ten-single-live-one-micro-batch-eleventh-no-op-proven'
      && report.decision?.nextApprovedStep === 'composer-eleventh-runtime-next-step-lock'
      && report.decision?.eleventhNoOpProvenOnly === true
      && report.decision?.eleventhLiveStillBlocked === true
