...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixteenth-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-sixteenth-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-sixteenth-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.sixteenthSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerAgentProviderRouter.js'
      && report.baseline?.sixteenthSingleLiveWaveId === 'DC16L',
    decisionPinned:
    ...
    allowedNowScoped:
      allowedNow.includes('composer sixteenth live freeze artifacts')
