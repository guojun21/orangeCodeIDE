...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentythird-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    baselinePinned:
      ...
      && report.baseline?.nextCandidateModuleId === 'out-build/vs/workbench/contrib/composer/browser/composerMultiDiffContentProvider.js'
      && report.baseline?.nextCandidateWaveId === 'DC23A',
    decisionPinned:
    ...
    stopConditionsPinned:
      stopConditions.includes('do not discuss DC23L until DC23A dedicated no-op runtime gate completes')
