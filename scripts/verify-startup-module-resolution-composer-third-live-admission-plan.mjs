...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-admission-plan.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-third-live-admission-plan-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    candidatePinned:
      report.candidate?.waveId === 'DC3L'
      && report.candidate?.moduleId === moduleId
      && report.candidate?.sourceFile === 'rebuilt/src/project-modules-beautified/out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js'
      && report.candidate?.runtimeInputFile === 'recovered/startup-loader/input/out-build/vs/workbench/contrib/composer/browser/composerModelFilters.js'
      && report.candidate?.liveShape === 'single-module-live',
    rationalePinned:
      Array.isArray(rationale)
      && rationale.includes('tiny browser-path composer filter module with no visible DOM/component/widget touch points')
      && rationale.includes('no manager/stateful resource-holder behavior exposed in the recovered slice')
      && rationale.includes('source and startup-loader input remain aligned after DC3 no-op runtime freeze')
      && rationale.includes('runner-ups still carry broader handler, interface, or debug-log semantics than composerModelFilters.js'),
    prerequisitesPinned:
      prerequisites.thirdNoOpPassed === true
      && prerequisites.thirdNoOpWaveId === 'DC3'
    ...
    requiredBeforeLivePresent:
      requiredBeforeLive.includes('build DC3L live contract')
      && requiredBeforeLive.includes('run composer-third export-delta gate')
      && requiredBeforeLive.includes('run composer-third fallback preflight')
      && requiredBeforeLive.includes('run composer-third sticky-disable preflight')
    ...
    stopConditionsPresent:
      ...
      && stopConditions.includes('DC3L single-live gate does not record overlay-hit')
      && stopConditions.includes('do not discuss wider composer batch while DC3 is only no-op proven'),
    runnerUpLockPinned:
    ...
    widerBatchEligibilityGatePinned:
      ...
      && widerBatchEligibilityGate.currentProofState === 'two-single-live-plus-first-micro-batch-plus-third-no-op'
      ...
      && widerBatchEligibilityGate.requiredMilestonesBeforeDiscussion.includes('DC3L single-live proven')
      && widerBatchEligibilityGate.requiredMilestonesBeforeDiscussion.includes('post-DC3L composer lane freeze review'),
    minimumWinPinned:
      minimumWin.definition === 'DC3L admission plan / freeze / next-step lock all verify green and nextApprovedAction uniquely locked to composer-third-live-contract'
      && Array.isArray(minimumWin.mustVerify)
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-third-live-admission-plan-verify.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-third-live-freeze-review-verify.json')
      && minimumWin.mustVerify.includes('startup-module-resolution-composer-third-live-next-step-lock-verify.json'),
    scopeBoundaryPinned:
      Array.isArray(scopeBoundary.allowOnly)
      && scopeBoundary.allowOnly.includes('DC3L admission artifacts')
      && scopeBoundary.allowOnly.includes('composer third live freeze artifacts')
      && scopeBoundary.allowOnly.includes('composer third live next-step lock artifacts')
    ...
    reachabilityAssessmentPinned:
      ...
      && reachabilityAssessment.safestShortPath === 'DC3L admission -> third-live freeze review -> third-live next-step lock -> composer-third-live-contract'
      && typeof reachabilityAssessment.biggestGap === 'string',
    followUpPriorityPinned:
      followUpPriority.next === 'composer-third-live-contract'
    ...
    nextActionPinned:
      ...
      && report.decision?.nextApprovedWaveId === 'DC3L'
