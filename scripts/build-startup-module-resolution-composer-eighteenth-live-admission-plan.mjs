...
const ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-runtime-next-step-lock.json');
...
const ELEVENTH_RUNTIME_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-eighteenth-module-resolution-runtime-gate.json');
const ELEVENTH_ADMISSION_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-admission.json');
...
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-eighteenth-live-admission-plan.json');
...
function main() {
  const eighteenthRuntimeLock = readJson(ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH);
  const eighteenthRuntimeFreeze = readJson(ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH);
  const eighteenthRuntimeGate = readJson(ELEVENTH_RUNTIME_GATE_PATH);
  const eighteenthAdmission = readJson(ELEVENTH_ADMISSION_PATH);
  ...
  const moduleId = eighteenthAdmission.approvedCandidate?.moduleId ?? null;
  const sourceFile = eighteenthAdmission.approvedCandidate?.sourceFile ?? null;
  const runtimeInputFile = eighteenthAdmission.approvedCandidate?.runtimeInputFile ?? null;

  const report = {
    generatedAt: new Date().toISOString(),
    lane: 'contrib-composer',
    phase: 'eighteenth-live-admission-plan',
    sources: {
      eighteenthRuntimeNextStepLock: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_NEXT_STEP_LOCK_PATH)),
      eighteenthRuntimeFreezeReview: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_FREEZE_REVIEW_PATH)),
      eighteenthRuntimeGate: normalizePath(path.relative(ROOT, ELEVENTH_RUNTIME_GATE_PATH)),
      eighteenthAdmission: normalizePath(path.relative(ROOT, ELEVENTH_ADMISSION_PATH)),
    ...
    candidate: {
      ...
      runtimeInputFile,
      rationale: [
        'worktreeSetupRunner.js is the next narrow composer browser analytics candidate after composerDataCreation.js is frozen into the proven set',
        ...
        'source and startup-loader input remain aligned after DC18 dedicated no-op runtime freeze',
        'runner-ups still fan out into broader browser surface, storage, agent coordination, or editor state than worktreeSetupRunner.js',
      ],
      liveShape: 'single-module-live',
    },
    prerequisites: {
      eighteenthNoOpPassed: eighteenthRuntimeGate.passed === true,
      eighteenthNoOpWaveId: eighteenthRuntimeGate.expectedWaveId ?? null,
      ...
      reviewChangesLaneStillFrozen: eighteenthRuntimeLock.decision?.reviewChangesLaneFrozen === true,
      broadBrowserStillHeld: eighteenthRuntimeLock.decision?.broadBrowserHeld === true,
    },
    blockedSurfaces: [
    ...
    requiredBeforeLive: [
      'build DC18L live contract',
      'run composer-eighteenth export-delta gate',
      'run composer-eighteenth fallback preflight',
      'run composer-eighteenth sticky-disable preflight',
      'apply wrapper patch',
      'run live gate',
      'run smoke',
    ...
    decision: {
      ...
      nextApprovedAction: 'composer-eighteenth-live-contract',
      nextApprovedWaveId: 'DC18L',
