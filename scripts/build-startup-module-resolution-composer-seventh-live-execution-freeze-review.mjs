...
const SEVENTH_LIVE_GATE_PATH = path.join(ROOT, 'mapped', 'contrib-composer-seventh-module-resolution-live-gate.json');
const ACCEPT_PATH = path.join(ROOT, 'mapped', 'accept-latest.json');
const QUALITY_PATH = path.join(ROOT, 'mapped', 'quality-report.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-seventh-live-execution-freeze-review.json');
...
function main() {
  const contractFreezeReview = readJson(SEVENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH);
  const liveGate = readJson(SEVENTH_LIVE_GATE_PATH);
  ...
  const report = {
    ...
    phase: 'composer-seventh-live-execution-freeze-review',
    sources: {
      seventhLiveContractFreezeReview: normalizePath(path.relative(ROOT, SEVENTH_LIVE_CONTRACT_FREEZE_REVIEW_PATH)),
      seventhLiveGate: normalizePath(path.relative(ROOT, SEVENTH_LIVE_GATE_PATH)),
    ...
    proven: {
      ...
      seventhSingleLiveModuleId: moduleId,
      seventhSingleLiveWaveId: liveGate.expectedWaveId ?? null,
    ...
    decision: {
      laneFrozen: true,
      laneState: 'contrib-composer-seven-single-live-one-micro-batch-proven',
      nextApprovedStep: 'composer-seventh-live-execution-next-step-lock',
