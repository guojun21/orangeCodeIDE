...
const INPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-live-execution-next-step-lock.json');
const OUTPUT_PATH = path.join(ROOT, 'mapped', 'startup-module-resolution-composer-twentysecond-live-execution-next-step-lock-verify.json');
...
function main() {
  ...
  const checks = {
    ...
    phasePinned: report.phase === 'composer-twentysecond-live-execution-next-step-lock',
    baselinePinned:
      ...
      && report.baseline?.twentyfirstSingleLiveModuleId === 'out-build/vs/workbench/contrib/composer/browser/browserScreenshotService.js'
      && report.baseline?.twentyfirstSingleLiveWaveId === 'DC22L',
    decisionPinned:
